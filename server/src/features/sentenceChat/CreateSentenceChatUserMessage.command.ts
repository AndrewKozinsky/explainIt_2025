...
> {
	...
	async execute(command: CreateSentenceChatUserMessageCommand): Promise<SentenceChatMessageOutModel> {
		...
		if (!trimmedQuestion) {
			throw new CustomGraphQLError(errorMessage.sentenceChat.questionIsEmpty, ErrorCode.BadRequest_400)
		}
		...
		if (lastMessage && (lastMessage.role !== 'assistant' || lastMessage.status === 'streaming')) {
			throw new CustomGraphQLError(errorMessage.sentenceChat.previousAnswerNotReady, ErrorCode.BadRequest_400)
		}
