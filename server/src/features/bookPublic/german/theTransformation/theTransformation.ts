...
export function theTransformationBookData(s3FolderName: string) {
	...
	return {
		...
		about: `Грегор Замза, молодой коммивояжёр, однажды утром просыпается в теле огромного жука. Он не столько ужасается своему облику, сколько переживает, что опоздает на работу — ведь он кормит всю семью.
