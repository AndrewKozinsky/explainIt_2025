export const chapter_8 = {
	name: 'Chapter VIII',
	header: 'The Deadly Poppy Field',
	data: [
		{
			sentence:
				'Our little party of travelers awakened the next morning refreshed and full of hope, and Dorothy breakfasted like a princess off peaches and plums from the trees beside the river.',
			translate:
				'Наша маленькая компания путешественников проснулась на следующее утро бодрой и исполненной надежды, а Дороти позавтракала, как принцесса, персиками и сливами с деревьев у берега реки.',
		},
		{
			sentence:
				'Behind them was the dark forest they had passed safely through, although they had suffered many discouragements; but before them was a lovely, sunny country that seemed to beckon them on to the Emerald City.',
			translate:
				'Позади остался тёмный лес, через который они благополучно прошли, хотя пережили немало испытаний; а впереди раскинулась прекрасная, солнечная страна, словно манившая их к Изумрудному городу.',
		},
		{
			sentence: 'To be sure, the broad river now cut them off from this beautiful land.',
			translate: 'Правда, теперь широкая река отрезала их от этой прекрасной страны.',
		},
		{
			sentence:
				'But the raft was nearly done, and after the Tin Woodman had cut a few more logs and fastened them together with wooden pins, they were ready to start.',
			translate:
				'Но плот был почти готов, и, когда Железный Дровосек нарубил ещё несколько брёвен и скрепил их деревянными штырями, они были готовы отплывать.',
		},
		{
			sentence: 'Dorothy sat down in the middle of the raft and held Toto in her arms.',
			translate: 'Дороти села посреди плота и прижала Тото к себе.',
		},
		{
			sentence:
				'When the Cowardly Lion stepped upon the raft it tipped badly, for he was big and heavy; but the Scarecrow and the Tin Woodman stood upon the other end to steady it, and they had long poles in their hands to push the raft through the water.',
			translate:
				'Когда Трусливый Лев ступил на плот, тот сильно накренился — Лев был большой и тяжёлый; но Пугало и Железный Дровосек встали на другой конец, чтобы удержать равновесие, и взяли в руки длинные шесты, чтобы толкать плот по воде.',
		},
		{
			sentence:
				'They got along quite well at first, but when they reached the middle of the river the swift current swept the raft downstream, farther and farther away from the road of yellow brick.',
			translate:
				'Сначала всё шло довольно хорошо, но когда они доплыли до середины реки, быстрое течение понесло плот вниз по течению, всё дальше и дальше от жёлтой кирпичной дороги.',
		},
		{
			sentence: 'And the water grew so deep that the long poles would not touch the bottom.',
			translate: 'И вода стала такой глубокой, что длинные шесты уже не доставали до дна.',
		},
		{
			sentence:
				'“This is bad,” said the Tin Woodman, “for if we cannot get to the land we shall be carried into the country of the Wicked Witch of the West, and she will enchant us and make us her slaves.” “And then I should get no brains,” said the Scarecrow.',
			translate:
				'«Дело плохо, — сказал Железный Дровосек. — Если мы не сможем добраться до берега, нас унесёт в страну Злой Ведьмы Запада, и она околдует нас и обратит в своих рабов». — «А тогда я так и не получу мозгов», — сказал Пугало.',
		},
		{
			sentence: '“And I should get no courage,” said the Cowardly Lion.',
			translate: '«А я не получу храбрости», — сказал Трусливый Лев.',
		},
		{
			sentence: '“And I should get no heart,” said the Tin Woodman.',
			translate: '«А я не получу сердца», — сказал Железный Дровосек.',
		},
		{
			sentence: '“And I should never get back to Kansas,” said Dorothy.',
			translate: '«А я никогда не вернусь в Канзас», — сказала Дороти.',
		},
		{
			sentence:
				'“We must certainly get to the Emerald City if we can,” the Scarecrow continued, and he pushed so hard on his long pole that it stuck fast in the mud at the bottom of the river.',
			translate:
				'«Нам во что бы то ни стало нужно добраться до Изумрудного города, если сможем», — продолжал Пугало и так сильно налёг на свой длинный шест, что тот увяз в иле на дне реки.',
		},
		{
			sentence:
				'Then, before he could pull it out again—or let go—the raft was swept away, and the poor Scarecrow was left clinging to the pole in the middle of the river.',
			translate:
				'И прежде чем он смог вытащить шест — или отпустить его — плот уже унесло течением, и бедное Пугало осталось, вцепившись в шест, посреди реки.',
		},
		{
			sentence: '“Good-bye!” he called after them, and they were very sorry to leave him.',
			translate: '«Прощайте!» — крикнул он им вслед, и всем было очень жаль оставлять его.',
		},
		{
			sentence:
				'Indeed, the Tin Woodman began to cry, but fortunately remembered that he might rust, and so dried his tears on Dorothy’s apron.',
			translate:
				'Железный Дровосек даже расплакался, но, к счастью, вовремя вспомнил, что может заржаветь, и вытер слёзы о фартук Дороти.',
		},
		{
			sentence: 'Of course this was a bad thing for the Scarecrow.',
			translate: 'Конечно, для Пугала это было очень плохо.',
		},
		{
			sentence: '“I am now worse off than when I first met Dorothy,” he thought.',
			translate: '«Теперь мне ещё хуже, чем когда я впервые встретил Дороти», — подумал он.',
		},
		{
			sentence:
				'“Then, I was stuck on a pole in a cornfield, where I could make-believe scare the crows, at any rate.',
			translate:
				'«Тогда я торчал на шесте в кукурузном поле, где, по крайней мере, мог делать вид, что пугаю ворон.',
		},
		{
			sentence: 'But surely there is no use for a Scarecrow stuck on a pole in the middle of a river.',
			translate: 'Но, конечно, нет никакого толку от Пугала, насаженного на шест посреди реки.',
		},
		{
			sentence:
				'I am afraid I shall never have any brains, after all!” Down the stream the raft floated, and the poor Scarecrow was left far behind.',
			translate:
				'Боюсь, мне так и не достанутся никакие мозги!» Плот тем временем плыл вниз по течению, и бедное Пугало осталось далеко позади.',
		},
		{
			sentence: 'Then the Lion said: “Something must be done to save us.',
			translate: 'Тут Лев сказал: «Надо что-то сделать, чтобы спасти нас.',
		},
		{
			sentence:
				'I think I can swim to the shore and pull the raft after me, if you will only hold fast to the tip of my tail.” So he sprang into the water, and the Tin Woodman caught fast hold of his tail.',
			translate:
				'Думаю, я смогу доплыть до берега и потянуть за собой плот, если вы только крепко удержитесь за кончик моего хвоста». С этими словами он прыгнул в воду, а Железный Дровосек крепко ухватился за его хвост.',
		},
		{
			sentence: 'Then the Lion began to swim with all his might toward the shore.',
			translate: 'Тогда Лев изо всех сил поплыл к берегу.',
		},
		{
			sentence:
				'It was hard work, although he was so big; but by and by they were drawn out of the current, and then Dorothy took the Tin Woodman’s long pole and helped push the raft to the land.',
			translate:
				'Это было нелёгкое дело, хотя он был очень велик; но вскоре их вынесло из быстрого течения, и тогда Дороти взяла длинный шест Железного Дровосека и помогла подтолкнуть плот к берегу.',
		},
		{
			sentence:
				'They were all tired out when they reached the shore at last and stepped off upon the pretty green grass, and they also knew that the stream had carried them a long way past the road of yellow brick that led to the Emerald City.',
			translate:
				'Когда наконец они достигли берега и сошли на красивую зелёную траву, все были совершенно вымотаны, и к тому же знали, что течение унесло их далеко мимо жёлтой кирпичной дороги, ведущей к Изумрудному городу.',
		},
		{
			sentence:
				'“What shall we do now?” asked the Tin Woodman, as the Lion lay down on the grass to let the sun dry him.',
			translate:
				'«Что же нам теперь делать?» — спросил Железный Дровосек, а Лев лёг на траву, чтобы солнце его обсушило.',
		},
		{
			sentence: '“We must get back to the road, in some way,” said Dorothy.',
			translate: '«Надо как-нибудь вернуться к дороге», — сказала Дороти.',
		},
		{
			sentence:
				'“The best plan will be to walk along the riverbank until we come to the road again,” remarked the Lion.',
			translate: '«Лучше всего идти вдоль берега реки, пока снова не выйдем к дороге», — заметил Лев.',
		},
		{
			sentence:
				'So, when they were rested, Dorothy picked up her basket and they started along the grassy bank, to the road from which the river had carried them.',
			translate:
				'И когда они отдохнули, Дороти взяла свою корзинку, и они двинулись по травянистому берегу к дороге, от которой их унесла река.',
		},
		{
			sentence:
				'It was a lovely country, with plenty of flowers and fruit trees and sunshine to cheer them, and had they not felt so sorry for the poor Scarecrow, they could have been very happy.',
			translate:
				'Это была прекрасная местность, с множеством цветов, фруктовых деревьев и солнечного света, поднимавшего им настроение, и если бы им не было так жалко бедного Пугала, они могли бы быть очень счастливы.',
		},
		{
			sentence:
				'They walked along as fast as they could, Dorothy only stopping once to pick a beautiful flower; and after a time the Tin Woodman cried out: “Look!” Then they all looked at the river and saw the Scarecrow perched upon his pole in the middle of the water, looking very lonely and sad.',
			translate:
				'Они шли так быстро, как могли; Дороти лишь раз остановилась, чтобы сорвать прекрасный цветок. И вскоре Железный Дровосек вскрикнул: «Смотрите!» Все обернулись к реке и увидели Пугало, сидевшее на своём шесте посреди воды, такое одинокое и печальное.',
		},
		{
			sentence: '“What can we do to save him?” asked Dorothy.',
			translate: '«Что мы можем сделать, чтобы спасти его?» — спросила Дороти.',
		},
		{
			sentence: 'The Lion and the Woodman both shook their heads, for they did not know.',
			translate: 'Лев и Дровосек лишь покачали головами — они не знали.',
		},
		{
			sentence:
				'So they sat down upon the bank and gazed wistfully at the Scarecrow until a Stork flew by, who, upon seeing them, stopped to rest at the water’s edge.',
			translate:
				'Они сели на берегу и с тоской смотрели на Пугало, пока мимо не пролетел Аист, который, увидев их, опустился отдохнуть у воды.',
		},
		{
			sentence: '“Who are you and where are you going?” asked the Stork.',
			translate: '«Кто вы и куда идёте?» — спросила Аистиха.',
		},
		{
			sentence:
				'“I am Dorothy,” answered the girl, “and these are my friends, the Tin Woodman and the Cowardly Lion; and we are going to the Emerald City.” “This isn’t the road,” said the Stork, as she twisted her long neck and looked sharply at the queer party.',
			translate:
				'«Я Дороти, — ответила девочка, — а это мои друзья, Железный Дровосек и Трусливый Лев; мы идём в Изумрудный город». — «Это не та дорога», — сказала Аистиха, изогнув длинную шею и пристально оглядев эту странную компанию.',
		},
		{
			sentence:
				'“I know it,” returned Dorothy, “but we have lost the Scarecrow, and are wondering how we shall get him again.” “Where is he?” asked the Stork.',
			translate:
				'«Я знаю, — ответила Дороти, — но мы потеряли Пугало и не знаем, как вернуть его». — «Где он?» — спросила Аистиха.',
		},
		{
			sentence: '“Over there in the river,” answered the little girl.',
			translate: '«Вон там, в реке», — ответила девочка.',
		},
		{
			sentence: '“If he wasn’t so big and heavy I would get him for you,” remarked the Stork.',
			translate: '«Если бы он не был таким большим и тяжёлым, я бы достала его для вас», — заметила Аистиха.',
		},
		{
			sentence:
				'“He isn’t heavy a bit,” said Dorothy eagerly, “for he is stuffed with straw; and if you will bring him back to us, we shall thank you ever and ever so much.” “Well, I’ll try,” said the Stork, “but if I find he is too heavy to carry I shall have to drop him in the river again.” So the big bird flew into the air and over the water till she came to where the Scarecrow was perched upon his pole.',
			translate:
				'«Он совсем не тяжёлый, — поспешно сказала Дороти, — он набит соломой; если вы принесёте его обратно, мы будем вам бесконечно благодарны». — «Ну, я попробую, — сказала Аистиха, — но если окажется, что он для меня тяжёл, мне придётся снова уронить его в реку». Большая птица взмыла в воздух и полетела над водой туда, где Пугало сидело на шесте.',
		},
		{
			sentence:
				'Then the Stork with her great claws grabbed the Scarecrow by the arm and carried him up into the air and back to the bank, where Dorothy and the Lion and the Tin Woodman and Toto were sitting.',
			translate:
				'Затем Аистиха своими мощными когтями схватила Пугало за руку, подняла его в воздух и перенесла на берег, где сидели Дороти, Лев, Железный Дровосек и Тото.',
		},
		{
			sentence:
				'When the Scarecrow found himself among his friends again, he was so happy that he hugged them all, even the Lion and Toto; and as they walked along he sang “Tol-de-ri-de-oh!” at every step, he felt so gay.',
			translate:
				'Когда Пугало снова очутилось среди друзей, оно было настолько счастливо, что обняло всех, даже Льва и Тото; и, шагая дальше, распевало на каждом шагу: «Тол-де-ри-де-ох!», так ему было весело.',
		},
		{
			sentence:
				'“I was afraid I should have to stay in the river forever,” he said, “but the kind Stork saved me, and if I ever get any brains I shall find the Stork again and do her some kindness in return.” “That’s all right,” said the Stork, who was flying along beside them.',
			translate:
				'«Я боялся, что мне придётся навсегда остаться в реке, — сказал он, — но добрая Аистиха спасла меня, и если у меня когда-нибудь появятся мозги, я разыщу её и отплачу ей добром». — «Это ничего», — сказала Аистиха, летевшая рядом с ними.',
		},
		{
			sentence: '“I always like to help anyone in trouble.',
			translate: '«Я всегда люблю помогать тем, кто в беде.',
		},
		{
			sentence: 'But I must go now, for my babies are waiting in the nest for me.',
			translate: 'Но теперь мне нужно лететь: мои детки ждут меня в гнезде».',
		},
		{
			sentence:
				'I hope you will find the Emerald City and that Oz will help you.” “Thank you,” replied Dorothy, and then the kind Stork flew into the air and was soon out of sight.',
			translate:
				'«Надеюсь, вы найдёте Изумрудный город и Оз поможет вам». — «Спасибо», — ответила Дороти, и добрая Аистиха взмыла в воздух и вскоре скрылась из виду.',
		},
		{
			sentence:
				'They walked along listening to the singing of the brightly colored birds and looking at the lovely flowers which now became so thick that the ground was carpeted with them.',
			translate:
				'Они шли дальше, слушая пение ярких птиц и любуясь прекрасными цветами, которых становилось так много, что земля словно покрылась ковром.',
		},
		{
			sentence:
				'There were big yellow and white and blue and purple blossoms, besides great clusters of scarlet poppies, which were so brilliant in color they almost dazzled Dorothy’s eyes.',
			translate:
				'Повсюду были крупные жёлтые, белые, синие и пурпурные цветы, а ещё — большие заросли алых маков, такие ослепительно яркие, что почти слепили глаза Дороти.',
		},
		{
			sentence:
				'“Aren’t they beautiful?” the girl asked, as she breathed in the spicy scent of the bright flowers.',
			translate: '«Разве они не чудесны?» — спросила девочка, вдыхая пряный аромат ярких цветов.',
		},
		{
			sentence: '“I suppose so,” answered the Scarecrow.',
			translate: '«Полагаю, что да», — ответило Пугало.',
		},
		{
			sentence:
				'“When I have brains, I shall probably like them better.” “If I only had a heart, I should love them,” added the Tin Woodman.',
			translate:
				'«Когда у меня будут мозги, вероятно, они понравятся мне ещё больше». — «Если бы у меня было сердце, я бы их полюбил», — добавил Железный Дровосек.',
		},
		{
			sentence: '“I always did like flowers,” said the Lion.',
			translate: '«А мне цветы всегда нравились», — сказал Лев.',
		},
		{
			sentence: '“They seem so helpless and frail.',
			translate: '«Они кажутся такими беззащитными и хрупкими.',
		},
		{
			sentence:
				'But there are none in the forest so bright as these.” They now came upon more and more of the big scarlet poppies, and fewer and fewer of the other flowers; and soon they found themselves in the midst of a great meadow of poppies.',
			translate:
				'Но в лесу нет ни одних настолько ярких, как эти». Теперь им попадалось всё больше и больше крупных алых маков и всё меньше других цветов; и вскоре они очутились среди большого макового луга.',
		},
		{
			sentence:
				'Now it is well known that when there are many of these flowers together their odor is so powerful that anyone who breathes it falls asleep, and if the sleeper is not carried away from the scent of the flowers, he sleeps on and on forever.',
			translate:
				'Известно, что когда этих цветов много вместе, их запах настолько силён, что всякий, кто его вдыхает, засыпает; и если спящего не унести подальше от аромата цветов, он будет спать и спать бесконечно.',
		},
		{
			sentence:
				'But Dorothy did not know this, nor could she get away from the bright red flowers that were everywhere about; so presently her eyes grew heavy and she felt she must sit down to rest and to sleep.',
			translate:
				'Но Дороти этого не знала и не могла уйти от ярко-красных цветов, которые были повсюду вокруг; вскоре её глаза отяжелели, и она почувствовала, что непременно должна присесть отдохнуть и поспать.',
		},
		{
			sentence: 'But the Tin Woodman would not let her do this.',
			translate: 'Но Железный Дровосек не позволил ей этого.',
		},
		{
			sentence:
				'“We must hurry and get back to the road of yellow brick before dark,” he said; and the Scarecrow agreed with him.',
			translate:
				'«Нам нужно поторопиться и вернуться на жёлтую кирпичную дорогу до темноты», — сказал он, и Пугало с ним согласилось.',
		},
		{
			sentence: 'So they kept walking until Dorothy could stand no longer.',
			translate: 'И они шли дальше, пока Дороти больше не смогла держаться на ногах.',
		},
		{
			sentence:
				'Her eyes closed in spite of herself and she forgot where she was and fell among the poppies, fast asleep.',
			translate: 'Её глаза закрылись помимо воли, она забыла, где находится, и упала среди маков, крепко заснув.',
		},
		{
			sentence: '“What shall we do?” asked the Tin Woodman.',
			translate: '«Что нам делать?» — спросил Железный Дровосек.',
		},
		{
			sentence: '“If we leave her here she will die,” said the Lion.',
			translate: '«Если мы оставим её здесь, она умрёт», — сказал Лев.',
		},
		{
			sentence: '“The smell of the flowers is killing us all.',
			translate: '«Запах цветов убивает нас всех.',
		},
		{
			sentence:
				'I myself can scarcely keep my eyes open, and the dog is asleep already.” It was true; Toto had fallen down beside his little mistress.',
			translate:
				'Я сам едва держу глаза открытыми, а собачка уже заснула». Это было правдой: Тото повалился рядом со своей маленькой хозяйкой.',
		},
		{
			sentence:
				'But the Scarecrow and the Tin Woodman, not being made of flesh, were not troubled by the scent of the flowers.',
			translate: 'Но Пугало и Железный Дровосек, поскольку они не из плоти, не страдали от запаха цветов.',
		},
		{
			sentence:
				'“Run fast,” said the Scarecrow to the Lion, “and get out of this deadly flower bed as soon as you can.',
			translate:
				'«Беги скорее, — сказал Пугало Льву, — и как можно скорее выбирайся из этого смертельного цветочного поля.',
		},
		{
			sentence:
				'We will bring the little girl with us, but if you should fall asleep you are too big to be carried.” So the Lion aroused himself and bounded forward as fast as he could go.',
			translate:
				'Мы вынесем девочку с собой, но если заснёшь ты, тебя не утащить — ты слишком велик». Тогда Лев встряхнулся и помчался вперёд так быстро, как только мог.',
		},
		{
			sentence: 'In a moment he was out of sight.',
			translate: 'Мгновение — и он уже скрылся из виду.',
		},
		{
			sentence: '“Let us make a chair with our hands and carry her,” said the Scarecrow.',
			translate: '«Давай сделаем кресло из наших рук и понесём её», — сказал Пугало.',
		},
		{
			sentence:
				'So they picked up Toto and put the dog in Dorothy’s lap, and then they made a chair with their hands for the seat and their arms for the arms and carried the sleeping girl between them through the flowers.',
			translate:
				'Они подняли Тото и посадили его на колени Дороти, затем сделали сиденье из своих ладоней, а подлокотники — из рук и понесли спящую девочку между собой через цветы.',
		},
		{
			sentence:
				'On and on they walked, and it seemed that the great carpet of deadly flowers that surrounded them would never end.',
			translate:
				'Они шли всё дальше и дальше, и казалось, этому огромному ковру смертельных цветов, окружавшему их, не будет конца.',
		},
		{
			sentence:
				'They followed the bend of the river, and at last came upon their friend the Lion, lying fast asleep among the poppies.',
			translate:
				'Они шли вдоль изгиба реки и, наконец, наткнулись на своего друга Льва, крепко спавшего среди маков.',
		},
		{
			sentence:
				'The flowers had been too strong for the huge beast and he had given up at last, and fallen only a short distance from the end of the poppy bed, where the sweet grass spread in beautiful green fields before them.',
			translate:
				'Цветы оказались слишком сильны даже для огромного зверя, и он всё же сдался и свалился всего в нескольких шагах от края макового поля, за которым простирались чудесные зелёные луга со сладкой травой.',
		},
		{
			sentence: '“We can do nothing for him,” said the Tin Woodman, sadly; “for he is much too heavy to lift.',
			translate:
				'«Мы ничего не можем для него сделать, — печально сказал Железный Дровосек, — он слишком тяжёл, чтобы поднять.',
		},
		{
			sentence:
				'We must leave him here to sleep on forever, and perhaps he will dream that he has found courage at last.” “I’m sorry,” said the Scarecrow.',
			translate:
				'Нам придётся оставить его здесь спать вечно, и, возможно, ему приснится, что он наконец обрёл храбрость». — «Мне жаль», — сказал Пугало.',
		},
		{
			sentence: '“The Lion was a very good comrade for one so cowardly.',
			translate: '«Лев был очень хорошим товарищем для столь трусливого существа.',
		},
		{
			sentence:
				'But let us go on.” They carried the sleeping girl to a pretty spot beside the river, far enough from the poppy field to prevent her breathing any more of the poison of the flowers, and here they laid her gently on the soft grass and waited for the fresh breeze to waken her.',
			translate:
				'Но пойдём дальше». Они отнесли спящую девочку к красивому месту у реки, достаточно далеко от макового поля, чтобы она больше не вдыхала ядовитый аромат цветов, осторожно уложили её на мягкую траву и стали ждать, когда свежий ветерок разбудит её.',
		},
	],
}
