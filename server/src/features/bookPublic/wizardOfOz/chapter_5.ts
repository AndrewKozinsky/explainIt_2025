export const chapter_5 = {
	name: 'Chapter V',
	header: 'The Rescue of the Tin Woodman',
	data: [
		{
			sentence:
				'When Dorothy awoke the sun was shining through the trees and Toto had long been out chasing birds around him and squirrels.',
			translate:
				'Когда Дороти проснулась, солнце светило сквозь деревья, а Тото уже давно бегал, гоняясь за птицами и белками.',
		},
		{
			sentence: 'She sat up and looked around her.',
			translate: 'Она села и огляделась вокруг.',
		},
		{
			sentence: 'There was the Scarecrow, still standing patiently in his corner, waiting for her.',
			translate: 'Пугало стояло, как и прежде, терпеливо в своём уголке, ожидая её.',
		},
		{
			sentence: '“We must go and search for water,” she said to him.',
			translate: '«Нам нужно пойти и найти воду», — сказала она ему.',
		},
		{
			sentence: '“Why do you want water?” he asked.',
			translate: '«Зачем тебе вода?» — спросил он.',
		},
		{
			sentence:
				'“To wash my face clean after the dust of the road, and to drink, so the dry bread will not stick in my throat.” “It must be inconvenient to be made of flesh,” said the Scarecrow thoughtfully, “for you must sleep, and eat and drink.',
			translate:
				'«Чтобы умыть лицо после дорожной пыли и чтобы пить, иначе сухой хлеб будет застревать у меня в горле». — «Наверное, неудобно быть сделанной из плоти, — задумчиво сказал Пугало, — ведь тебе нужно спать, есть и пить.',
		},
		{
			sentence:
				'However, you have brains, and it is worth a lot of bother to be able to think properly.” They left the cottage and walked through the trees until they found a little spring of clear water, where Dorothy drank and bathed and ate her breakfast.',
			translate:
				'Однако у тебя есть мозги, а ради умения правильно думать стоит терпеть многие неудобства». Они вышли из избушки и пошли между деревьями, пока не нашли маленький ключ с чистой водой; там Дороти напилась, умылась и позавтракала.',
		},
		{
			sentence:
				'She saw there was not much bread left in the basket, and the girl was thankful the Scarecrow did not have to eat anything, for there was scarcely enough for herself and Toto for the day.',
			translate:
				'Она заметила, что в корзинке осталось мало хлеба, и обрадовалась, что Пугалу не нужно есть, потому что еды едва хватало на неё саму и Тото на день.',
		},
		{
			sentence:
				'When she had finished her meal, and was about to go back to the road of yellow brick, she was startled to hear a deep groan near by.',
			translate:
				'Когда она закончила еду и уже собиралась вернуться на жёлтую кирпичную дорогу, её вдруг испугал глубокий стон неподалёку.',
		},
		{
			sentence: '“What was that?” she asked timidly.',
			translate: '«Что это было?» — робко спросила она.',
		},
		{
			sentence:
				'“I cannot imagine,” replied the Scarecrow; “but we can go and see.” Just then another groan reached their ears, and the sound seemed to come from behind them.',
			translate:
				'«Понятия не имею, — ответил Пугало, — но мы можем пойти и посмотреть». В тот же миг до их ушей донёсся ещё один стон, и звук, казалось, исходил у них из‑за спины.',
		},
		{
			sentence:
				'They turned and walked through the forest a few steps, when Dorothy discovered something shining in a ray of sunshine that fell between the trees.',
			translate:
				'Они повернули и прошли несколько шагов по лесу, и тут Дороти заметила что‑то блестящее в луче солнца, пробившемся между деревьями.',
		},
		{
			sentence: 'She ran to the place and then stopped short, with a little cry of surprise.',
			translate: 'Она подбежала туда и вдруг резко остановилась, тихо вскрикнув от удивления.',
		},
		{
			sentence:
				'One of the big trees had been partly chopped through, and standing beside it, with an uplifted axe in his hands, was a man made entirely of tin.',
			translate:
				'Одно из больших деревьев было наполовину срублено, а рядом, с поднятым топором в руках, стоял человек, целиком сделанный из жести.',
		},
		{
			sentence:
				'His head and arms and legs were jointed upon his body, but he stood perfectly motionless, as if he could not stir at all.',
			translate:
				'Его голова, руки и ноги были прикреплены к туловищу шарнирами, но он стоял совершенно неподвижно, словно вовсе не мог шевельнуться.',
		},
		{
			sentence:
				'Dorothy looked at him in amazement, and so did the Scarecrow, while Toto barked sharply and made a snap at the tin legs, which hurt his teeth.',
			translate:
				'Дороти изумлённо уставилась на него, и Пугало тоже; а Тото заливисто лаял и попробовал цапнуть жестяные ноги — да только о них тут же ушиб себе зубы.',
		},
		{
			sentence: '“Did you groan?” asked Dorothy.',
			translate: '«Это ты стонал?» — спросила Дороти.',
		},
		{
			sentence: '“Yes,” answered the tin man, “I did.',
			translate: '«Да, — ответил Железный Дровосек, — это я.',
		},
		{
			sentence:
				'I’ve been groaning for more than a year, and no one has ever heard me before or come to help me.” “What can I do for you?” she inquired softly, for she was moved by the sad voice in which the man spoke.',
			translate:
				'Я стону уже больше года, и до сих пор никто меня не слышал и не пришёл на помощь». — «Что я могу для тебя сделать?» — мягко спросила она, тронутая грустным голосом, которым говорил мужчина.',
		},
		{
			sentence: '“Get an oil-can and oil my joints,” he answered.',
			translate: '«Возьми маслёнку и смажь мои суставы», — ответил он.',
		},
		{
			sentence:
				'“They are rusted so badly that I cannot move them at all; if I am well oiled I shall soon be all right again.',
			translate:
				'«Они так сильно заржавели, что я совсем не могу двигаться; если меня хорошенько промаслить, я скоро снова буду в порядке.',
		},
		{
			sentence:
				'You will find an oil-can on a shelf in my cottage.” Dorothy at once ran back to the cottage and found the oil-can, and then she returned and asked anxiously, “Where are your joints?” “Oil my neck, first,” replied the Tin Woodman.',
			translate:
				'Ты найдёшь маслёнку на полке в моей избушке». Дороти тут же побежала к избушке, нашла маслёнку, вернулась и с тревогой спросила: «Где у тебя суставы?» — «Сначала смажь шею», — попросил Железный Дровосек.',
		},
		{
			sentence:
				'So she oiled it, and as it was quite badly rusted the Scarecrow took hold of the tin head and moved it gently from side to side until it worked freely, and then the man could turn it himself.',
			translate:
				'Она смазала её, и, так как шея сильно заржавела, Пугало взялся за жестяную голову и осторожно покачивал её из стороны в сторону, пока та не начала свободно вращаться; тогда Железный Дровосек смог поворачивать её сам.',
		},
		{
			sentence: '“Now oil the joints in my arms,” he said.',
			translate: '«Теперь смажь суставы на руках», — сказал он.',
		},
		{
			sentence:
				'And Dorothy oiled them and the Scarecrow bent them carefully until they were quite free from rust and as good as new.',
			translate:
				'И Дороти смазала их, а Пугало аккуратно сгибал и разгибал руки, пока ржавчина вовсе не рассыпалась и руки не стали как новые.',
		},
		{
			sentence:
				'The Tin Woodman gave a sigh of satisfaction and lowered his axe, which he leaned against the tree.',
			translate: 'Железный Дровосек вздохнул с облегчением и опустил топор, прислонив его к дереву.',
		},
		{
			sentence: '“This is a great comfort,” he said.',
			translate: '«Какое облегчение», — сказал он.',
		},
		{
			sentence:
				'“I have been holding that axe in the air ever since I rusted, and I’m glad to be able to put it down at last.',
			translate: '«Я держал этот топор на весу с тех самых пор, как заржавел, и рад наконец опустить его.',
		},
		{
			sentence:
				'Now, if you will oil the joints of my legs, I shall be all right once more.” So they oiled his legs until he could move them freely; and he thanked them again and again for his release, for he seemed a very polite creature, and very grateful.',
			translate:
				'А теперь, если смажете суставы на моих ногах, я снова буду совсем как прежде». Они смазали и ноги, пока он не смог свободно ими двигать; он снова и снова благодарил их за освобождение — он оказался очень вежливым и признательным существом.',
		},
		{
			sentence:
				'“I might have stood there always if you had not come along,” he said; “so you have certainly saved my life.',
			translate:
				'«Я мог бы простоять здесь вечно, если бы вы не пришли, — сказал он, — так что вы, конечно, спасли мне жизнь.',
		},
		{
			sentence:
				'How did you happen to be here?” “We are on our way to the Emerald City to see the Great Oz,” she answered, “and we stopped at your cottage to pass the night.” “Why do you wish to see Oz?” he asked.',
			translate:
				'Как вы оказались здесь?» — «Мы идём в Изумрудный город к Великому Озу, — ответила она, — и остановились на ночь в твоей избушке». — «Зачем вам видеть Оза?» — спросил он.',
		},
		{
			sentence:
				'“I want him to send me back to Kansas, and the Scarecrow wants him to put a few brains into his head,” she replied.',
			translate:
				'«Я хочу, чтобы он отправил меня обратно в Канзас, а Пугало хочет, чтобы он вложил ему немного мозгов в голову», — ответила она.',
		},
		{
			sentence: 'The Tin Woodman appeared to think deeply for a moment.',
			translate: 'Железный Дровосек на миг глубоко задумался.',
		},
		{
			sentence: 'Then he said: “Do you suppose Oz could give me a heart?” “Why, I guess so,” Dorothy answered.',
			translate: 'Потом сказал: «Как думаешь, сможет ли Оз дать мне сердце?» — «Думаю, да», — ответила Дороти.',
		},
		{
			sentence: '“It would be as easy as to give the Scarecrow brains.” “True,” the Tin Woodman returned.',
			translate: '«Это будет не труднее, чем дать Пугалу мозги». — «Верно», — сказал Железный Дровосек.',
		},
		{
			sentence:
				'“So, if you will allow me to join your party, I will also go to the Emerald City and ask Oz to help me.” “Come along,” said the Scarecrow heartily, and Dorothy added that she would be pleased to have his company.',
			translate:
				'«Тогда, если позволите, я присоединюсь к вам; я тоже пойду в Изумрудный город просить Оза помочь мне». — «Пойдём», — сердечно сказал Пугало, а Дороти добавила, что будет рада его обществу.',
		},
		{
			sentence:
				'So the Tin Woodman shouldered his axe and they all passed through the forest until they came to the road that was paved with yellow brick.',
			translate:
				'Железный Дровосек взвалил топор на плечо, и все вместе они пошли по лесу, пока не вышли на дорогу, вымощенную жёлтым кирпичом.',
		},
		{
			sentence: 'The Tin Woodman had asked Dorothy to put the oil-can in her basket.',
			translate: 'Железный Дровосек попросил Дороти положить маслёнку в её корзинку.',
		},
		{
			sentence:
				'“For,” he said, “if I should get caught in the rain, and rust again, I would need the oil-can badly.” It was a bit of good luck to have their new comrade join the party, for soon after they had begun their journey again they came to a place where the trees and branches grew so thick over the road that the travelers could not pass.',
			translate:
				'«Потому что, — сказал он, — если меня застанет дождь и я снова заржавею, маслёнка будет крайне нужна». Им повезло, что новый спутник присоединился к ним, потому что вскоре после возобновления пути они подошли к месту, где деревья и ветви так густо смыкались над дорогой, что путники не могли пройти.',
		},
		{
			sentence:
				'But the Tin Woodman set to work with his axe and chopped so well that soon he cleared a passage for the entire party.',
			translate:
				'Но Железный Дровосек взялся за дело своим топором и так ловко вырубил ветви, что вскоре расчистил проход для всех.',
		},
		{
			sentence:
				'Dorothy was thinking so earnestly as they walked along that she did not notice when the Scarecrow stumbled into a hole and rolled over to the side of the road.',
			translate:
				'Дороти была так погружена в мысли на ходу, что не заметила, как Пугало угодил в яму и скатился к обочине.',
		},
		{
			sentence: 'Indeed he was obliged to call to her to help him up again.',
			translate: 'Ему даже пришлось окликнуть её, чтобы она помогла подняться.',
		},
		{
			sentence: '“Why didn’t you walk around the hole?” asked the Tin Woodman.',
			translate: '«Почему ты не обошёл яму?» — спросил Железный Дровосек.',
		},
		{
			sentence: '“I don’t know enough,” replied the Scarecrow cheerfully.',
			translate: '«Я недостаточно сообразителен», — весело ответил Пугало.',
		},
		{
			sentence:
				'“My head is stuffed with straw, you know, and that is why I am going to Oz to ask him for some brains.” “Oh, I see,” said the Tin Woodman.',
			translate:
				'«Ведь у меня голова набита соломой, и потому я иду к Озу просить у него мозгов». — «Понимаю», — сказал Железный Дровосек.',
		},
		{
			sentence:
				'“But, after all, brains are not the best things in the world.” “Have you any?” inquired the Scarecrow.',
			translate:
				'«Но, в конце концов, мозги — не самая лучшая вещь на свете». — «А у тебя они есть?» — осведомился Пугало.',
		},
		{
			sentence: '“No, my head is quite empty,” answered the Woodman.',
			translate: '«Нет, моя голова совершенно пустая», — ответил Дровосек.',
		},
		{
			sentence:
				'“But once I had brains, and a heart also; so, having tried them both, I should much rather have a heart.” “And why is that?” asked the Scarecrow.',
			translate:
				'«Зато раньше у меня были и мозги, и сердце; и, попробовав и то и другое, я бы гораздо охотнее выбрал сердце». — «Почему?» — спросил Пугало.',
		},
		{
			sentence:
				'“I will tell you my story, and then you will know.” So, while they were walking through the forest, the Tin Woodman told the following story: “I was born the son of a woodman who chopped down trees in the forest and sold the wood for a living.',
			translate:
				'«Я расскажу тебе свою историю, и ты поймёшь». И пока они шли по лесу, Железный Дровосек рассказал следующее: «Я родился сыном дровосека, который валил в лесу деревья и продавал древесину.',
		},
		{
			sentence:
				'When I grew up, I too became a wood-chopper, and after my father died I took care of my old mother as long as she lived.',
			translate:
				'Когда я вырос, я тоже стал дровосеком, а после смерти отца заботился о своей старой матери, пока она жила.',
		},
		{
			sentence:
				'Then I made up my mind that instead of living alone I would marry, so that I might not become lonely.',
			translate: 'Потом я решил, что, вместо того чтобы жить одному, женюсь, чтобы мне не было одиноко.',
		},
		{
			sentence:
				'“There was one of the Munchkin girls who was so beautiful that I soon grew to love her with all my heart.',
			translate: '«Одна девушка из мункинов была столь прекрасна, что я вскоре полюбил её всем сердцем.',
		},
		{
			sentence:
				'She, on her part, promised to marry me as soon as I could earn enough money to build a better house for her; so I set to work harder than ever.',
			translate:
				'Она, со своей стороны, обещала выйти за меня, как только я заработаю достаточно, чтобы построить для неё лучший дом; и я принялся работать усерднее прежнего.',
		},
		{
			sentence:
				'But the girl lived with an old woman who did not want her to marry anyone, for she was so lazy she wished the girl to remain with her and do the cooking and the housework.',
			translate:
				'Но девушка жила со старой женщиной, которая не хотела, чтобы она за кого‑нибудь выходила: она была столь ленива, что хотела, чтобы девушка оставалась с ней, готовила и вела хозяйство.',
		},
		{
			sentence:
				'So the old woman went to the Wicked Witch of the East, and promised her two sheep and a cow if she would prevent the marriage.',
			translate:
				'Тогда старуха пошла к Злой Ведьме Востока и пообещала ей двух овец и корову, если та помешает свадьбе.',
		},
		{
			sentence:
				'Thereupon the Wicked Witch enchanted my axe, and when I was chopping away at my best one day, for I was anxious to get the new house and my wife as soon as possible, the axe slipped all at once and cut off my left leg.',
			translate:
				'Тогда Злая Ведьма заколдовала мой топор, и однажды, когда я работал изо всех сил — ведь мне хотелось поскорее получить новый дом и жену, — топор вдруг соскользнул и отрубил мне левую ногу.',
		},
		{
			sentence:
				'“This at first seemed a great misfortune, for I knew a one-legged man could not do very well as a wood-chopper.',
			translate:
				'«Поначалу это показалось страшным несчастьем: я знал, что одноногому дровосеку придётся нелегко.',
		},
		{
			sentence: 'So I went to a tinsmith and had him make me a new leg out of tin.',
			translate: 'Тогда я отправился к жестянщику, и он сделал мне новую ногу из жести.',
		},
		{
			sentence: 'The leg worked very well, once I was used to it.',
			translate: 'Когда я к ней привык, она служила очень хорошо.',
		},
		{
			sentence:
				'But my action angered the Wicked Witch of the East, for she had promised the old woman I should not marry the pretty Munchkin girl.',
			translate:
				'Но мой поступок разгневал Злую Ведьму Востока, ведь она обещала старухе, что я не женюсь на милой девушке‑мункине.',
		},
		{
			sentence: 'When I began chopping again, my axe slipped and cut off my right leg.',
			translate: 'Когда я снова принялся рубить, топор соскользнул и отрубил мне правую ногу.',
		},
		{
			sentence: 'Again I went to the tinsmith, and again he made me a leg out of tin.',
			translate: 'Я опять пошёл к жестянщику, и он снова сделал мне ногу из жести.',
		},
		{
			sentence:
				'After this the enchanted axe cut off my arms, one after the other; but, nothing daunted, I had them replaced with tin ones.',
			translate:
				'После этого зачарованный топор отрубил мне руки — одну за другой; но я нисколько не пал духом и заменил их жестяными.',
		},
		{
			sentence:
				'The Wicked Witch then made the axe slip and cut off my head, and at first I thought that was the end of me.',
			translate:
				'Потом Ведьма заставила топор соскользнуть и отрубить мне голову, и сперва я подумал, что это конец.',
		},
		{
			sentence: 'But the tinsmith happened to come along, and he made me a new head out of tin.',
			translate: 'Но как раз мимо проходил жестянщик и сделал мне новую голову из жести.',
		},
		{
			sentence:
				'“I thought I had beaten the Wicked Witch then, and I worked harder than ever; but I little knew how cruel my enemy could be.',
			translate:
				'«Мне казалось, что я победил Злую Ведьму и работал ещё усерднее; но я и не предполагал, насколько жестокой может быть моя противница.',
		},
		{
			sentence:
				'She thought of a new way to kill my love for the beautiful Munchkin maiden, and made my axe slip again, so that it cut right through my body, splitting me into two halves.',
			translate:
				'Она придумала новый способ убить мою любовь к прекрасной девушке‑мункине: заставила топор снова соскользнуть так, что он рассёк моё тело надвое.',
		},
		{
			sentence:
				'Once more the tinsmith came to my help and made me a body of tin, fastening my tin arms and legs and head to it, by means of joints, so that I could move around as well as ever.',
			translate:
				'Жестянщик снова пришёл мне на помощь и сделал мне жестяное тело, прикрепив к нему мои жестяные руки, ноги и голову на шарнирах, так что я мог двигаться как прежде.',
		},
		{
			sentence: 'But, alas!',
			translate: 'Но, увы!',
		},
		{
			sentence:
				'I had now no heart, so that I lost all my love for the Munchkin girl, and did not care whether I married her or not.',
			translate:
				'У меня больше не было сердца, и потому я потерял всю любовь к девушке‑мункине и уже не заботился, женюсь ли на ней или нет.',
		},
		{
			sentence: 'I suppose she is still living with the old woman, waiting for me to come after her.',
			translate: 'Полагаю, она и сейчас живёт со старухой, ожидая, что я приду за ней.',
		},
		{
			sentence:
				'“My body shone so brightly in the sun that I felt very proud of it and it did not matter now if my axe slipped, for it could not cut me.',
			translate:
				'«Моё тело так ярко блестело на солнце, что я гордился им, и теперь было всё равно, если топор соскользнёт — он уже не мог меня порезать.',
		},
		{
			sentence:
				'There was only one danger—that my joints would rust; but I kept an oil-can in my cottage and took care to oil myself whenever I needed it.',
			translate:
				'Опасность была только одна — что мои суставы заржавеют; но я держал в избушке маслёнку и смазывал себя, когда это было нужно.',
		},
		{
			sentence:
				'However, there came a day when I forgot to do this, and, being caught in a rainstorm, before I thought of the danger my joints had rusted, and I was left to stand in the woods until you came to help me.',
			translate:
				'Однако случился день, когда я про это забыл, и, попав под дождь, ещё прежде, чем я вспомнил об опасности, мои суставы заржавели, и я остался стоять в лесу, пока вы не пришли мне на помощь.',
		},
		{
			sentence:
				'It was a terrible thing to undergo, but during the year I stood there I had time to think that the greatest loss I had known was the loss of my heart.',
			translate:
				'Это было ужасное испытание, но за тот год, что я стоял, я успел подумать о том, что самая большая моя утрата — это утрата сердца.',
		},
		{
			sentence:
				'While I was in love I was the happiest man on earth; but no one can love who has not a heart, and so I am resolved to ask Oz to give me one.',
			translate:
				'Пока я был влюблён, я был самым счастливым человеком на земле; но никто не может любить, не имея сердца, и потому я твёрдо решил просить Оза дать мне его.',
		},
		{
			sentence:
				'If he does, I will go back to the Munchkin maiden and marry her.” Both Dorothy and the Scarecrow had been greatly interested in the story of the Tin Woodman, and now they knew why he was so anxious to get a new heart.',
			translate:
				'Если даст — я вернусь к девушке‑мункине и женюсь на ней». И Дороти с Пугалом, выслушав рассказ Железного Дровосека, поняли, почему он так стремится получить новое сердце.',
		},
		{
			sentence:
				'“All the same,” said the Scarecrow, “I shall ask for brains instead of a heart; for a fool would not know what to do with a heart if he had one.” “I shall take the heart,” returned the Tin Woodman; “for brains do not make one happy, and happiness is the best thing in the world.” Dorothy did not say anything, for she was puzzled to know which of her two friends was right, and she decided if she could only get back to Kansas and Aunt Em, it did not matter so much whether the Woodman had no brains and the Scarecrow no heart, or each got what he wanted.',
			translate:
				'«А я всё равно попрошу мозги, а не сердце, — сказал Пугало, — ведь дурак не знал бы, что делать с сердцем, даже если бы оно у него было». — «А я выберу сердце, — возразил Железный Дровосек, — потому что мозги не делают человека счастливым, а счастье — лучшее в мире». Дороти ничего не сказала: ей было трудно решить, кто из друзей прав; она подумала, что если только удастся вернуться в Канзас к тёте Эм, то уже не так важно, останется ли Дровосек без мозгов, а Пугало — без сердца, или же оба получат, чего хотят.',
		},
		{
			sentence:
				'What worried her most was that the bread was nearly gone, and another meal for herself and Toto would empty the basket.',
			translate:
				'Больше всего её тревожило, что хлеб почти кончился, и ещё один приём пищи для неё и Тото опустошит корзинку.',
		},
		{
			sentence:
				'To be sure, neither the Woodman nor the Scarecrow ever ate anything, but she was not made of tin nor straw, and could not live unless she was fed.',
			translate:
				'Правда, ни Дровосек, ни Пугало ничего не ели, но она была не из жести и не из соломы и не могла жить без еды.',
		},
	],
}
