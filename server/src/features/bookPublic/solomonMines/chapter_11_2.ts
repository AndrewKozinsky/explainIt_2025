export const chapter_11_2 = {
	name: 'Chapter XI',
	header: 'We give a sign (2)',
	data: [
		{
			sentence:
				'The idea was a splendid one; indeed, the only weak spot about it was a fear lest Good’s almanack might be incorrect.',
			translate:
				'Мысль была превосходная; по сути, единственной её слабостью был страх, что альманах Гуда может оказаться неверным.',
		},
		{
			sentence:
				'If we made a false prophecy on such a subject, our prestige would be gone for ever, and so would Ignosi’s chance of the throne of the Kukuanas.',
			translate:
				'Если бы мы дали ложное пророчество в таком деле, наш престиж был бы утрачен навсегда — и вместе с ним шансы Игноси на трон Кукуанов.',
		},
		{
			sentence:
				'“Suppose that the almanack is wrong,” suggested Sir Henry to Good, who was busily employed in working out something on a blank page of the book.',
			translate:
				'«Предположим, альманах ошибается», — предложил сэр Генри Гуду, который усердно что‑то вычислял на чистой странице книги.',
		},
		{
			sentence: '“I see no reason to suppose anything of the sort,” was his answer.',
			translate: '«Не вижу причин предполагать подобное», — последовал ответ.',
		},
		{
			sentence:
				'“Eclipses always come up to time; at least that is my experience of them, and it especially states that this one will be visible in South Africa.',
			translate:
				'«Затмения всегда наступают вовремя; по крайней мере, мой опыт таков, и здесь особо сказано, что это будет видно в Южной Африке.',
		},
		{
			sentence:
				'I have worked out the reckonings as well as I can, without knowing our exact position; and I make out that the eclipse should begin here about ten o’clock tomorrow night, and last till half-past twelve.',
			translate:
				'Я произвёл вычисления насколько мог, не зная нашей точной позиции; и у меня выходит, что здесь затмение начнётся завтра около десяти вечера и продлится до половины первого.',
		},
		{
			sentence: 'For an hour and a half or so there should be almost total darkness.”',
			translate: '«Около часа с половиной будет почти полная тьма».',
		},
		{
			sentence: '“Well,” said Sir Henry, “I suppose we had better risk it.”',
			translate: '«Что ж, — сказал сэр Генри, — полагаю, нам лучше рискнуть».',
		},
		{
			sentence:
				'I acquiesced, though doubtfully, for eclipses are queer cattle to deal with—it might be a cloudy night, for instance, or our dates might be wrong—and sent Umbopa to summon the chiefs back.',
			translate:
				'Я согласился, хоть и с сомнением, ибо с затмениями дело тонкое — ночь может оказаться пасмурной, к примеру, или наши даты могут быть неверны, — и послал Умбопу позвать вождей обратно.',
		},
		{
			sentence: 'Presently they came, and I addressed them thus—',
			translate: 'Вскоре они пришли, и я обратился к ним так —',
		},
		{
			sentence: '“Great men of the Kukuanas, and thou, Infadoos, listen.',
			translate: '«Великие мужи Кукуанов, и ты, Инфадоос, слушайте.',
		},
		{
			sentence:
				'We love not to show our powers, for to do so is to interfere with the course of nature, and to plunge the world into fear and confusion.',
			translate:
				'Мы не любим являть свои силы, ибо этим вмешиваемся в ход природы и ввергаем мир в страх и смятение.',
		},
		{
			sentence:
				'But since this matter is a great one, and as we are angered against the king because of the slaughter we have seen, and because of the act of the Isanusi Gagool, who would have put our friend Ignosi to death, we have determined to break a rule, and to give such a sign as all men may see.',
			translate:
				'Но раз дело велико, и мы разгневаны на царя из‑за резни, что мы видели, и из‑за поступка Исануси Гагаулы, которая хотела предать смерти нашего друга Игноси, — мы решили нарушить правило и дать такой знак, который все смогут увидеть.',
		},
		{
			sentence: 'Come hither”; and I led them to the door of the hut and pointed to the red ball of the moon.',
			translate: '«Подойдите сюда», — и я подвёл их к двери хижины и указал на красный шар луны.',
		},
		{
			sentence: '“What see ye there?”',
			translate: '«Что вы там видите?»',
		},
		{
			sentence: '“We see the sinking moon,” answered the spokesman of the party.',
			translate: '«Мы видим заходящую луну», — ответил представитель группы.',
		},
		{
			sentence: '“It is so.',
			translate: '«Так и есть.',
		},
		{
			sentence:
				'Now tell me, can any mortal man put out that moon before her hour of setting, and bring the curtain of black night down upon the land?”',
			translate:
				'А теперь скажите: может ли какой смертный потушить ту луну раньше часа её захода и опустить на землю завесу чёрной ночи?»',
		},
		{
			sentence: 'The chief laughed a little at the question.',
			translate: 'Вождь слегка усмехнулся вопросу.',
		},
		{
			sentence: '“No, my lord, that no man can do.',
			translate: '«Нет, мой владыка, этого никто не может сделать.',
		},
		{
			sentence: 'The moon is stronger than man who looks on her, nor can she vary in her courses.”',
			translate: 'Луна сильнее человека, что взирает на неё, и не может отклониться от своего пути».',
		},
		{
			sentence: '“Ye say so.',
			translate: '«Так вы говорите.',
		},
		{
			sentence:
				'Yet I tell you that to-morrow night, about two hours before midnight, we will cause the moon to be eaten up for a space of an hour and half an hour.',
			translate:
				'Однако я говорю вам, что завтра ночью, за два часа до полуночи, мы заставим луну быть «съеденной» на срок в час с половиной.',
		},
		{
			sentence:
				'Yes, deep darkness shall cover the earth, and it shall be for a sign that Ignosi is indeed king of the Kukuanas.',
			translate: 'Да, глубокая тьма покроет землю, и это будет знаком, что Игноси воистину король Кукуанов.',
		},
		{
			sentence: 'If we do this thing, will ye be satisfied?”',
			translate: 'Если мы сотворим это, будете ли вы удовлетворены?»',
		},
		{
			sentence:
				'“Yea, my lords,” answered the old chief with a smile, which was reflected on the faces of his companions; “if ye do this thing, we will be satisfied indeed.”',
			translate:
				'«Да, мои владыки, — ответил старый вождь с улыбкой, отразившейся на лицах его спутников; — если вы сделаете это, мы и впрямь будем удовлетворены».',
		},
		{
			sentence:
				'“It shall be done; we three, Incubu, Bougwan, and Macumazahn, have said it, and it shall be done.',
			translate: '«Так и будет: мы трое, Инкубу, Бугван и Макумазан, сказали это — и так и будет.',
		},
		{
			sentence: 'Dost thou hear, Infadoos?”',
			translate: 'Слышишь ли, Инфадоос?»',
		},
		{
			sentence:
				'“I hear, my lord, but it is a wonderful thing that ye promise, to put out the moon, the mother of the world, when she is at her full.”',
			translate:
				'«Слышу, мой владыка, но дивное же это дело вы обещаете — погасить луну, мать мира, когда она в полноте».',
		},
		{
			sentence: '“Yet shall we do it, Infadoos.”',
			translate: '«И всё же мы сделаем это, Инфадоос».',
		},
		{
			sentence: '“It is well, my lords.',
			translate: '«Это хорошо, мои владыки.',
		},
		{
			sentence:
				'To-day, two hours after sunset, Twala will send for my lords to witness the girls dance, and one hour after the dance begins the girl whom Twala thinks the fairest shall be killed by Scragga, the king’s son, as a sacrifice to the Silent Ones, who sit and keep watch by the mountains yonder,” and he pointed towards the three strange-looking peaks where Solomon’s road was supposed to end.',
			translate:
				'Сегодня, через два часа после захода солнца, Твала пошлёт за моими владыками, чтобы они увидели пляску девушек, и через час после начала пляски девушку, которую Твала сочтёт прекраснейшей, Скрагга, сын царя, убьёт в жертву Безмолвным, что сидят и стерегут у вон тех гор», — и он указал на три вершины странного вида, где, как полагали, заканчивалась дорога Соломона.',
		},
		{
			sentence:
				'“Then let my lords darken the moon, and save the maiden’s life, and the people will believe indeed.”',
			translate: '«Тогда пусть мои владыки затемнят луну и спасут жизнь девицы — и народ действительно поверит».',
		},
		{
			sentence: '“Ay,” said the old chief, still smiling a little, “the people will believe indeed.”',
			translate: '«Ай, — сказал старый вождь, всё ещё слегка улыбаясь, — народ и впрямь поверит».',
		},
		{
			sentence:
				'“Two miles from Loo,” went on Infadoos, “there is a hill curved like a new moon, a stronghold, where my regiment, and three other regiments which these chiefs command, are stationed.',
			translate:
				'«В двух милях от Лу, — продолжил Инфадоос, — есть холм, изогнутый, как молодой месяц, укрепление, где стоит мой полк и ещё три полка, которыми командуют эти вожди.',
		},
		{
			sentence: 'This morning we will make a plan whereby two or three other regiments may be moved there also.',
			translate: 'Этим утром мы составим план, по которому туда можно будет перебросить ещё два‑три полка.',
		},
		{
			sentence:
				'Then, if in truth my lords can darken the moon, in the darkness I will take my lords by the hand and lead them out of Loo to this place, where they shall be safe, and thence we can make war upon Twala the king.”',
			translate:
				'Тогда, если воистину мои владыки смогут затемнить луну, в темноте я возьму моих владык за руки и выведу их из Лу к этому месту, где они будут в безопасности, и оттуда мы сможем начать войну против царя Твалы».',
		},
		{
			sentence: '“It is good,” said I.',
			translate: '«Это хорошо», — сказал я.',
		},
		{
			sentence: '“Now leave us to sleep awhile and to make ready our magic.”',
			translate: '«А теперь оставьте нас немного поспать и готовить наше колдовство».',
		},
		{
			sentence: 'Infadoos rose, and, having saluted us, departed with the chiefs.',
			translate: 'Инфадоос поднялся и, отдав нам честь, удалился вместе с вождями.',
		},
	],
}
