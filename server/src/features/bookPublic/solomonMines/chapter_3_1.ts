export const chapter_3_1 = {
	name: 'Chapter III',
	header: 'Umbora enters our service',
	data: [
		{
			sentence:
				'It takes from four to five days, according to the speed of the vessel and the state of the weather, to run up from the Cape to Durban.',
			translate:
				'Переход от Мыса до Дурбана занимает от четырёх до пяти дней — в зависимости от скорости судна и погоды.',
		},
		{
			sentence:
				'Sometimes, if the landing is bad at East London, where they have not yet made that wonderful harbour they talk so much of, and sink such a mint of money in, a ship is delayed for twenty-four hours before the cargo boats can get out to take off the goods.',
			translate:
				'Иногда, если в Ист‑Лондоне плохая высадка — там ещё не устроили ту чудесную гавань, о которой так много говорят и в которую вбухивают целое состояние, — судно задерживается на сутки, прежде чем грузовые лодки смогут выйти и забрать товары.',
		},
		{
			sentence:
				'But on this occasion we had not to wait at all, for there were no breakers on the Bar to speak of, and the tugs came out at once with the long strings of ugly flat-bottomed boats behind them, into which the packages were bundled with a crash.',
			translate:
				'Но на этот раз нам вовсе не пришлось ждать: на Баре почти не было прибойной волны, и буксиры тотчас вышли, таща за собой длинные гирлянды неуклюжих плоскодонок, в которые тюки швыряли с грохотом.',
		},
		{
			sentence:
				'It did not matter what they might be, over they went slap-bang; whether they contained china or woollen goods they met with the same treatment.',
			translate:
				'Неважно, что там было, — всё летело туда шлёп‑бах; фарфор ли внутри, шерстяной ли товар — обращались с ним одинаково.',
		},
		{
			sentence:
				'I saw one case holding four dozen of champagne smashed all to bits, and there was the champagne fizzing and boiling about in the bottom of the dirty cargo boat.',
			translate:
				'Я видел, как ящик с четырьмя дюжинами шампанского разнесло вдребезги, и вот уже шампанское шипело и кипело по дну грязной грузовой лодки.',
		},
		{
			sentence:
				'It was a wicked waste, and evidently so the Kafirs in the boat thought, for they found a couple of unbroken bottles, and knocking off the necks drank the contents.',
			translate:
				'Греховная, прямо скажем, растрата — и, по‑видимому, так же подумали кафры в лодке: они нашли пару целых бутылок, сбили горлышки и выпили содержимое.',
		},
		{
			sentence:
				'But they had not allowed for the expansion caused by the fizz in the wine, and, feeling themselves swelling, rolled about in the bottom of the boat, calling out that the good liquor was “tagati”—that is, bewitched.',
			translate:
				'Но они не учли распирания от газа в вине и, почувствовав, как их раздувает, катались по дну лодки, выкрикивая, что добрый напиток — «tagati», то есть околдованный.',
		},
		{
			sentence:
				'I spoke to them from the vessel, and told them it was the white man’s strongest medicine, and that they were as good as dead men.',
			translate:
				'Я крикнул им с судна и сказал, что это сильнейшее лекарство белых и что они уже считай покойники.',
		},
		{
			sentence:
				'Those Kafirs went to the shore in a very great fright, and I do not think that they will touch champagne again.',
			translate:
				'Те кафры высадились на берег в смертельном испуге, и не думаю, что они ещё хоть раз притронутся к шампанскому.',
		},
		{
			sentence:
				'Well, all the time that we were steaming up to Natal I was thinking over Sir Henry Curtis’s offer.',
			translate: 'Всю дорогу, пока мы шли паром к Наталю, я обдумывал предложение сэра Генри Кёртиса.',
		},
		{
			sentence:
				'We did not speak any more on the subject for a day or two, though I told them many hunting yarns, all true ones.',
			translate:
				'Мы день‑другой больше об этом не говорили, хотя я рассказал им немало охотничьих баек — все до одной правдивые.',
		},
		{
			sentence:
				'There is no need to tell lies about hunting, for so many curious things happen within the knowledge of a man whose business it is to hunt; but this is by the way.',
			translate:
				'Нет нужды врать об охоте, потому что с человеком, чьё дело — охотиться, случается столько диковинного; впрочем, это к слову.',
		},
		{
			sentence:
				'At last, one beautiful evening in January, which is our hottest month, we steamed past the coast of Natal, expecting to make Durban Point by sunset.',
			translate:
				'Наконец, в один прекрасный январский вечер — у нас это самый жаркий месяц — мы шли вдоль побережья Наталя, рассчитывая к закату выйти к Дурбан‑Пойнт.',
		},
		{
			sentence:
				'It is a lovely coast all along from East London, with its red sandhills and wide sweeps of vivid green, dotted here and there with Kafir kraals, and bordered by a ribbon of white surf, which spouts up in pillars of foam where it hits the rocks.',
			translate:
				'Это чудесное побережье тянется всё от Ист‑Лондона: красные песчаные дюны и широкие полосы яркой зелени, тут и там усыпанные кафрскими краалями, а по краю — лента белого прибоя, взлетающего столбами пены там, где он бьётся о скалы.',
		},
		{
			sentence: 'But just before you come to Durban there is a peculiar richness about the landscape.',
			translate: 'Но незадолго до Дурбана в пейзаже появляется особая насыщенность и изобилие.',
		},
		{
			sentence:
				'There are the sheer kloofs cut in the hills by the rushing rains of centuries, down which the rivers sparkle; there is the deepest green of the bush, growing as God planted it, and the other greens of the mealie gardens and the sugar patches, while now and again a white house, smiling out at the placid sea, puts a finish and gives an air of homeliness to the scene.',
			translate:
				'Тут — отвесные клуфы, прорезанные в холмах стремительными дождями веков, по которым искрятся речки; здесь — густейшая зелень буша, растущего так, как посадил Бог, и другие оттенки зелени кукурузных огородов и сахарных плантаций; а время от времени белый дом, улыбаясь спокойному морю, завершает картину и придаёт ей ощущение домашности.',
		},
		{
			sentence:
				'For to my mind, however beautiful a view may be, it requires the presence of man to make it complete, but perhaps that is because I have lived so much in the wilderness, and therefore know the value of civilisation, though to be sure it drives away the game.',
			translate:
				'Ибо, на мой взгляд, каким бы прекрасным ни был вид, для полноты ему нужно присутствие человека; быть может, это потому, что я столь много прожил в пустынных местах и потому знаю цену цивилизации — хотя, конечно, она отгоняет дичь.',
		},
		{
			sentence:
				'The Garden of Eden, no doubt, looked fair before man was, but I always think that it must have been fairer when Eve adorned it.',
			translate:
				'Сад Эдема, без сомнения, был прекрасен и до появления человека, но я всегда думаю, что стал он ещё прекраснее, когда его украсила Ева.',
		},
		{
			sentence:
				'To return, we had miscalculated a little, and the sun was well down before we dropped anchor off the Point, and heard the gun which told the good folks of Durban that the English Mail was in.',
			translate:
				'Возвращаясь к делу: мы немного просчитались, и солнце уже совсем село, когда мы встали на якорь у Пойнта и услышали пушечный выстрел, оповестивший добрых жителей Дурбана, что прибыла английская почта.',
		},
		{
			sentence:
				'It was too late to think of getting over the Bar that night, so we went comfortably to dinner, after seeing the Mails carried off in the life-boat.',
			translate:
				'Было слишком поздно, чтобы в ту ночь и думать о проходе через Бар, так что мы спокойно отправились ужинать, проводив почту, уносимую на спасательной шлюпке.',
		},
		{
			sentence:
				'When we came up again the moon was out, and shining so brightly over sea and shore that she almost paled the quick, large flashes from the lighthouse.',
			translate:
				'Когда мы снова поднялись на палубу, луна уже взошла и сияла над морем и берегом так ярко, что почти бледнила быстрые, широкие вспышки маяка.',
		},
		{
			sentence:
				'From the shore floated sweet spicy odours that always remind me of hymns and missionaries, and in the windows of the houses on the Berea sparkled a hundred lights.',
			translate:
				'С берега тянулись сладкие пряные запахи, которые всегда напоминают мне гимны и миссионеров, а в окнах домов на Берее мерцали сотни огней.',
		},
		{
			sentence:
				'From a large brig lying near also came the music of the sailors as they worked at getting the anchor up in order to be ready for the wind.',
			translate:
				'С большого брига, стоявшего рядом, доносилась и матросская песня — они выбирали якорь, чтобы быть готовы к ветру.',
		},
		{
			sentence:
				'Altogether it was a perfect night, such a night as you sometimes get in Southern Africa, and it threw a garment of peace over everybody as the moon threw a garment of silver over everything.',
			translate:
				'В целом это была совершенная ночь — такая, какие порой выпадают в Южной Африке; она набросила на всех покров мира, как луна набросила на всё серебряный покров.',
		},
		{
			sentence:
				'Even the great bulldog, belonging to a sporting passenger, seemed to yield to its gentle influences, and forgetting his yearning to come to close quarters with the baboon in a cage on the foc’sle, snored happily at the door of the cabin, dreaming no doubt that he had finished him, and happy in his dream.',
			translate:
				'Даже огромный бульдог, принадлежавший одному спортивному пассажиру, как будто поддался её мягкому воздействию и, забыв своё жгучее желание сойтись вплотную с павианом в клетке на баке, счастливо похрапывал у двери каюты, во сне, без сомнения, прикончив противника и блаженствуя в своём сне.',
		},
	],
}
