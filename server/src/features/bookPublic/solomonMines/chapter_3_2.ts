export const chapter_3_2 = {
	name: 'Chapter III',
	header: 'Umbora enters our service (2)',
	data: [
		{
			sentence:
				'We three—that is, Sir Henry Curtis, Captain Good, and myself—went and sat by the wheel, and were quiet for a while.',
			translate:
				'Мы втроём — то есть сэр Генри Кёртис, капитан Гуд и я — пошли и сели у штурвала и некоторое время молчали.',
		},
		{
			sentence: '“Well, Mr. Quatermain,” said Sir Henry presently, “have you been thinking about my proposals?”',
			translate: '«Ну что, мистер Квотермейн, — сказал вскоре сэр Генри, — вы размышляли о моих предложениях?»',
		},
		{
			sentence: '“Ay,” echoed Captain Good, “what do you think of them, Mr. Quatermain?”',
			translate: '«Да, — подхватил капитан Гуд, — что вы о них думаете, мистер Квотермейн?»',
		},
		{
			sentence:
				'“I hope that you are going to give us the pleasure of your company so far as Solomon’s Mines, or wherever the gentleman you knew as Neville may have got to.”',
			translate:
				'«Надеюсь, вы доставите нам удовольствие составить компанию до самых Копей Соломона — или куда бы ни добрался тот джентльмен, которого вы знали под именем Невилла».',
		},
		{
			sentence: 'I rose and knocked out my pipe before I answered.',
			translate: 'Я поднялся и выбил трубку, прежде чем ответить.',
		},
		{
			sentence: 'I had not made up my mind, and wanted an additional moment to decide.',
			translate: 'Я ещё не принял решения и хотел выиграть дополнительный миг.',
		},
		{
			sentence:
				'Before the burning tobacco had fallen into the sea I had decided; just that little extra second did the trick.',
			translate:
				'Прежде чем горящий табак упал в море, я уже решил; именно эта крохотная лишняя секунда и сделала своё дело.',
		},
		{
			sentence: 'It is often the way when you have been bothering a long time over a thing.',
			translate: 'Так часто и бывает, когда долго мучаешься над чем‑то.',
		},
		{
			sentence:
				'“Yes, gentlemen,” I said, sitting down again, “I will go, and by your leave I will tell you why, and on what conditions.',
			translate:
				'«Да, господа, — сказал я, снова усаживаясь, — я поеду и, с вашего позволения, скажу вам почему и на каких условиях».',
		},
		{
			sentence: 'First for the terms which I ask.',
			translate: 'Прежде всего — об условиях, которые я требую».',
		},
		{
			sentence:
				'“1. You are to pay all expenses, and any ivory or other valuables we may get is to be divided between Captain Good and myself.”',
			translate:
				'«1. Все расходы берёте на себя, а любую слоновую кость или иные ценности, что мы добудем, делим между капитаном Гудом и мной».',
		},
		{
			sentence:
				'“2. That you give me £500 for my services on the trip before we start, I undertaking to serve you faithfully till you choose to abandon the enterprise, or till we succeed, or disaster overtakes us.”',
			translate:
				'«2. До начала пути вы выдаёте мне 500 фунтов за мои услуги; я обязуюсь верно служить вам, пока вы не решите отказаться от предприятия, или пока мы не добьёмся успеха, или пока нас не постигнет беда».',
		},
		{
			sentence:
				'“3. That before we trek you execute a deed agreeing, in the event of my death or disablement, to pay my boy Harry, who is studying medicine over there in London, at Guy’s Hospital, a sum of £200 a year for five years, by which time he ought to be able to earn a living for himself if he is worth his salt.”',
			translate:
				'«3. Прежде чем мы отправимся в путь, вы оформляете акт, по которому в случае моей смерти или увечья выплачиваете моему мальчику Гарри, который учится медицине там, в Лондоне, в госпитале Гая, по 200 фунтов в год в течение пяти лет, к тому времени он должен суметь зарабатывать себе на жизнь, если он чего‑нибудь стоит».',
		},
		{
			sentence: 'That is all, I think, and I daresay you will say quite enough too.”',
			translate: '«Вот и всё, думаю; и, полагаю, вы скажете — более чем достаточно».',
		},
		{
			sentence: '“No,” answered Sir Henry, “I accept them gladly.',
			translate: '«Нет, — ответил сэр Генри, — я охотно принимаю их.',
		},
		{
			sentence:
				'I am bent upon this project, and would pay more than that for your help, considering the peculiar and exclusive knowledge which you possess.”',
			translate:
				'Я решительно настроен на это предприятие и заплатил бы и больше за вашу помощь, учитывая особые и исключительные знания, которыми вы обладаете».',
		},
		{
			sentence: '“Pity I did not ask it, then, but I won’t go back on my word.',
			translate: '«Жаль, что я этого не попросил, но от своих слов не отступлю.',
		},
		{
			sentence: 'And now that I have got my terms I will tell you my reasons for making up my mind to go.',
			translate: 'А теперь, когда я получил свои условия, скажу вам, почему решил идти.',
		},
		{
			sentence:
				'First of all, gentlemen, I have been observing you both for the last few days, and if you will not think me impertinent I may say that I like you, and believe that we shall come up well to the yoke together.',
			translate:
				'Прежде всего, господа, я присматривался к вам обоим последние несколько дней и, если не сочтёте меня дерзким, скажу, что вы мне нравитесь, и я верю, что мы потянем в одной упряжке.',
		},
		{
			sentence: 'That is something, let me tell you, when one has a long journey like this before one.',
			translate: 'А это, скажу вам, немало значит, когда впереди такой долгий путь.',
		},
		{
			sentence:
				'“And now as to the journey itself, I tell you flatly, Sir Henry and Captain Good, that I do not think it probable we can come out of it alive, that is, if we attempt to cross the Suliman Mountains.',
			translate:
				'«А теперь о самом путешествии: скажу прямо, сэр Генри и капитан Гуд, я не считаю вероятным, что мы выберемся из него живыми — если попытаемся пересечь горы Сулиман.',
		},
		{
			sentence: 'What was the fate of the old Dom da Silvestra three hundred years ago?',
			translate: 'Какова была участь старого дона да Сильвестры триста лет назад?',
		},
		{
			sentence: 'What was the fate of his descendant twenty years ago?',
			translate: 'Какова была участь его потомка двадцать лет назад?',
		},
		{
			sentence: 'What has been your brother’s fate?',
			translate: 'Какова была участь вашего брата?',
		},
		{
			sentence: 'I tell you frankly, gentlemen, that as their fates were so I believe ours will be.”',
			translate: 'Откровенно скажу, господа: какова была их участь, такою, по моему убеждению, будет и наша».',
		},
		{
			sentence: 'I paused to watch the effect of my words.',
			translate: 'Я сделал паузу, чтобы посмотреть, какое впечатление произвели мои слова.',
		},
		{
			sentence: 'Captain Good looked a little uncomfortable, but Sir Henry’s face did not change.',
			translate: 'Капитан Гуд выглядел слегка смущённым, но лицо сэра Генри не изменилось.',
		},
		{
			sentence: '“We must take our chance,” he said.',
			translate: '«Надо положиться на судьбу», — сказал он.',
		},
		{
			sentence:
				'“You may perhaps wonder,” I went on, “why, if I think this, I, who am, as I told you, a timid man, should undertake such a journey.',
			translate:
				'«Вы, возможно, удивляетесь, — продолжал я, — почему, если я так думаю, я, человек осторожный, как уже говорил, решаюсь на подобное путешествие.',
		},
		{
			sentence: 'It is for two reasons.',
			translate: 'Причины две.',
		},
		{
			sentence:
				'First I am a fatalist, and believe that my time is appointed to come quite without reference to my own movements and will, and that if I am to go to Suliman’s Mountains to be killed, I shall go there and shall be killed.',
			translate:
				'Во‑первых, я фаталист и верю, что мой срок назначен независимо от моих действий и воли, и что если мне суждено пойти в горы Сулиман и быть убитым, я туда пойду и буду убит.',
		},
		{
			sentence: 'God Almighty, no doubt, knows His mind about me, so I need not trouble on that point.',
			translate:
				'Всемогущий Бог, несомненно, знает Своё намерение насчёт меня, так что мне не о чем беспокоиться в этом отношении.',
		},
		{
			sentence: 'Secondly, I am a poor man.',
			translate: 'Во‑вторых, я человек небогатый.',
		},
		{
			sentence: 'For nearly forty years I have hunted and traded, but I have never made more than a living.',
			translate: 'Почти сорок лет я охотился и торговал, но никогда не зарабатывал больше, чем на жизнь.',
		},
		{
			sentence:
				'Well, gentlemen, I don’t know if you are aware that the average life of an elephant hunter from the time he takes to the trade is between four and five years.',
			translate:
				'Что ж, господа, не знаю, известно ли вам, что средний «срок жизни» охотника на слонов с момента, как он берётся за ремесло, — от четырёх до пяти лет.',
		},
		{
			sentence:
				'So you see I have lived through about seven generations of my class, and I should think that my time cannot be far off, anyway.',
			translate:
				'Так что видите: я прожил примерно семь поколений людей моего ремесла, и думаю, мой срок в любом случае уже недалёк.',
		},
		{
			sentence:
				'Now, if anything were to happen to me in the ordinary course of business, by the time my debts are paid there would be nothing left to support my son Harry whilst he was getting in the way of earning a living, whereas now he will be set up for five years.',
			translate:
				'Теперь, если со мной случится что‑нибудь по обычному ходу дел, после уплаты долгов ничего не останется на содержание моего сына Гарри, пока он входит в колею, чтобы зарабатывать на жизнь, тогда как теперь он будет обеспечен на пять лет.',
		},
		{
			sentence: 'There is the whole affair in a nutshell.”',
			translate: '«Вся история — в двух словах».',
		},
		{
			sentence:
				'“Mr. Quatermain,” said Sir Henry, who had been giving me his most serious attention, “your motives for undertaking an enterprise which you believe can only end in disaster reflect a great deal of credit on you.',
			translate:
				'«Мистер Квотермейн, — сказал сэр Генри, который слушал меня с величайшим вниманием, — ваши мотивы предпринять дело, которое, по вашему убеждению, может закончиться лишь гибелью, делают вам большую честь.',
		},
		{
			sentence: 'Whether or not you are right, of course time and the event alone can show.',
			translate: 'Правы вы или нет — это, конечно, покажут лишь время и события.',
		},
		{
			sentence:
				'But whether you are right or wrong, I may as well tell you at once that I am going through with it to the end, sweet or bitter.',
			translate: 'Но правы вы или неправы, скажу сразу: я доведу это до конца — будь он сладок или горек.',
		},
		{
			sentence:
				'If we are to be knocked on the head, all I have to say is, that I hope we get a little shooting first, eh, Good?”',
			translate:
				'Если уж нам суждено получить по голове, то всё, что я хочу сказать: надеюсь, нам сначала доведётся немного пострелять, а, Гуд?»',
		},
		{
			sentence: '“Yes, yes,” put in the captain.',
			translate: '«Да, да», — вставил капитан.',
		},
		{
			sentence:
				'“We have all three of us been accustomed to face danger, and to hold our lives in our hands in various ways, so it is no good turning back now.',
			translate:
				'«Мы втроём привыкли встречать опасность и держать свою жизнь в руках самыми разными способами, так что теперь нет смысла отступать.',
		},
		{
			sentence: 'And now I vote we go down to the saloon and take an observation just for luck, you know.”',
			translate: 'А теперь предлагаю спуститься в салун и «снять наблюдение» на удачу, понимаете».',
		},
		{
			sentence: 'And we did—through the bottom of a tumbler.',
			translate: 'И мы так и сделали — через донышко стакана.',
		},
	],
}
