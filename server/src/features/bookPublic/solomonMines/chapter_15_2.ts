export const chapter_15_2 = {
	name: 'Chapter XV',
	header: 'Good falls sick (2)',
	data: [
		{
			sentence:
				'After that I got a little fitful sleep, only to wake from time to time with a start, thinking that I was once more an actor in the terrible events of the last twenty-four hours.',
			translate:
				'После того я немного подремал беспокойным сном, но то и дело вздрагивая просыпался, думая, что снова участвую в ужасных событиях последних двадцати четырёх часов.',
		},
		{
			sentence:
				'Now I seemed to see that warrior whom my hand had sent to his last account charging at me on the mountain-top; now I was once more in that glorious ring of Greys, which made its immortal stand against all Twala’s regiments upon the little mound; and now again I saw Twala’s plumed and gory head roll past my feet with gnashing teeth and glaring eye.',
			translate:
				'То мне чудилось, что тот воин, которого я отправил на тот свет, несётся на меня на вершине горы; то я вновь был в том славном кольце Серых, которое бессмертно держало оборону против всех полков Твалы на небольшом холме; и вот опять я видел, как украшенная перьями и окровавленная голова Твалы со скалёнными зубами и свирепым взором катится мимо моих ног.',
		},
		{
			sentence:
				'At last, somehow or other, the night passed away; but when dawn broke I found that my companions had slept no better than myself.',
			translate:
				'Наконец, так или иначе, ночь прошла; но когда занялась заря, я обнаружил, что мои товарищи спали не лучше меня.',
		},
		{
			sentence:
				'Good, indeed, was in a high fever, and very soon afterwards began to grow light-headed, and also, to my alarm, to spit blood, the result, no doubt, of some internal injury, inflicted during the desperate efforts made by the Kukuana warrior on the previous day to force his big spear through the chain armour.',
			translate:
				'Гуд, правда, был в сильном жару и вскоре начал бредить, а также, к моему ужасу, плевать кровью — несомненный результат какого‑то внутреннего повреждения, полученного во время отчаянных попыток воина‑кукуана накануне проткнуть его большим копьём кольчужные доспехи.',
		},
		{
			sentence:
				'Sir Henry, however, seemed pretty fresh, notwithstanding his wound on the face, which made eating difficult and laughter an impossibility, though he was so sore and stiff that he could scarcely stir.',
			translate:
				'Сэр Генри, однако, выглядел довольно бодрым, несмотря на рану на лице, делающую приём пищи трудным, а смех — невозможным, хотя он был так избит и скован, что едва мог шевельнуться.',
		},
		{
			sentence:
				'About eight o’clock we had a visit from Infadoos, who appeared but little the worse—tough old warrior that he was—for his exertions in the battle, although he informed us that he had been up all night.',
			translate:
				'Около восьми часов к нам заглянул Инфадус, который, будучи закалённым старым воином, выглядел лишь чуть пострадавшим от усилий в битве, хотя и сообщил нам, что не сомкнул глаз всю ночь.',
		},
		{
			sentence:
				'He was delighted to see us, but much grieved at Good’s condition, and shook our hands cordially.',
			translate: 'Он был рад нас видеть, но очень сокрушался о состоянии Гуда и крепко пожал нам руки.',
		},
		{
			sentence:
				'I noticed, however, that he addressed Sir Henry with a kind of reverence, as though he were something more than man; and, indeed, as we afterwards found out, the great Englishman was looked on throughout Kukuanaland as a supernatural being.',
			translate:
				'Я, однако, заметил, что он обращался к сэру Генри с неким благоговением, словно тот был чем‑то большим, чем человек; и, действительно, как мы позднее узнали, великого англичанина по всему Кукуаналенду считали сверхъестественным существом.',
		},
		{
			sentence:
				'No man, the soldiers said, could have fought as he fought or, at the end of a day of such toil and bloodshed, could have slain Twala, who, in addition to being the king, was supposed to be the strongest warrior in the country, in single combat, shearing through his bull-neck at a stroke.',
			translate:
				'Ни один человек, говорили воины, не смог бы сражаться так, как он, или — в конце дня такого труда и кровопролития — в одиночном поединке сразить Твалу, который, помимо того что был царём, считался сильнейшим воином в стране, одним ударом срезав его бычью шею.',
		},
		{
			sentence:
				'Indeed, that stroke became proverbial in Kukuanaland, and any extraordinary blow or feat of strength was henceforth known as “Incubu’s blow.”',
			translate:
				'И в самом деле, тот удар стал в Кукуаналенде притчей во языцех, и всякий необычайный удар или проявление силы отныне называли «ударом Инкубу».',
		},
		{
			sentence:
				'Infadoos told us also that all Twala’s regiments had submitted to Ignosi, and that like submissions were beginning to arrive from chiefs in the outlying country.',
			translate:
				'Инфадус также сообщил нам, что все полки Твалы подчинились Игноси и что подобные подчинения начали поступать и от вождей из окраинных областей.',
		},
		{
			sentence:
				'Twala’s death at the hands of Sir Henry had put an end to all further chance of disturbance; for Scragga had been his only legitimate son, so there was no rival claimant to the throne left alive.',
			translate:
				'Смерть Твалы от руки сэра Генри положила конец всякой дальнейшей возможности смуты; ведь Скрагга был его единственным законным сыном, так что соперничающих претендентов на трон больше в живых не осталось.',
		},
		{
			sentence: 'I remarked that Ignosi had swum to power through blood.',
			translate: 'Я заметил, что Игноси пришёл к власти через кровь.',
		},
		{
			sentence: 'The old chief shrugged his shoulders.',
			translate: 'Старый вождь пожал плечами.',
		},
		{
			sentence:
				'“Yes,” he answered; “but the Kukuana people can only be kept cool by letting their blood flow sometimes.',
			translate:
				'«Да, — ответил он; — но народ кукуанов можно удерживать спокойным, лишь позволяя их крови изредка проливаться.',
		},
		{
			sentence:
				'Many are killed, indeed, but the women are left, and others must soon grow up to take the places of the fallen.',
			translate:
				'Многих, конечно, убивают, но женщины остаются, и вскоре другие должны вырасти, чтобы занять места павших.',
		},
		{
			sentence: 'After this the land would be quiet for a while.”',
			translate: 'После этого земля на какое‑то время будет спокойна».',
		},
		{
			sentence:
				'Afterwards, in the course of the morning, we had a short visit from Ignosi, on whose brows the royal diadem was now bound.',
			translate:
				'Позднее, в течение утра, нас коротко навестил Игноси, на чьём челе теперь покоилась царская диадема.',
		},
		{
			sentence:
				'As I contemplated him advancing with kingly dignity, an obsequious guard following his steps, I could not help recalling to my mind the tall Zulu who had presented himself to us at Durban some few months back, asking to be taken into our service, and reflecting on the strange revolutions of the wheel of fortune.',
			translate:
				'Созерцая, как он шествует с царственным достоинством, а услужливый караул следует по его стопам, я невольно вспоминал высокого зулуса, который несколько месяцев назад явился к нам в Дурбане, прося взять его к нам на службу, и размышлял о странных переворотах колеса фортуны.',
		},
		{
			sentence: '“Hail, O king!” I said, rising.',
			translate: '«Здрав будь, о царь!» — сказал я, вставая.',
		},
		{
			sentence: '“Yes, Macumazahn. King at last, by the might of your three right hands,” was the ready answer.',
			translate: '«Да, Макумазан. Наконец‑то царь — силою ваших трёх правых рук», — последовал готовый ответ.',
		},
		{
			sentence:
				'All was, he said, going well; and he hoped to arrange a great feast in two weeks’ time in order to show himself to the people.',
			translate:
				'Он сказал, что всё идёт хорошо; и он надеялся устроить через две недели большой пир, чтобы показаться народу.',
		},
	],
}
