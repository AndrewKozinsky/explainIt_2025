export const chapter_11_5 = {
	name: 'Chapter XI (5)',
	header: 'We give a sign (5)',
	data: [
		{
			sentence:
				'Two of the men stepped forward, and as they advanced, the girl, for the first time realising her impending fate, screamed aloud and turned to fly.',
			translate:
				'Двое мужчин выступили вперёд, и, когда они приближались, девушка, впервые осознав свою близкую участь, пронзительно закричала и бросилась бежать.',
		},
		{
			sentence:
				'But the strong hands caught her fast, and brought her, struggling and weeping, before us.',
			translate:
				'Но крепкие руки крепко схватили её и, несмотря на её рыдания и сопротивление, привели к нам.',
		},
		{
			sentence: '“What is thy name, girl?” piped Gagool.',
			translate: '«Как твоё имя, девушка?» — пискнула Гагаула.',
		},
		{
			sentence: '“What! wilt thou not answer? Shall the king’s son do his work at once?”',
			translate: '«Что же! не ответишь? Или сын царя исполнит свою работу немедля?»',
		},
		{
			sentence:
				'At this hint, Scragga, looking more evil than ever, advanced a step and lifted his great spear, and at that moment I saw Good’s hand creep to his revolver.',
			translate:
				'На этот намёк Скрагга, выглядевший злее прежнего, шагнул вперёд и поднял своё большое копьё, и в тот же миг я увидел, как рука Гуда потянулась к револьверу.',
		},
		{
			sentence:
				'The poor girl caught the faint glint of steel through her tears, and it sobered her anguish.',
			translate:
				'Бедная девушка сквозь слёзы уловила слабый блеск стали — и это немного отрезвило её отчаяние.',
		},
		{
			sentence:
				'She ceased struggling, and clasping her hands convulsively, stood shuddering from head to foot.',
			translate:
				'Она перестала сопротивляться и, судорожно сложив руки, стояла, сотрясаясь от головы до пят.',
		},
		{
			sentence:
				'“See,” cried Scragga in high glee, “she shrinks from the sight of my little plaything even before she has tasted it,” and he tapped the broad blade of his spear.',
			translate:
				'«Смотрите, — в великой радости закричал Скрагга, — она съёживается при виде моей игрушечки ещё до того, как попробует её», — и он постучал по широкому лезвию своего копья.',
		},
		{
			sentence:
				'“If ever I get the chance you shall pay for that, you young hound!” I heard Good mutter beneath his breath.',
			translate:
				'«Попадись ты мне когда‑нибудь — заплатишь за это, щенок!» — услышал я, как проворчал Гуд себе под нос.',
		},
		{
			sentence:
				'“Now that thou art quiet, give us thy name, my dear. Come, speak out, and fear not,” said Gagool in mockery.',
			translate:
				'«Раз уж ты угомонилась, назови нам своё имя, милая. Ну же, говори и не бойся», — насмешливо сказала Гагаула.',
		},
		{
			sentence:
				'“Oh, mother,” answered the girl, in trembling accents, “my name is Foulata, of the house of Suko. Oh, mother, why must I die? I have done no wrong!”',
			translate:
				'«О, мать, — ответила девушка дрожащим голосом, — моё имя Фулата, из дома Суко. О, мать, почему я должна умереть? Я не сделала ничего плохого!»',
		},
		{
			sentence: '“Be comforted,” went on the old woman in her hateful tone of mockery.',
			translate: '«Утешайся», — продолжала старуха в своей ненавистной насмешливой манере.',
		},
		{
			sentence:
				'“Thou must die, indeed, as a sacrifice to the Old Ones who sit yonder,” and she pointed to the peaks; “but it is better to sleep in the night than to toil in the daytime; it is better to die than to live, and thou shalt die by the royal hand of the king’s own son.”',
			translate:
				'«Ты и впрямь должна умереть — как жертва Безмолвным, что сидят вон там», — и она указала на вершины; — «но лучше спать ночью, чем трудиться днём; лучше умереть, чем жить, и ты умрёшь от царской руки самого сына царя».',
		},
		{
			sentence:
				'The girl Foulata wrung her hands in anguish, and cried out aloud, “Oh, cruel! and I so young! ',
			translate:
				'Девушка Фулата заломила руки в муке и громко вскрикнула: «О, жестоко! и я такая юная! ',
		},
		{
			sentence:
				'What have I done that I should never again see the sun rise out of the night, or the stars come following on his track in the evening, that I may no more gather the flowers when the dew is heavy, or listen to the laughing of the waters?',
			translate:
				'Что я сделала такого, чтобы мне больше никогда не видеть, как солнце восходит из ночи, или как звёзды идут по его следу вечером; чтобы мне более не собирать цветы, когда роса тяжела, и не слушать смех воды?',
		},
		{
			sentence:
				'Woe is me, that I shall never see my father’s hut again, nor feel my mother’s kiss, nor tend the lamb that is sick!',
			translate:
				'Горе мне, что я больше не увижу хижины моего отца, не почувствую материнского поцелуя и не смогу выхаживать больного ягнёнка!',
		},
		{
			sentence:
				'Woe is me, that no lover shall put his arm around me and look into my eyes, nor shall men children be born of me! Oh, cruel, cruel!”',
			translate:
				'Горе мне, что ни один возлюбленный не обнимет меня и не заглянет в мои глаза, и не родятся от меня сыновья! О, жестоко, жестоко!»',
		},
		{
			sentence:
				'And again she wrung her hands and turned her tear-stained flower-crowned face to Heaven, looking so lovely in her despair—for she was indeed a beautiful woman—that assuredly the sight of her would have melted the hearts of any less cruel than were the three fiends before us.',
			translate:
				'И снова она заломила руки и подняла к небу своё заплаканное, увенчанное цветами лицо, так прекрасная в своём отчаянии — ибо она и впрямь была красавицей, — что вид её наверняка растопил бы сердца кого угодно, только не трёх демонов перед нами.',
		},
		{
			sentence:
				'Prince Arthur’s appeal to the ruffians who came to blind him was not more touching than that of this savage girl.',
			translate:
				'Мольба принца Артура к головорезам, пришедшим ослепить его, не была трогательнее, чем мольба этой дикарки.',
		},
		{
			sentence:
				'But it did not move Gagool or Gagool’s master, though I saw signs of pity among the guards behind, and on the faces of the chiefs; and as for Good, he gave a fierce snort of indignation, and made a motion as though to go to her assistance.',
			translate:
				'Но это не тронуло ни Гагаулу, ни господина Гагаулы, хотя я заметил признаки сострадания у стражей позади и на лицах вождей; что до Гуда, он яростно фыркнул от негодования и сделал движение, словно намереваясь прийти ей на помощь.',
		},
		{
			sentence:
				'With all a woman’s quickness, the doomed girl interpreted what was passing in his mind, and by a sudden movement flung herself before him, and clasped his “beautiful white legs” with her hands.',
			translate:
				'Со всей женской быстротой обречённая поняла, что творится у него в голове, и резким движением бросилась перед ним, обхватив руками его «прекрасные белые ноги».',
		},
		{
			sentence:
				'“Oh, white father from the Stars!” she cried, “throw over me the mantle of thy protection; let me creep into the shadow of thy strength, that I may be saved.',
			translate:
				'«О, белый отец со Звёзд! — вскричала она. — Набрось на меня покров твоей защиты; позволь мне укрыться в тени твоей силы, чтобы я была спасена.',
		},
		{
			sentence: 'Oh, keep me from these cruel men and from the mercies of Gagool!”',
			translate: 'О, избавь меня от этих жестоких людей и от «милостей» Гагаулы!»',
		},
		{
			sentence: '“All right, my hearty, I’ll look after you,” sang out Good in nervous Saxon.',
			translate: '«Ладно, моя хорошая, я о тебе позабочусь», — нервно выкрикнул Гуд по‑саксонски.',
		},
		{
			sentence: '“Come, get up, there’s a good girl,” and he stooped and caught her hand.',
			translate: '«Ну же, вставай, будь умницей», — и он наклонился и взял её за руку.',
		},
		{
			sentence: 'Twala turned and motioned to his son, who advanced with his spear lifted.',
			translate: 'Твала повернулся и сделал знак сыну, и тот выступил вперёд с поднятым копьём.',
		},
		{
			sentence: '“Now’s your time,” whispered Sir Henry to me; “what are you waiting for?”',
			translate: '«Сейчас твой момент, — прошептал мне сэр Генри; — чего ты ждёшь?»',
		},
		{
			sentence:
				'“I am waiting for that eclipse,” I answered; “I have had my eye on the moon for the last half-hour, and I never saw it look healthier.”',
			translate:
				'«Я жду того затмения, — ответил я; — я полчаса не спускаю глаз с луны и никогда не видел, чтобы она выглядела здоровее».',
		},
		{
			sentence: '“Well, you must risk it now, or the girl will be killed. Twala is losing patience.”',
			translate: '«Ну, теперь надо рискнуть, иначе девушку убьют. Твала теряет терпение».',
		},
		{
			sentence:
				'Recognising the force of the argument, and having cast one more despairing look at the bright face of the moon, for never did the most ardent astronomer with a theory to prove await a celestial event with such anxiety, I stepped with all the dignity that I could command between the prostrate girl and the advancing spear of Scragga.',
			translate:
				'Признав силу довода и бросив ещё один отчаянный взгляд на светлое лицо луны — ибо никогда самый горячий астроном, стремящийся доказать свою теорию, не ждал небесного явления с такой тревогой, — я, собрав всю возможную важность, встал между распростёртой девушкой и приближающимся копьём Скрагги.',
		},
		{
			sentence:
				'“King,” I said, “it shall not be; we will not endure this thing; let the girl go in safety.”',
			translate:
				'«О царь, этого не будет; мы не потерпим этого; отпусти девушку невредимой», — сказал я.',
		},
	],
}
