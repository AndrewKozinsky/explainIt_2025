export const chapter_3_3 = {
	name: 'Chapter III',
	header: 'Umbora enters our service (3)',
	data: [
		{
			sentence:
				'Next day we went ashore, and I put up Sir Henry and Captain Good at the little shanty I have built on the Berea, and which I call my home.',
			translate:
				'На следующий день мы сошли на берег, и я разместил сэра Генри и капитана Гуда в небольшом домике на Берее, который я построил и называю своим домом.',
		},
		{
			sentence:
				'There are only three rooms and a kitchen in it, and it is constructed of green brick with a galvanised iron roof, but there is a good garden with the best loquot trees in it that I know, and some nice young mangoes, of which I hope great things.',
			translate:
				'В нём всего три комнаты и кухня; построен он из сырцового кирпича с оцинкованной железной крышей, но при нём есть хороший сад с лучшими мушмуловыми деревьями из всех, что я знаю, и с молодыми манговыми деревцами, на которые я очень рассчитываю.',
		},
		{
			sentence: 'The curator of the botanical gardens gave them to me.',
			translate: 'Их мне подарил смотритель ботанического сада.',
		},
		{
			sentence:
				'It is looked after by an old hunter of mine named Jack, whose thigh was so badly broken by a buffalo cow in Sikukunis country that he will never hunt again.',
			translate:
				'Садом присматривает мой старый охотник по имени Джек, у которого в стране Сикукуниса буйволица так сильно сломала бедро, что охотиться он больше не сможет.',
		},
		{
			sentence: 'But he can potter about and garden, being a Griqua by birth.',
			translate: 'Но возиться по хозяйству и заниматься садом он может; по происхождению он гриква.',
		},
		{
			sentence: 'You will never persuade a Zulu to take much interest in gardening.',
			translate: 'Зулу вы никогда не уговорите серьёзно заинтересоваться садоводством.',
		},
		{
			sentence: 'It is a peaceful art, and peaceful arts are not in his line.',
			translate: 'Это мирное искусство, а мирные искусства не по его части.',
		},
		{
			sentence:
				'Sir Henry and Good slept in a tent pitched in my little grove of orange trees at the end of the garden, for there was no room for them in the house, and what with the smell of the bloom, and the sight of the green and golden fruit—in Durban you will see all three on the tree together — I daresay it is a pleasant place enough, for we have few mosquitos here on the Berea, unless there happens to come an unusually heavy rain.',
			translate:
				'Сэр Генри и Гуд спали в палатке, поставленной в моей маленькой роще апельсиновых деревьев в конце сада, потому что в доме для них не было места; а уж с запахом цветения и видом зелёных и золотых плодов — в Дурбане вы увидите все три стадии разом на одном дереве — полагаю, место это весьма приятно, да и комаров у нас на Берее немного, если только не случится необычно сильный дождь.',
		},
		{
			sentence:
				'Well, to get on — for if I do not, Harry, you will be tired of my story before ever we fetch up at Suliman’s Mountains—having once made up my mind to go I set about making the necessary preparations.',
			translate:
				'Ну, ближе к делу — иначе, Гарри, ты устанешь от моего рассказа ещё до того, как мы доберёмся до гор Сулиман, — раз уж я решил идти, то принялся за необходимые приготовления.',
		},
		{
			sentence: 'First I secured the deed from Sir Henry, providing for you, my boy, in case of accidents.',
			translate:
				'Прежде всего я получил у сэра Генри акт, обеспечивающий тебя, мальчик мой, на случай несчастья.',
		},
		{
			sentence:
				'There was some difficulty about its legal execution, as Sir Henry was a stranger here, and the property to be charged is over the water; but it was ultimately got over with the help of a lawyer, who charged £20 for the job—a price that I thought outrageous.',
			translate:
				'С его юридическим оформлением были некоторые трудности: сэр Генри здесь чужак, да и имущество, на которое налагалось обязательство, за морем; но в конце концов всё уладили с помощью адвоката, который взял за дело 20 фунтов — цена, по‑моему, возмутительная.',
		},
		{
			sentence: 'Then I pocketed my cheque for £500.',
			translate: 'Потом я убрал в карман чек на 500 фунтов.',
		},
		{
			sentence:
				'Having paid this tribute to my bump of caution, I purchased a wagon and a span of oxen on Sir Henry’s behalf, and beauties they were.',
			translate:
				'Уплатив дань своему чувству осторожности, я купил от имени сэра Генри фургон и запряжку волов — и какие это были красавцы.',
		},
		{
			sentence:
				'It was a twenty-two-foot wagon with iron axles, very strong, very light, and built throughout of stink wood; not quite a new one, having been to the Diamond Fields and back, but, in my opinion, all the better for that, for I could see that the wood was well seasoned.',
			translate:
				'Это был двадцатидвухфутовый фургон с железными осями, очень крепкий, очень лёгкий и весь из стинквуда; не совсем новый — уже сходил на Алмазные прииски и обратно, — но, на мой взгляд, тем и лучше: я видел, что древесина хорошо выдержана.',
		},
		{
			sentence:
				'If anything is going to give in a wagon, or if there is green wood in it, it will show out on the first trip.',
			translate:
				'Если в фургоне чему‑то суждено подвести, или если в нём есть сырой лес, это проявится уже в первом рейсе.',
		},
		{
			sentence:
				'This particular vehicle was what we call a “half-tented” wagon, that is to say, only covered in over the after twelve feet, leaving all the front part free for the necessaries we had to carry with us.',
			translate:
				'Именно этот экипаж был так называемым «полупалаточным» фургоном, то есть закрытым только на задних двенадцати футах, оставляя всю переднюю часть свободной для необходимых припасов.',
		},
		{
			sentence:
				'In this after part were a hide “cartle,” or bed, on which two people could sleep, also racks for rifles, and many other little conveniences.',
			translate:
				'В этой задней части были кожевая «cartle», то есть лежанка, на которой могли спать двое, стойки для ружей и многие другие мелкие удобства.',
		},
		{
			sentence: 'I gave £125 for it, and think that it was cheap at the price.',
			translate: 'Я заплатил за него 125 фунтов и считаю, что это дёшево за такую вещь.',
		},
		{
			sentence:
				'Then I bought a beautiful team of twenty Zulu oxen, which I had kept my eye on for a year or two.',
			translate:
				'Затем я купил прекрасную упряжь из двадцати зулусских волов, за которой присматривал уже год‑другой.',
		},
		{
			sentence: 'Sixteen oxen is the usual number for a team, but I took four extra to allow for casualties.',
			translate: 'Обычно в упряжке бывает шестнадцать волов, но я взял ещё четырёх про запас — на случай потерь.',
		},
		{
			sentence:
				'These Zulu cattle are small and light, not more than half the size of the Africander oxen, which are generally used for transport purposes; but they will live where the Africanders would starve, and with a moderate load can make five miles a day better going, being quicker and not so liable to become footsore.',
			translate:
				'Эти зулусские быки малы и легки — не более половины размера африкандерских волов, которых обычно используют для перевозок; зато они выживут там, где африкандеры бы подохли с голоду, и при умеренном грузе идут на пять миль в день лучше — быстрее и меньше сбивают ноги.',
		},
		{
			sentence:
				'What is more, this lot were thoroughly “salted,” that is, they had worked all over South Africa, and so had become proof, comparatively speaking, against red water, which so frequently destroys whole teams of oxen when they get on to strange “veldt” or grass country.',
			translate:
				'Более того, эта партия была как следует «солёной», то есть ходила по всей Южной Африке и потому стала, сравнительно говоря, устойчива к «red water» — «красной воде», которая так часто губит целые упряжки волов, когда они попадают на незнакомый вельд, или степные пастбища.',
		},
		{
			sentence:
				'As for “lung sick,” which is a dreadful form of pneumonia, very prevalent in this country, they had all been inoculated against it.',
			translate:
				'Что до «lung sick», тяжёлой формы воспаления лёгких, очень распространённой в этих краях, все они были от неё привиты.',
		},
		{
			sentence:
				'This is done by cutting a slit in the tail of an ox, and binding in a piece of the diseased lung of an animal which has died of the sickness.',
			translate:
				'Это делается так: в хвосте быка делают разрез и привязывают туда кусочек поражённого лёгкого животного, погибшего от этой хвори.',
		},
		{
			sentence:
				'The result is that the ox sickens, takes the disease in a mild form, which causes its tail to drop off, as a rule about a foot from the root, and becomes proof against future attacks.',
			translate:
				'В результате бык заболевает в лёгкой форме, от чего его хвост отпадает — обычно примерно в футе от корня, — и затем он становится устойчив к дальнейшим нападениям болезни.',
		},
		{
			sentence:
				'It seems cruel to rob the animal of his tail, especially in a country where there are so many flies, but it is better to sacrifice the tail and keep the ox than to lose both tail and ox, for a tail without an ox is not much good, except to dust with.',
			translate:
				'Жестоко лишать животное хвоста, особенно в стране, где так много мух, но лучше пожертвовать хвостом и сохранить быка, чем потерять и хвост, и быка, потому что хвост без быка мало на что годится — разве что пыль смахивать.',
		},
		{
			sentence: 'Still it does look odd to trek along behind twenty stumps, where there ought to be tails.',
			translate: 'И всё же странно тащиться в пути позади двадцати обрубков там, где должны быть хвосты.',
		},
		{
			sentence:
				'It seems as though Nature made a trifling mistake, and stuck the stern ornaments of a lot of prize bull-dogs on to the rumps of the oxen.',
			translate:
				'Будто Природа слегка ошиблась и пришпилила хвостовые украшения от кучи призовых бульдогов к крупам волов.',
		},
		{
			sentence:
				'Next came the question of provisioning and medicines, one which required the most careful consideration, for what we had to do was to avoid lumbering the wagon, and yet to take everything absolutely necessary.',
			translate:
				'Затем настал черёд вопроса о припасах и лекарствах — вопроса, требовавшего величайшей вдумчивости: нам надо было не перегрузить фургон и вместе с тем взять всё совершенно необходимое.',
		},
		{
			sentence:
				'Fortunately, it turned out that Good is a bit of a doctor, having at some point in his previous career managed to pass through a course of medical and surgical instruction, which he has more or less kept up.',
			translate:
				'К счастью, выяснилось, что Гуд немного врач: в какой‑то момент своей прежней карьеры он прошёл курс медицинской и хирургической подготовки и с тех пор более‑менее поддерживал навыки.',
		},
		{
			sentence:
				'He is not, of course, qualified, but he knows more about it than many a man who can write M.D. after his name, as we found out afterwards, and he had a splendid travelling medicine chest and a set of instruments.',
			translate:
				'Разумеется, диплома у него нет, но, как мы впоследствии убедились, он знает в этом деле больше, чем иной, кто вправе приписывать к имени M.D.; к тому же у него был превосходный походный аптечный ящик и набор инструментов.',
		},
		{
			sentence: 'Whilst we were at Durban he cut off a Kafir’s big toe in a way which it was a pleasure to see.',
			translate:
				'Пока мы были в Дурбане, он отнял у кафра большой палец на ноге — так, что было приятно смотреть.',
		},
		{
			sentence:
				'But he was quite nonplussed when the Kafir, who had sat stolidly watching the operation, asked him to put on another, saying that a “white one” would do at a pinch.',
			translate:
				'Но он был совершенно озадачен, когда кафр, неподвижно просидевший и наблюдавший операцию, попросил приделать другой, сказав, что «белый» тоже сойдёт в крайнем случае.',
		},
		{
			sentence:
				'There remained, when these questions were satisfactorily settled, two further important points for consideration, namely, that of arms and that of servants.',
			translate:
				'Когда с этими вопросами было благополучно покончено, остались ещё два важных пункта для рассмотрения: вооружение и прислуга.',
		},
		{
			sentence:
				'As to the arms I cannot do better than put down a list of those which we finally decided on from among the ample store that Sir Henry had brought with him from England, and those which I owned.',
			translate:
				'Что до оружия, то лучше всего будет привести список того, на чём мы в итоге остановились — из богатого запаса, который сэр Генри привёз с собой из Англии, и из того, что было у меня.',
		},
		{
			sentence: 'I copy it from my pocket-book, where I made the entry at the time.',
			translate: 'Я переписываю его из записной книжки, куда тогда занёс.',
		},
	],
}
