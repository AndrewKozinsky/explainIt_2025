export const chapter_3 = {
	name: 'Chapter III',
	header: 'How Dorothy Saved the Scarecrow',
	data: [
		{
			sentence: 'When Dorothy was left alone she began to feel hungry.',
			translate: 'Когда Дороти осталась одна, она почувствовала голод.',
		},
		{
			sentence: 'So she went to the cupboard and cut herself some bread, which she spread with butter.',
			translate: 'Она подошла к буфету, порезала себе хлеб и намазала его маслом.',
		},
		{
			sentence:
				'She gave some to Toto, and taking a pail from the shelf she carried it down to the little brook and filled it with clear, sparkling water.',
			translate:
				'Она дала немного Тото и, взяв с полки вёдрышко, отнесла его к ручью и наполнила чистой искрящейся водой.',
		},
		{
			sentence: 'Toto ran over to the trees and began to bark at the birds sitting there.',
			translate: 'Тото подбежал к деревьям и принялся лаять на сидевших там птиц.',
		},
		{
			sentence:
				'Dorothy went to get him, and saw such delicious fruit hanging from the branches that she gathered some of it, finding it just what she wanted to help out her breakfast.',
			translate:
				'Дороти пошла за ним и увидела на ветвях такие аппетитные плоды, что сорвала немного — как раз то, что хотелось добавить к завтраку.',
		},
		{
			sentence:
				'Then she went back to the house, and having helped herself and Toto to a good drink of the cool, clear water, she set about making ready for the journey to the City of Emeralds.',
			translate:
				'Потом она вернулась в дом и, напоив себя и Тото прохладной чистой водой, принялась готовиться к пути в Изумрудный город.',
		},
		{
			sentence:
				'Dorothy had only one other dress, but that happened to be clean and was hanging on a peg beside her bed.',
			translate:
				'У Дороти было всего ещё одно платье, но оно как раз было чистым и висело на крючке возле кровати.',
		},
		{
			sentence:
				'It was gingham, with checks of white and blue; and although the blue was somewhat faded with many washings, it was still a pretty frock.',
			translate:
				'Это был ситцевый сарафан в бело‑синюю клетку; и хотя синий немного выцвел от многочисленных стирок, платье всё равно было милым.',
		},
		{
			sentence:
				'The girl washed herself carefully, dressed herself in the clean gingham, and tied her pink sunbonnet on her head.',
			translate:
				'Девочка тщательно умылась, надела чистое клетчатое платье и повязала на голову розовую косынку от солнца.',
		},
		{
			sentence:
				'She took a little basket and filled it with bread from the cupboard, laying a white cloth over the top.',
			translate: 'Она взяла маленькую корзинку, положила в неё хлеб из буфета и накрыла сверху белой салфеткой.',
		},
		{
			sentence: 'Then she looked down at her feet and noticed how old and worn her shoes were.',
			translate: 'Потом она посмотрела на свои ноги и заметила, какие у неё старые и изношенные башмаки.',
		},
		{
			sentence: '“They surely will never do for a long journey, Toto,” she said.',
			translate: '«Для такого длинного пути они никуда не годятся, Тото», — сказала она.',
		},
		{
			sentence:
				'And Toto looked up into her face with his little black eyes and wagged his tail to show he knew what she meant.',
			translate: 'И Тото поднял на неё свои чёрные глазки и завилял хвостом, показывая, что понял её.',
		},
		{
			sentence:
				'At that moment Dorothy saw lying on the table the silver shoes that had belonged to the Witch of the East.',
			translate: 'В этот момент Дороти увидела на столе серебряные башмачки, принадлежавшие Ведьме Востока.',
		},
		{
			sentence: '“I wonder if they will fit me,” she said to Toto.',
			translate: '«Интересно, подойдут ли они мне», — сказала она Тото.',
		},
		{
			sentence:
				'“They would be just the thing to take a long walk in, for they could not wear out.” She took off her old leather shoes and tried on the silver ones, which fitted her as well as if they had been made for her.',
			translate:
				'«В них как раз можно будет долго идти — они ведь не износятся». Она сняла свои старые кожаные башмаки и примерила серебряные — они подошли ей так, словно были сделаны специально для неё.',
		},
		{
			sentence: 'Finally she picked up her basket.',
			translate: 'Наконец она взяла свою корзинку.',
		},
		{
			sentence: '“Come along, Toto,” she said.',
			translate: '«Пойдём, Тото», — сказала она.',
		},
		{
			sentence:
				'“We will go to the Emerald City and ask the Great Oz how to get back to Kansas again.” She closed the door, locked it, and put the key carefully in the pocket of her dress.',
			translate:
				'«Мы пойдём в Изумрудный город и попросим Великого Оза сказать, как вернуться в Канзас». Она закрыла дверь, заперла её и аккуратно положила ключ в карман платья.',
		},
		{
			sentence: 'And so, with Toto trotting along soberly behind her, she started on her journey.',
			translate: 'И так, с Тото, серьёзно семенившим следом, она отправилась в путь.',
		},
		{
			sentence:
				'There were several roads nearby, but it did not take her long to find the one paved with yellow bricks.',
			translate: 'Рядом было несколько дорог, но ей быстро удалось найти ту, что была вымощена жёлтым кирпичом.',
		},
		{
			sentence:
				'Within a short time she was walking briskly toward the Emerald City, her silver shoes tinkling merrily on the hard, yellow road-bed.',
			translate:
				'Вскоре она бодро шагала к Изумрудному городу, и её серебряные башмачки весело позванивали по твёрдому жёлтому настилу.',
		},
		{
			sentence:
				'The sun shone bright and the birds sang sweetly, and Dorothy did not feel nearly so bad as you might think a little girl would who had been suddenly whisked away from her own country and set down in the midst of a strange land.',
			translate:
				'Солнце ярко светило, птицы сладко пели, и Дороти было далеко не так плохо, как можно было бы ожидать от девочки, внезапно унесённой из родной страны и оставленной посреди незнакомой земли.',
		},
		{
			sentence: 'She was surprised, as she walked along, to see how pretty the country was about her.',
			translate: 'Идя дальше, она с удивлением видела, как красива вокруг местность.',
		},
		{
			sentence:
				'There were neat fences at the sides of the road, painted a dainty blue color, and beyond them were fields of grain and vegetables in abundance.',
			translate:
				'По краям дороги тянулись аккуратные заборчики, окрашенные в нежно‑голубой цвет, а за ними — обильные поля злаков и овощей.',
		},
		{
			sentence: 'Evidently the Munchkins were good farmers and able to raise large crops.',
			translate: 'Очевидно, мункины были хорошими земледельцами и умели выращивать богатые урожаи.',
		},
		{
			sentence:
				'Once in a while she would pass a house, and the people came out to look at her and bow low as she went by; for everyone knew she had been the means of destroying the Wicked Witch and setting them free from bondage.',
			translate:
				'Порой ей попадался дом, и люди выходили посмотреть на неё и низко кланялись, ведь все знали, что именно благодаря ей была уничтожена Злая Ведьма и они получили свободу.',
		},
		{
			sentence:
				'The houses of the Munchkins were odd-looking dwellings, for each was round, with a big dome for a roof.',
			translate: 'Дома мункинов были необычные: каждый круглый, с большой куполообразной крышей.',
		},
		{
			sentence: 'All were painted blue, for in this country of the East blue was the favorite color.',
			translate: 'Все они были окрашены в голубой — любимый цвет в этой восточной стране.',
		},
		{
			sentence:
				'Toward evening, when Dorothy was tired with her long walk and began to wonder where she should pass the night, she came to a house rather larger than the rest.',
			translate:
				'Под вечер, утомившись долгой ходьбой и размышляя, где бы переночевать, она подошла к дому, чуть большему, чем остальные.',
		},
		{
			sentence: 'On the green lawn before it many men and women were dancing.',
			translate: 'На зелёной лужайке перед ним танцевали мужчины и женщины.',
		},
		{
			sentence:
				'Five little fiddlers played as loudly as possible, and the people were laughing and singing, while a big table near by was loaded with delicious fruits and nuts, pies and cakes, and many other good things to eat.',
			translate:
				'Пятеро маленьких скрипачей играли изо всех сил, люди смеялись и пели, а рядом стоял большой стол, ломившийся от вкусных фруктов и орехов, пирогов и тортов и всякой другой снеди.',
		},
		{
			sentence:
				'The people greeted Dorothy kindly, and invited her to supper and to pass the night with them; for this was the home of one of the richest Munchkins in the land, and his friends were gathered with him to celebrate their freedom from the bondage of the Wicked Witch.',
			translate:
				'Люди приветливо встретили Дороти, пригласили её к ужину и переночевать у них; это был дом одного из самых богатых мункинов, и друзья собрались у него отпраздновать освобождение от власти Злой Ведьмы.',
		},
		{
			sentence:
				'Dorothy ate a hearty supper and was waited upon by the rich Munchkin himself, whose name was Boq.',
			translate: 'Дороти сытно поужинала; ей прислуживал сам богатый мункин по имени Бок.',
		},
		{
			sentence: 'Then she sat upon a settee and watched the people dance.',
			translate: 'Потом она села на скамью и стала смотреть, как танцуют люди.',
		},
		{
			sentence: 'When Boq saw her silver shoes he said, “You must be a great sorceress.” “Why?” asked the girl.',
			translate:
				'Увидев её серебряные башмачки, Бок сказал: «Должно быть, ты великая чародейка». «Почему?» — спросила девочка.',
		},
		{
			sentence: '“Because you wear silver shoes and have killed the Wicked Witch.',
			translate: '«Потому что ты носишь серебряные башмачки и убила Злую Ведьму.',
		},
		{
			sentence:
				'Besides, you have white in your frock, and only witches and sorceresses wear white.” “My dress is blue and white checked,” said Dorothy, smoothing out the wrinkles in it.',
			translate:
				'Кроме того, в твоём платье есть белый цвет, а белое носят только ведьмы и волшебницы». «Моё платье — синяя и белая клетка», — сказала Дороти, разглаживая на нём складки.',
		},
		{
			sentence: '“It is kind of you to wear that,” said Boq.',
			translate: '«Это очень мило с твоей стороны — носить такое», — сказал Бок.',
		},
		{
			sentence: '“Blue is the color of the Munchkins, and white is the witch color.',
			translate: '«Синий — цвет мункинов, а белый — цвет ведьм.',
		},
		{
			sentence:
				'So we know you are a friendly witch.” Dorothy did not know what to say to this, for all the people seemed to think her a witch, and she knew very well she was only an ordinary little girl who had come by the chance of a cyclone into a strange land.',
			translate:
				'Так что мы знаем, что ты добрая ведьма». Дороти не знала, что на это ответить: все, казалось, считали её ведьмой, хотя она прекрасно понимала, что она лишь обычная девочка, занесённая сюда случайностью циклона.',
		},
		{
			sentence:
				'When she had tired watching the dancing, Boq led her into the house, where he gave her a room with a pretty bed in it.',
			translate: 'Когда ей надоело смотреть танцы, Бок провёл её в дом и отвёл комнату с красивой кроватью.',
		},
		{
			sentence:
				'The sheets were made of blue cloth, and Dorothy slept soundly in them till morning, with Toto curled up on the blue rug beside her.',
			translate:
				'Простыни были из голубой ткани, и Дороти крепко спала в них до утра, а Тото клубочком лежал рядом на голубом коврике.',
		},
		{
			sentence:
				'She ate a hearty breakfast, and watched a wee Munchkin baby, who played with Toto and pulled his tail and crowed and laughed in a way that greatly amused Dorothy.',
			translate:
				'Она сытно позавтракала и наблюдала за крошкой‑мункином, который играл с Тото, дёргал его за хвост и радостно гоготал и смеялся — что очень забавляло Дороти.',
		},
		{
			sentence: 'Toto was a fine curiosity to all the people, for they had never seen a dog before.',
			translate: 'Для всех людей Тото был настоящей диковинкой — они никогда прежде не видели собаки.',
		},
		{
			sentence: '“How far is it to the Emerald City?” the girl asked.',
			translate: '«Как далеко до Изумрудного города?» — спросила девочка.',
		},
		{
			sentence: '“I do not know,” answered Boq gravely, “for I have never been there.',
			translate: '«Не знаю, — серьёзно ответил Бок, — я там никогда не был.',
		},
		{
			sentence: 'It is better for people to keep away from Oz, unless they have business with him.',
			translate: 'Людям лучше держаться подальше от Оза, если только им не нужно к нему по делу.',
		},
		{
			sentence: 'But it is a long way to the Emerald City, and it will take you many days.',
			translate: 'А дорога до Изумрудного города длинная, и у тебя уйдут на неё многие дни.',
		},
		{
			sentence:
				'The country here is rich and pleasant, but you must pass through rough and dangerous places before you reach the end of your journey.” This worried Dorothy a little, but she knew that only the Great Oz could help her get to Kansas again, so she bravely resolved not to turn back.',
			translate:
				'Здесь страна богата и приятна, но тебе предстоит пройти через суровые и опасные места, прежде чем доберёшься до конца пути». Это немного встревожило Дороти, но она знала, что только Великий Оз поможет ей снова попасть в Канзас, и смело решила не поворачивать назад.',
		},
		{
			sentence: 'She bade her friends good-bye, and again started along the road of yellow brick.',
			translate: 'Она попрощалась с друзьями и снова отправилась по жёлтой кирпичной дороге.',
		},
		{
			sentence:
				'When she had gone several miles she thought she would stop to rest, and so climbed to the top of the fence beside the road and sat down.',
			translate: 'Пройдя несколько миль, она решила отдохнуть, забралась на верх забора у дороги и села.',
		},
		{
			sentence:
				'There was a great cornfield beyond the fence, and not far away she saw a Scarecrow, placed high on a pole to keep the birds from the ripe corn.',
			translate:
				'За забором простиралось большое кукурузное поле, и неподалёку она увидела Пугало, высоко насаженное на кол, чтобы отпугивать птиц от спелой кукурузы.',
		},
		{
			sentence: 'Dorothy leaned her chin upon her hand and gazed thoughtfully at the Scarecrow.',
			translate: 'Дороти опёрлась подбородком на руку и задумчиво уставилась на Пугало.',
		},
		{
			sentence:
				'Its head was a small sack stuffed with straw, with eyes, nose, and mouth painted on it to represent a face.',
			translate:
				'Его голова была небольшим мешочком, набитым соломой; глаза, нос и рот были нарисованы, изображая лицо.',
		},
		{
			sentence:
				'An old, pointed blue hat, that had belonged to some Munchkin, was perched on his head, and the rest of the figure was a blue suit of clothes, worn and faded, which had also been stuffed with straw.',
			translate:
				'На голове сидела старая остроконечная синяя шляпа, принадлежавшая какому‑то мункину, а остальная фигура — синий костюм, поношенный и выцветший — тоже была набита соломой.',
		},
		{
			sentence:
				'On the feet were some old boots with blue tops, such as every man wore in this country, and the figure was raised above the stalks of corn by means of the pole stuck up its back.',
			translate:
				'На ногах были старые сапоги с синими голенищами, какие носил здесь каждый, а фигуру держал над кукурузными стеблями шест, вонзённый в её спину.',
		},
		{
			sentence:
				'While Dorothy was looking earnestly into the queer, painted face of the Scarecrow, she was surprised to see one of the eyes slowly wink at her.',
			translate:
				'Пока Дороти пристально всматривалась в странное нарисованное лицо Пугала, она с удивлением увидела, как один из глаз медленно подмигнул ей.',
		},
		{
			sentence:
				'She thought she must have been mistaken at first, for none of the scarecrows in Kansas ever wink; but presently the figure nodded its head to her in a friendly way.',
			translate:
				'Сначала она решила, что ей показалось — ведь ни одно пугало в Канзасе не подмигивает; но вскоре фигура кивнула ей дружелюбно.',
		},
		{
			sentence:
				'Then she climbed down from the fence and walked up to it, while Toto ran around the pole and barked.',
			translate: 'Тогда она слезла с забора и подошла к нему, а Тото бегал вокруг шеста и лаял.',
		},
		{
			sentence: '“Good day,” said the Scarecrow, in a rather husky voice.',
			translate: '«Добрый день», — сказал Пугало довольно хриплым голосом.',
		},
		{
			sentence: '“Did you speak?” asked the girl, in wonder.',
			translate: '«Это ты сказал?» — удивилась девочка.',
		},
		{
			sentence: '“Certainly,” answered the Scarecrow.',
			translate: '«Разумеется», — ответило Пугало.',
		},
		{
			sentence: '“How do you do?” “I’m pretty well, thank you,” replied Dorothy politely.',
			translate: '«Как поживаешь?» «Спасибо, неплохо», — вежливо ответила Дороти.',
		},
		{
			sentence:
				'“How do you do?” “I’m not feeling well,” said the Scarecrow, with a smile, “for it is very tedious being perched up here night and day to scare away crows.” “Can’t you get down?” asked Dorothy.',
			translate:
				'«А ты?» — сказал Пугало с улыбкой, — «Мне не слишком хорошо, — сказал Пугало, — целыми днями и ночами торчать здесь, отпугивая ворон, очень скучно». «А ты не можешь слезть?» — спросила Дороти.',
		},
		{
			sentence: '“No, for this pole is stuck up my back.',
			translate: '«Нет, шест воткнут мне в спину.',
		},
		{
			sentence:
				'If you will please take away the pole I shall be greatly obliged to you.” Dorothy reached up both arms and lifted the figure off the pole, for, being stuffed with straw, it was quite light.',
			translate:
				'Если бы ты убрала шест, я был бы тебе очень признателен». Дороти подняла обе руки и сняла фигуру с шеста — ведь, будучи набитой соломой, она была совсем лёгкой.',
		},
		{
			sentence: '“Thank you very much,” said the Scarecrow, when he had been set down on the ground.',
			translate: '«Большое спасибо», — сказал Пугало, когда его поставили на землю.',
		},
		{
			sentence:
				'“I feel like a new man.” Dorothy was puzzled at this, for it sounded queer to hear a stuffed man speak, and to see him bow and walk along beside her.',
			translate:
				'«Чувствую себя совсем новым человеком». Дороти это удивило: странно было слышать, как говорит набитый соломой человек, видеть, как он кланяется и шагает рядом с ней.',
		},
		{
			sentence: '“Who are you?” asked the Scarecrow when he had stretched himself and yawned.',
			translate: '«Кто ты?» — спросило Пугало, потянувшись и зевнув.',
		},
		{
			sentence:
				'“And where are you going?” “My name is Dorothy,” said the girl, “and I am going to the Emerald City, to ask the Great Oz to send me back to Kansas.” “Where is the Emerald City?” he inquired.',
			translate:
				'«И куда ты идёшь?» «Меня зовут Дороти, — сказала девочка, — я иду в Изумрудный город просить Великого Оза отправить меня обратно в Канзас». «А где Изумрудный город?» — поинтересовался он.',
		},
		{
			sentence: '“And who is Oz?” “Why, don’t you know?” she returned, in surprise.',
			translate: '«А кто такой Оз?» «Разве ты не знаешь?» — изумилась она.',
		},
		{
			sentence: '“No, indeed.',
			translate: '«Нет, вовсе нет.',
		},
		{
			sentence: 'I don’t know anything.',
			translate: 'Я вообще ничего не знаю.',
		},
		{
			sentence: 'You see, I am stuffed, so I have no brains at all,” he answered sadly.',
			translate: 'Видишь ли, я набит соломой, так что у меня вовсе нет мозгов», — печально ответил он.',
		},
		{
			sentence:
				'“Oh,” said Dorothy, “I’m awfully sorry for you.” “Do you think,” he asked, “if I go to the Emerald City with you, that Oz would give me some brains?” “I cannot tell,” she returned, “but you may come with me, if you like.',
			translate:
				'«Ох, — сказала Дороти, — мне так жаль тебя». «Как думаешь, — спросил он, — если я пойду в Изумрудный город с тобой, Оз даст мне немного мозгов?» «Не знаю, — ответила она, — но можешь пойти со мной, если хочешь.',
		},
		{
			sentence:
				'If Oz will not give you any brains you will be no worse off than you are now.” “That is true,” said the Scarecrow.',
			translate: 'Если Оз не даст тебе мозгов, тебе не станет хуже, чем сейчас». «Это верно», — сказал Пугало.',
		},
		{
			sentence:
				'“You see,” he continued confidentially, “I don’t mind my legs and arms and body being stuffed, because I cannot get hurt.',
			translate:
				'«Понимаешь, — доверительно продолжил он, — меня не беспокоит, что мои руки, ноги и тело набиты соломой: меня нельзя поранить.',
		},
		{
			sentence: 'If anyone treads on my toes or sticks a pin into me, it doesn’t matter, for I can’t feel it.',
			translate: 'Если кто‑нибудь наступит мне на пальцы или уколет булавкой — всё равно, я этого не чувствую.',
		},
		{
			sentence:
				'But I do not want people to call me a fool, and if my head stays stuffed with straw instead of with brains, as yours is, how am I ever to know anything?” “I understand how you feel,” said the little girl, who was truly sorry for him.',
			translate:
				'Но я не хочу, чтобы меня считали глупцом; и если моя голова останется набитой соломой, а не мозгами, как у тебя, — как же мне когда‑нибудь что‑нибудь узнать?» «Я понимаю, как ты себя чувствуешь», — сказала девочка, которой было его по‑настоящему жаль.',
		},
		{
			sentence:
				'“If you will come with me I’ll ask Oz to do all he can for you.” “Thank you,” he answered gratefully.',
			translate:
				'«Если пойдёшь со мной, я попрошу Оза сделать для тебя всё, что сможет». «Спасибо», — благодарно ответил он.',
		},
		{
			sentence: 'They walked back to the road.',
			translate: 'Они вернулись к дороге.',
		},
		{
			sentence:
				'Dorothy helped him over the fence, and they started along the path of yellow brick for the Emerald City.',
			translate:
				'Дороти помогла ему перелезть через забор, и они направились по жёлтой кирпичной тропе к Изумрудному городу.',
		},
		{
			sentence: 'Toto did not like this addition to the party at first.',
			translate: 'Тото поначалу не понравился такой попутчик.',
		},
		{
			sentence:
				'He smelled around the stuffed man as if he suspected there might be a nest of rats in the straw, and he often growled in an unfriendly way at the Scarecrow.',
			translate:
				'Он обнюхивал набитого соломой человечка, будто подозревал, что в соломе завелось гнездо крыс, и часто недружелюбно рычал на Пугало.',
		},
		{
			sentence: '“Don’t mind Toto,” said Dorothy to her new friend.',
			translate: '«Не обращай внимания на Тото», — сказала Дороти своему новому другу.',
		},
		{
			sentence: '“He never bites.” “Oh, I’m not afraid,” replied the Scarecrow.',
			translate: '«Он никогда не кусается». «Я и не боюсь», — ответило Пугало.',
		},
		{
			sentence: '“He can’t hurt the straw.',
			translate: '«Соломе он не повредит.',
		},
		{
			sentence: 'Do let me carry that basket for you.',
			translate: 'Позволь мне нести твою корзинку.',
		},
		{
			sentence: 'I shall not mind it, for I can’t get tired.',
			translate: 'Мне это не в тягость: я ведь не устаю.',
		},
		{
			sentence: 'I’ll tell you a secret,” he continued, as he walked along.',
			translate: '«Я открою тебе секрет», — продолжил он на ходу.',
		},
		{
			sentence:
				'“There is only one thing in the world I am afraid of.” “What is that?” asked Dorothy; “the Munchkin farmer who made you?” “No,” answered the Scarecrow; “it’s a lighted match.”',
			translate:
				'«Есть лишь одна вещь на свете, которой я боюсь». «И что же это? — спросила Дороти. — Тот самый мункин‑фермер, что сделал тебя?» «Нет, — ответило Пугало, — горящая спичка».',
		},
	],
}
