export const chapter_17 = {
	name: 'Chapter XVII',
	header: 'How the Balloon Was Launched',
	data: [
		{
			sentence: 'For three days Dorothy heard nothing from Oz.',
			translate: 'Три дня Дороти не получала вестей от Оза.',
		},
		{
			sentence:
				'These were sad days for the little girl, although her friends were all quite happy and contented.',
			translate:
				'Это были печальные дни для девочки, хотя её друзья были вполне счастливы и довольны.',
		},
		{
			sentence:
				'The Scarecrow told them there were wonderful thoughts in his head; but he would not say what they were because he knew no one could understand them but himself.',
			translate:
				'Пугало говорил им, что в его голове чудесные мысли; но не рассказывал какие, ведь знал, что кроме него их никто не поймёт.',
		},
		{
			sentence:
				'When the Tin Woodman walked about he felt his heart rattling around in his breast; and he told Dorothy he had discovered it to be a kinder and more tender heart than the one he had owned when he was made of flesh.',
			translate:
				'Когда Железный Дровосек ходил, он чувствовал, как его сердце постукивает в груди; и он сказал Дороти, что обнаружил: оно добрее и нежнее того, которое было у него, когда он был из плоти.',
		},
		{
			sentence:
				'The Lion declared he was afraid of nothing on earth, and would gladly face an army or a dozen of the fierce Kalidahs.',
			translate:
				'Лев заявил, что не боится ничто на земле и охотно встретил бы армию или дюжину свирепых Калидас.',
		},
		{
			sentence:
				'Thus each of the little party was satisfied except Dorothy, who longed more than ever to get back to Kansas.',
			translate:
				'Так каждый из маленькой компании был доволен, кроме Дороти, которая как никогда тосковала по возвращению в Канзас.',
		},
		{
			sentence:
				'On the fourth day, to her great joy, Oz sent for her, and when she entered the Throne Room he greeted her pleasantly:',
			translate:
				'На четвёртый день, к её великой радости, Оз позвал её, и когда она вошла в Тронный зал, он любезно поприветствовал её:',
		},
		{
			sentence: '“Sit down, my dear; I think I have found the way to get you out of this country.”',
			translate: '«Садись, дорогая; кажется, я нашёл способ вывести тебя из этой страны».',
		},
		{
			sentence: '“And back to Kansas?” she asked eagerly.',
			translate: '«И обратно в Канзас?» — поспешно спросила она.',
		},
		{
			sentence:
				'“Well, I’m not sure about Kansas,” said Oz, “for I haven’t the faintest notion which way it lies. But the first thing to do is to cross the desert, and then it should be easy to find your way home.”',
			translate:
				'«Ну, насчёт Канзаса я не уверен, — сказал Оз, — ведь не имею ни малейшего понятия, в какой стороне он. Но сперва нужно пересечь пустыню, а потом уже будет несложно найти дорогу домой».',
		},
		{
			sentence: '“How can I cross the desert?” she inquired.',
			translate: '«Как же мне пересечь пустыню?» — спросила она.',
		},
		{
			sentence:
				'“Well, I’ll tell you what I think,” said the little man. “You see, when I came to this country it was in a balloon. You also came through the air, being carried by a cyclone. So I believe the best way to get across the desert will be through the air. Now, it is quite beyond my powers to make a cyclone; but I’ve been thinking the matter over, and I believe I can make a balloon.”',
			translate:
				'«Скажу, как я думаю, — сказал маленький человек. — Когда я попал в эту страну, это было на воздушном шаре. Ты тоже прилетела по воздуху, унесённая циклоном. Значит, лучший способ пересечь пустыню — по воздуху. Циклон создать мне не под силу; но я обдумал всё и верю, что смогу сделать шар».',
		},
		{
			sentence: '“How?” asked Dorothy.',
			translate: '«Как?» — спросила Дороти.',
		},
		{
			sentence:
				'“A balloon,” said Oz, “is made of silk, which is coated with glue to keep the gas in it. I have plenty of silk in the Palace, so it will be no trouble to make the balloon. But in all this country there is no gas to fill the balloon with, to make it float.”',
			translate:
				'«Шар, — сказал Оз, — делается из шёлка, который покрывают клеем, чтобы удерживать газ. В Дворце у меня достаточно шёлка, так что сделать шар нетрудно. Но во всей этой стране нет газа, чтобы наполнить шар и заставить его плыть в воздухе».',
		},
		{
			sentence: '“If it won’t float,” remarked Dorothy, “it will be of no use to us.”',
			translate: '«Если он не будет держаться в воздухе, — заметила Дороти, — толку от него не будет».',
		},
		{
			sentence:
				'“True,” answered Oz. “But there is another way to make it float, which is to fill it with hot air. Hot air isn’t as good as gas, for if the air should get cold the balloon would come down in the desert, and we should be lost.”',
			translate:
				'«Верно, — ответил Оз. — Но есть другой способ — наполнить его горячим воздухом. Горячий воздух хуже газа: если он остынет, шар опустится в пустыню, и мы погибнем».',
		},
		{
			sentence: '“We!” exclaimed the girl. “Are you going with me?”',
			translate: '«Мы!» — воскликнула девочка. — «Ты пойдёшь со мной?»',
		},
		{
			sentence:
				'“Yes, of course,” replied Oz. “I am tired of being such a humbug. If I should go out of this Palace my people would soon discover I am not a Wizard, and then they would be vexed with me for having deceived them. So I have to stay shut up in these rooms all day, and it gets tiresome. I’d much rather go back to Kansas with you and be in a circus again.”',
			translate:
				'«Да, конечно, — ответил Оз. — Мне надоело быть таким обманщиком. Стоит мне выйти из Дворца — люди быстро поймут, что я не Волшебник, и будут сердиться за обман. Так что мне приходится сидеть тут целыми днями, и это утомляет. Куда охотнее я вернулся бы с тобой в Канзас и снова работал в цирке».',
		},
		{
			sentence: '“I shall be glad to have your company,” said Dorothy.',
			translate: '«Я буду рада твоей компании», — сказала Дороти.',
		},
		{
			sentence:
				'“Thank you,” he answered. “Now, if you will help me sew the silk together, we will begin to work on our balloon.”',
			translate:
				'«Спасибо, — ответил он. — А теперь, если поможешь мне сшить шёлк, мы начнём работу над нашим шаром».',
		},
		{
			sentence:
				'So Dorothy took a needle and thread, and as fast as Oz cut the strips of silk into proper shape the girl sewed them neatly together.',
			translate:
				'Дороти взяла иголку с ниткой, и по мере того как Оз кроил шёлковые полосы нужной формы, девочка аккуратно сшивала их.',
		},
		{
			sentence:
				'First there was a strip of light green silk, then a strip of dark green and then a strip of emerald green; for Oz had a fancy to make the balloon in different shades of the color about them.',
			translate:
				'Сначала шла полоска светло‑зелёного шёлка, затем тёмно‑зелёного, а потом изумрудного: Оз задумал сделать шар в разных оттенках окружающего их цвета.',
		},
		{
			sentence:
				'It took three days to sew all the strips together, but when it was finished they had a big bag of green silk more than twenty feet long.',
			translate:
				'Чтобы сшить все полосы, понадобилось три дня, но, когда закончили, у них получился большой мешок из зелёного шёлка длиной более двадцати футов.',
		},
		{
			sentence:
				'Then Oz painted it on the inside with a coat of thin glue, to make it airtight, after which he announced that the balloon was ready.',
			translate:
				'Потом Оз покрыл его изнутри слоем жидкого клея, чтобы он не пропускал воздух, и объявил, что шар готов.',
		},
		{
			sentence:
				'“But we must have a basket to ride in,” he said. So he sent the soldier with the green whiskers for a big clothes basket, which he fastened with many ropes to the bottom of the balloon.',
			translate:
				'«Но нам нужна корзина, чтобы ехать», — сказал он. Тогда он послал солдата с зелёными усами за большой бельевой корзиной и прикрепил её множеством верёвок к шарu.',
		},
		{
			sentence:
				'When it was all ready, Oz sent word to his people that he was going to make a visit to a great brother Wizard who lived in the clouds.',
			translate:
				'Когда всё было готово, Оз передал людям, что отправляется с визитом к великому брату‑Волшебнику, живущему в облаках.',
		},
		{
			sentence: 'The news spread rapidly throughout the city and everyone came to see the wonderful sight.',
			translate: 'Весть быстро разнеслась по городу, и все пришли посмотреть на это чудо.',
		},
		{
			sentence:
				'Oz ordered the balloon carried out in front of the Palace, and the people gazed upon it with much curiosity.',
			translate:
				'Оз велел вынести шар перед Дворцом, и люди разглядывали его с большим любопытством.',
		},
		{
			sentence:
				'The Tin Woodman had chopped a big pile of wood, and now he made a fire of it, and Oz held the bottom of the balloon over the fire so that the hot air that arose from it would be caught in the silken bag.',
			translate:
				'Железный Дровосек наколол большую кучу дров и развёл из них огонь, а Оз держал устье шара над пламенем, чтобы горячий воздух, поднимаясь, наполнял шёлковый мешок.',
		},
		{
			sentence: 'Gradually the balloon swelled out and rose into the air, until finally the basket just touched the ground.',
			translate: 'Постепенно шар надулся и поднялся в воздух, так что корзина лишь касалась земли.',
		},
		{
			sentence: 'Then Oz got into the basket and said to all the people in a loud voice:',
			translate: 'Тогда Оз забрался в корзину и громко сказал всем людям:',
		},
		{
			sentence: '“I am now going away to make a visit. While I am gone the Scarecrow will rule over you. I command you to obey him as you would me.”',
			translate: '«Сейчас я уезжаю с визитом. Пока меня не будет, вами будет править Пугало. Повинуйтесь ему, как мне».',
		},
		{
			sentence:
				'The balloon was by this time tugging hard at the rope that held it to the ground, for the air within it was hot, and this made it so much lighter in weight than the air without that it pulled hard to rise into the sky.',
			translate:
				'К тому времени шар уже сильно тянул верёвку, удерживавшую его на земле: внутри был горячий воздух, из‑за чего он стал гораздо легче окружающего и рвался в небо.',
		},
		{
			sentence: '“Come, Dorothy!” cried the Wizard. “Hurry up, or the balloon will fly away.”',
			translate: '«Идём, Дороти!» — крикнул Волшебник. — «Скорее, а то шар улетит!»',
		},
		{
			sentence:
				'“I can’t find Toto anywhere,” replied Dorothy, who did not wish to leave her little dog behind. Toto had run into the crowd to bark at a kitten, and Dorothy at last found him.',
			translate:
				'«Я нигде не могу найти Тото», — ответила Дороти, не желавшая оставлять свою собачку. Тото убежал в толпу лаять на котёнка, и Дороти наконец нашла его.',
		},
		{
			sentence: 'She picked him up and ran towards the balloon.',
			translate: 'Она подхватила его и побежала к шару.',
		},
		{
			sentence:
				'She was within a few steps of it, and Oz was holding out his hands to help her into the basket, when, crack! went the ropes, and the balloon rose into the air without her.',
			translate:
				'Она была в нескольких шагах, и Оз уже протягивал руки, чтобы помочь ей забраться в корзину, как вдруг — треск! — верёвки лопнули, и шар поднялся в воздух без неё.',
		},
		{
			sentence: '“Come back!” she screamed. “I want to go, too!”',
			translate: '«Вернись!» — закричала она. — «Я тоже хочу лететь!»',
		},
		{
			sentence: '“I can’t come back, my dear,” called Oz from the basket. “Good-bye!”',
			translate: '«Я не могу вернуться, дорогая», — крикнул из корзины Оз. — «Прощай!»',
		},
		{
			sentence:
				'“Good-bye!” shouted everyone, and all eyes were turned upward to where the Wizard was riding in the basket, rising every moment farther and farther into the sky.',
			translate:
				'«Прощай!» — закричали все, и взгляды обратились ввысь, туда, где Волшебник в корзине с каждой секундой поднимался всё выше и выше.',
		},
		{
			sentence:
				'And that was the last any of them ever saw of Oz, the Wonderful Wizard, though he may have reached Omaha safely, and be there now, for all we know.',
			translate:
				'И это было последнее, что кто‑либо из них видел Оза, Дивного Волшебника, хоть, возможно, он благополучно добрался до Омахи и, как знать, сейчас там.',
		},
		{
			sentence:
				'But the people remembered him lovingly, and said to one another:',
			translate:
				'Но люди вспоминали его с любовью и говорили друг другу:',
		},
		{
			sentence:
				'“Oz was always our friend. When he was here he built for us this beautiful Emerald City, and now he is gone he has left the Wise Scarecrow to rule over us.”',
			translate:
				'«Оз всегда был нашим другом. Когда он был здесь, он построил для нас этот прекрасный Изумрудный город, а теперь, когда его нет, он оставил Мудрое Пугало править нами».',
		},
		{
			sentence:
				'Still, for many days they grieved over the loss of the Wonderful Wizard, and would not be comforted.',
			translate:
				'И всё же многие дни они горевали об утрате Дивного Волшебника и никак не могли утешиться.',
		},
	],
}
