export const chapter_4 = {
	name: 'Chapter IV',
	header: 'The Road Through the Forest',
	data: [
		{
			sentence:
				'After a few hours the road began to be rough, and the walking grew so difficult that the Scarecrow often stumbled over the yellow bricks, which were here very uneven.',
			translate:
				'Через несколько часов дорога стала неровной, и идти было так трудно, что Пугало часто спотыкался о жёлтые кирпичи, которые здесь лежали очень неровно.',
		},
		{
			sentence:
				'Sometimes, indeed, they were broken or missing altogether, leaving holes that Toto jumped across and Dorothy walked around.',
			translate:
				'Иногда они и вовсе оказывались сломанными или отсутствовали, оставляя ямы, через которые Тото перепрыгивал, а Дороти обходила.',
		},
		{
			sentence:
				'As for the Scarecrow, having no brains, he walked straight ahead, and so stepped into the holes and fell at full length on the hard bricks.',
			translate:
				'Что до Пугала, то, не имея мозгов, он шёл прямо вперёд, поэтому наступал в ямы и растягивался во весь рост на твёрдых кирпичах.',
		},
		{
			sentence:
				'It never hurt him, however, and Dorothy would pick him up and set him upon his feet again, while he joined her in laughing merrily at his own mishap.',
			translate:
				'Однако ему это никогда не причиняло вреда, и Дороти поднимала его и снова ставила на ноги, а он вместе с ней весело смеялся над собственной неудачей.',
		},
		{
			sentence: 'The farms were not nearly so well cared for here as they were farther back.',
			translate: 'Здесь за фермами ухаживали куда хуже, чем раньше по пути.',
		},
		{
			sentence:
				'There were fewer houses and fewer fruit trees, and the farther they went the more dismal and lonesome the country became.',
			translate:
				'Домов и плодовых деревьев встречалось меньше, и чем дальше они шли, тем мрачнее и пустыннее становилась местность.',
		},
		{
			sentence:
				'At noon they sat down by the roadside, near a little brook, and Dorothy opened her basket and got out some bread.',
			translate:
				'В полдень они присели у обочины, возле небольшого ручья, и Дороти открыла корзинку и достала хлеб.',
		},
		{
			sentence: 'She offered a piece to the Scarecrow, but he refused.',
			translate: 'Она предложила кусочек Пугалу, но он отказался.',
		},
		{
			sentence:
				'“I am never hungry,” he said, “and it is a lucky thing I am not, for my mouth is only painted, and if I should cut a hole in it so I could eat, the straw I am stuffed with would come out, and that would spoil the shape of my head.” Dorothy saw at once that this was true, so she only nodded and went on eating her bread.',
			translate:
				'«Я никогда не бываю голоден, — сказал он, — и это к счастью, ведь мой рот только нарисован; если вырезать в нём дырку, чтобы я мог есть, солома, которой я набит, вылезет, и форма моей головы испортится». Дороти сразу поняла, что это правда, кивнула и продолжила есть свой хлеб.',
		},
		{
			sentence:
				'“Tell me something about yourself and the country you came from,” said the Scarecrow, when she had finished her dinner.',
			translate:
				'«Расскажи что‑нибудь о себе и о стране, откуда ты пришла», — сказал Пугало, когда она закончила обедать.',
		},
		{
			sentence:
				'So she told him all about Kansas, and how gray everything was there, and how the cyclone had carried her to this queer Land of Oz.',
			translate:
				'Тогда она рассказала ему всё о Канзасе, о том, какой там всё серый, и как циклон занёс её в эту странную Страну Оз.',
		},
		{
			sentence:
				'The Scarecrow listened carefully, and said, “I cannot understand why you should wish to leave this beautiful country and go back to the dry, gray place you call Kansas.” “That is because you have no brains” answered the girl.',
			translate:
				'Пугало внимательно слушал и сказал: «Не понимаю, почему ты хочешь покинуть такую прекрасную страну и вернуться в сухое, серое место, которое ты называешь Канзасом». — «Потому что у тебя нет мозгов», — ответила девочка.',
		},
		{
			sentence:
				'“No matter how dreary and gray our homes are, we people of flesh and blood would rather live there than in any other country, be it ever so beautiful.',
			translate:
				'«Как бы ни были унылы и серы наши дома, мы, люди из плоти и крови, предпочитаем жить именно там, а не в какой‑нибудь другой стране, пусть даже самой красивой.',
		},
		{
			sentence: 'There is no place like home.” The Scarecrow sighed.',
			translate: 'Нет места лучше дома», — вздохнул Пугало.',
		},
		{
			sentence: '“Of course I cannot understand it,” he said.',
			translate: '«Конечно, я не могу этого понять», — сказал он.',
		},
		{
			sentence:
				'“If your heads were stuffed with straw, like mine, you would probably all live in the beautiful places, and then Kansas would have no people at all.',
			translate:
				'«Если бы и ваши головы были набиты соломой, как моя, вы, вероятно, все жили бы в красивых местах, и тогда в Канзасе вовсе не осталось бы людей.',
		},
		{
			sentence:
				'It is fortunate for Kansas that you have brains.” “Won’t you tell me a story, while we are resting?” asked the child.',
			translate:
				'К счастью для Канзаса, у вас есть мозги». — «Не расскажешь ли мне историю, пока мы отдыхаем?» — попросил ребёнок.',
		},
		{
			sentence:
				'The Scarecrow looked at her reproachfully, and answered: “My life has been so short that I really know nothing whatever.',
			translate:
				'Пугало укоризненно посмотрел на неё и ответил: «Моя жизнь была столь короткой, что я действительно ничего не знаю.',
		},
		{
			sentence: 'I was only made day before yesterday.',
			translate: 'Меня сделали всего позавчера.',
		},
		{
			sentence: 'What happened in the world before that time is all unknown to me.',
			translate: 'Что происходило в мире до того, для меня совершенно неизвестно.',
		},
		{
			sentence:
				'Luckily, when the farmer made my head, one of the first things he did was to paint my ears, so that I heard what was going on.',
			translate:
				'К счастью, когда фермер делал мою голову, одним из первых дел он нарисовал мне уши, так что я слышал, что происходит вокруг.',
		},
		{
			sentence:
				'There was another Munchkin with him, and the first thing I heard was the farmer saying, ‘How do you like those ears?’ “‘They aren’t straight,’” answered the other.',
			translate:
				'С ним был другой мункин, и первое, что я услышал, — как фермер сказал: “Ну как тебе эти уши?” — “Они кривые”, — ответил другой.',
		},
		{
			sentence: '“‘Never mind,’” said the farmer.',
			translate: '“Неважно”, — сказал фермер.',
		},
		{
			sentence: '“‘They are ears just the same,’” which was true enough.',
			translate: '“Всё равно уши”, — и это было чистой правдой.',
		},
		{
			sentence: '“‘Now I’ll make the eyes,’” said the farmer.',
			translate: '“А теперь сделаю глаза”, — сказал фермер.',
		},
		{
			sentence:
				'So he painted my right eye, and as soon as it was finished I found myself looking at him and at everything around me with a great deal of curiosity, for this was my first glimpse of the world.',
			translate:
				'Он нарисовал мне правый глаз, и как только закончил, я обнаружил, что смотрю на него и на всё вокруг с большим любопытством — это был мой первый взгляд на мир.',
		},
		{
			sentence: '“‘That’s a rather pretty eye,’” remarked the Munchkin who was watching the farmer.',
			translate: '“Неплохой глаз”, — заметил мункин, наблюдавший за фермером.',
		},
		{
			sentence:
				'“‘Blue paint is just the color for eyes.’ “‘I think I’ll make the other a little bigger,’” said the farmer.',
			translate:
				'“Синяя краска — как раз цвет для глаз.” — “Думаю, другой сделаю чуть побольше”, — сказал фермер.',
		},
		{
			sentence: 'And when the second eye was done I could see much better than before.',
			translate: 'И когда второй глаз был готов, я стал видеть гораздо лучше, чем прежде.',
		},
		{
			sentence: 'Then he made my nose and my mouth.',
			translate: 'Потом он сделал мне нос и рот.',
		},
		{
			sentence: 'But I did not speak, because at that time I didn’t know what a mouth was for.',
			translate: 'Но я не заговорил, потому что тогда ещё не знал, для чего нужен рот.',
		},
		{
			sentence:
				'I had the fun of watching them make my body and my arms and legs; and when they fastened on my head, at last, I felt very proud, for I thought I was just as good a man as anyone.',
			translate:
				'Мне было забавно наблюдать, как они делают моё тело, руки и ноги; а когда, наконец, прикрепили голову, я почувствовал большую гордость, потому что подумал, что я ничуть не хуже любого человека.',
		},
		{
			sentence:
				'“‘This fellow will scare the crows fast enough,’ said the farmer. ‘He looks just like a man.’ “‘Why, he is a man,’ said the other, and I quite agreed with him.',
			translate:
				'“Этот парень быстро распугает ворон”, — сказал фермер. — “Выглядит прямо как человек.” — “Да он и есть человек”, — сказал другой, и я полностью с ним согласился.',
		},
		{
			sentence:
				'The farmer carried me under his arm to the cornfield, and set me up on a tall stick, where you found me.',
			translate: 'Фермер подмышкой отнёс меня на кукурузное поле и посадил на высокий шест, где вы меня и нашли.',
		},
		{
			sentence: 'He and his friend soon after walked away and left me alone.',
			translate: 'Вскоре он с товарищем ушли и оставили меня одного.',
		},
		{
			sentence: '“I did not like to be deserted this way.',
			translate: '«Мне не понравилось, что меня так бросили.',
		},
		{
			sentence: 'So I tried to walk after them.',
			translate: 'Поэтому я попытался пойти за ними.',
		},
		{
			sentence: 'But my feet would not touch the ground, and I was forced to stay on that pole.',
			translate: 'Но мои ноги не доставали до земли, и мне пришлось остаться на том шесте.',
		},
		{
			sentence:
				'It was a lonely life to lead, for I had nothing to think of, having been made such a little while before.',
			translate: 'Жизнь была одинокой, ведь мне не о чем было думать — меня сделали совсем недавно.',
		},
		{
			sentence:
				'Many crows and other birds flew into the cornfield, but as soon as they saw me they flew away again, thinking I was a Munchkin; and this pleased me and made me feel that I was quite an important person.',
			translate:
				'Многие вороны и другие птицы слетались на поле, но, завидев меня, снова улетали, думая, что я — мункин; и это радовало меня и заставляло чувствовать себя весьма важной персоной.',
		},
		{
			sentence:
				'By and by an old crow flew near me, and after looking at me carefully he perched upon my shoulder and said: “‘I wonder if that farmer thought to fool me in this clumsy manner.',
			translate:
				'Вскоре старый ворон подлетел ко мне, внимательно осмотрел и уселся на моё плечо, сказав: “Интересно, неужели этот фермер думал обмануть меня таким грубым способом.',
		},
		{
			sentence:
				'Any crow of sense could see that you are only stuffed with straw.’ Then he hopped down at my feet and ate all the corn he wanted.',
			translate:
				'Любой разумный ворон сразу поймёт, что ты всего лишь набит соломой”. Потом он спрыгнул к моим ногам и съел столько кукурузы, сколько хотел.',
		},
		{
			sentence:
				'The other birds, seeing he was not harmed by me, came to eat the corn too, so in a short time there was a great flock of them about me.',
			translate:
				'Другие птицы, увидев, что я его не тронул, тоже прилетели клевать кукурузу, и очень скоро вокруг меня собралось целое стадо.',
		},
		{
			sentence:
				'“I felt sad at this, for it showed I was not such a good Scarecrow after all; but the old crow comforted me, saying, ‘If you only had brains in your head you would be as good a man as any of them, and a better man than some of them.',
			translate:
				'«Мне стало грустно, потому что это показало — я не такое уж хорошее Пугало; но старый ворон утешил меня, сказав: “Если бы у тебя в голове были мозги, ты был бы не хуже любого из них, а лучше некоторых”.',
		},
		{
			sentence:
				'Brains are the only things worth having in this world, no matter whether one is a crow or a man.’ “After the crows had gone I thought this over, and decided I would try hard to get some brains.',
			translate:
				'Мозги — единственное, что стоит иметь в этом мире, неважно, ворон ты или человек». «Когда вороны улетели, я всё это обдумал и решил во что бы то ни стало раздобыть мозги.',
		},
		{
			sentence:
				'By good luck you came along and pulled me off the stake, and from what you say I am sure the Great Oz will give me brains as soon as we get to the Emerald City.” “I hope so,” said Dorothy earnestly, “since you seem anxious to have them.” “Oh, yes; I am anxious,” returned the Scarecrow.',
			translate:
				'К счастью, ты прошла мимо и сняла меня с кола, и по твоим словам я уверен, что Великий Оз даст мне мозги, как только мы доберёмся до Изумрудного города». — «Надеюсь, — искренне сказала Дороти, — раз уж тебе так хочется их получить». — «О да, очень хочется», — ответил Пугало.',
		},
		{
			sentence:
				'“It is such an uncomfortable feeling to know one is a fool.” “Well,” said the girl, “let us go.” And she handed the basket to the Scarecrow.',
			translate:
				'«Очень неприятно осознавать, что ты — дурак». — «Ну что ж, — сказала девочка, — пойдём». И она передала корзинку Пугалу.',
		},
		{
			sentence: 'There were no fences at all by the roadside now, and the land was rough and untilled.',
			translate: 'По обочинам дороги больше вовсе не было заборов, а земля стала бугристой и необработанной.',
		},
		{
			sentence:
				'Toward evening they came to a great forest, where the trees grew so big and close together that their branches met over the road of yellow brick.',
			translate:
				'К вечеру они подошли к большому лесу, где деревья росли такими большими и близкими рядами, что их ветви сходились над жёлтой кирпичной дорогой.',
		},
		{
			sentence:
				'It was almost dark under the trees, for the branches shut out the daylight; but the travelers did not stop, and went on into the forest.',
			translate:
				'Под деревьями было почти темно — ветви заслоняли дневной свет; но путники не остановились и вошли в лес.',
		},
		{
			sentence:
				'“If this road goes in, it must come out,” said the Scarecrow, “and as the Emerald City is at the other end of the road, we must go wherever it leads us.” “Anyone would know that,” said Dorothy.',
			translate:
				'«Если эта дорога куда‑то ведёт внутрь, значит, она и выведет», — сказал Пугало, — «а раз Изумрудный город на другом её конце, нам нужно идти туда, куда она ведёт». — «Это и так ясно каждому», — сказала Дороти.',
		},
		{
			sentence: '“Certainly; that is why I know it,” returned the Scarecrow.',
			translate: '«Разумеется; потому я это и знаю», — ответил Пугало.',
		},
		{
			sentence:
				'“If it required brains to figure it out, I never should have said it.” After an hour or so the light faded away, and they found themselves stumbling along in the darkness.',
			translate:
				'«Если бы, чтобы додуматься до этого, требовались мозги, я бы этого не сказал». Примерно через час свет совсем погас, и они стали спотыкаться в темноте.',
		},
		{
			sentence:
				'Dorothy could not see at all, but Toto could, for some dogs see very well in the dark; and the Scarecrow declared he could see as well as by day.',
			translate:
				'Дороти ничего не видела, а Тото — да: некоторые собаки отлично видят в темноте; Пугало же уверял, что видит так же хорошо, как днём.',
		},
		{
			sentence: 'So she took hold of his arm and managed to get along fairly well.',
			translate: 'Тогда она взяла его под руку, и им удалось идти на ощупь довольно сносно.',
		},
		{
			sentence:
				'“If you see any house, or any place where we can pass the night,” she said, “you must tell me; for it is very uncomfortable walking in the dark.” Soon after the Scarecrow stopped.',
			translate:
				'«Если увидишь какой‑нибудь дом или место, где можно переночевать, — сказала она, — скажи мне; идти в темноте очень неприятно». Вскоре Пугало остановился.',
		},
		{
			sentence: '“I see a little cottage at the right of us,” he said, “built of logs and branches.',
			translate: '«Я вижу справа от нас маленькую избушку, — сказал он, — из брёвен и веток.',
		},
		{
			sentence: 'Shall we go there?” “Yes, indeed,” answered the child.',
			translate: 'Пойдём туда?» — «Да, конечно», — ответила девочка.',
		},
		{
			sentence:
				'“I am all tired out.” So the Scarecrow led her through the trees until they reached the cottage, and Dorothy entered and found a bed of dried leaves in one corner.',
			translate:
				'«Я совсем выбилась из сил». Тогда Пугало провёл её сквозь деревья, и, когда они добрались до избушки, Дороти вошла и нашла в углу постель из сухих листьев.',
		},
		{
			sentence: 'She lay down at once, and with Toto beside her soon fell into a sound sleep.',
			translate: 'Она сразу легла и вскоре крепко заснула, Тото устроился рядом.',
		},
		{
			sentence:
				'The Scarecrow, who was never tired, stood up in another corner and waited patiently until morning came.',
			translate:
				'Пугало, который никогда не уставал, остался стоять в другом углу и терпеливо ждал наступления утра.',
		},
	],
}
