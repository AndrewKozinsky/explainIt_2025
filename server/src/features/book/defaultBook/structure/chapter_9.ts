export const chapter_9 = {
	name: 'Chapter IX',
	header: 'The Queen of the Field Mice',
	data: [
		{
			sentence:
				'“We cannot be far from the road of yellow brick, now,” remarked the Scarecrow, as he stood beside the girl, “for we have come nearly as far as the river carried us away.” The Tin Woodman was about to reply when he heard a low growl, and turning his head (which worked beautifully on hinges) he saw a strange beast come bounding over the grass toward them.',
			translate:
				'«Мы уже недалеко от жёлтой кирпичной дороги, — заметило Пугало, стоя рядом с девочкой, — ведь мы прошли почти столько же, насколько река унесла нас». Железный Дровосек уже собирался ответить, как услышал тихое рычание и, повернув голову (которая прекрасно вращалась на петлях), увидел странного зверя, несущегося к ним по траве.',
		},
		{
			sentence:
				'It was, indeed, a great yellow Wildcat, and the Woodman thought it must be chasing something, for its ears were lying close to its head and its mouth was wide open, showing two rows of ugly teeth, while its red eyes glowed like balls of fire.',
			translate:
				'Это и в самом деле была большая жёлтая рысь, и Дровосек решил, что она за кем-то гонится: её уши были прижаты к голове, пасть широко раскрыта, так что видны были два ряда уродливых зубов, а красные глаза сверкали, как огненные шары.',
		},
		{
			sentence:
				'As it came nearer the Tin Woodman saw that running before the beast was a little gray field mouse, and although he had no heart he knew it was wrong for the Wildcat to try to kill such a pretty, harmless creature.',
			translate:
				'Когда зверь приблизился, Железный Дровосек увидел, что перед ним бежит маленькая серая полёвка, и, хотя у него не было сердца, он понимал, что неправильно позволять рыси убить такое милое, безобидное создание.',
		},
		{
			sentence:
				'So the Woodman raised his axe, and as the Wildcat ran by he gave it a quick blow that cut the beast’s head clean off from its body, and it rolled over at his feet in two pieces.',
			translate:
				'Тогда Дровосек поднял топор, и, когда рысь пробежала мимо, нанёс быстрый удар, отрубив зверю голову начисто от тела, и та перекатилась к его ногам двумя частями.',
		},
		{
			sentence:
				'The field mouse, now that it was freed from its enemy, stopped short; and coming slowly up to the Woodman it said, in a squeaky little voice: “Oh, thank you!',
			translate:
				'Полёвка, избавившись от врага, резко остановилась; медленно подойдя к Дровосеку, она пропищала тоненьким голоском: «О, спасибо!',
		},
		{
			sentence:
				'Thank you ever so much for saving my life.” “Don’t speak of it, I beg of you,” replied the Woodman.',
			translate:
				'Огромное спасибо за то, что спасли мне жизнь». — «Не стоит об этом, умоляю вас», — ответил Дровосек.',
		},
		{
			sentence:
				'“I have no heart, you know, so I am careful to help all those who may need a friend, even if it happens to be only a mouse.” “Only a mouse!” cried the little animal, indignantly.',
			translate:
				'«У меня нет сердца, знаете, потому я особенно стараюсь помогать всем, кому может понадобиться друг, даже если это всего лишь мышка». — «Всего лишь мышка!» — возмущённо вскрикнуло маленькое животное.',
		},
		{
			sentence:
				'“Why, I am a Queen—the Queen of all the Field Mice!” “Oh, indeed,” said the Woodman, making a bow.',
			translate: '«Да я Королева — Королева всех полевых мышей!» — «Вот как», — сказал Дровосек, кланяясь.',
		},
		{
			sentence:
				'“Therefore you have done a great deed, as well as a brave one, in saving my life,” added the Queen.',
			translate:
				'«Значит, спасая мне жизнь, ты совершил не только смелый, но и великий поступок», — добавила Королева.',
		},
		{
			sentence:
				'At that moment several mice were seen running up as fast as their little legs could carry them, and when they saw their Queen they exclaimed: “Oh, your Majesty, we thought you would be killed!',
			translate:
				'В этот момент к ним подбежало несколько мышей, насколько только могли несясь на своих коротких лапках, и, увидев свою Королеву, они воскликнули: «О, Ваше Величество, мы думали, вас убьют!',
		},
		{
			sentence:
				'How did you manage to escape the great Wildcat?” They all bowed so low to the little Queen that they almost stood upon their heads.',
			translate:
				'Как вам удалось спастись от огромной рыси?» Они так низко поклонились маленькой Королеве, что едва не стали на головы.',
		},
		{
			sentence: '“This funny tin man,” she answered, “killed the Wildcat and saved my life.',
			translate: '«Этот забавный железный человек, — ответила она, — убил рысь и спас мне жизнь.',
		},
		{
			sentence:
				'So hereafter you must all serve him, and obey his slightest wish.” “We will!” cried all the mice, in a shrill chorus.',
			translate:
				'Поэтому отныне вы все должны служить ему и повиноваться малейшему его желанию». — «Будем!» — пронзительным хором пискнули все мыши.',
		},
		{
			sentence:
				'And then they scampered in all directions, for Toto had awakened from his sleep, and seeing all these mice around him he gave one bark of delight and jumped right into the middle of the group.',
			translate:
				'И тут же они врассыпную бросились во все стороны, потому что Тото проснулся и, увидев вокруг себя столько мышей, радостно тявкнул и прыгнул прямо в самую середину стайки.',
		},
		{
			sentence: 'Toto had always loved to chase mice when he lived in Kansas, and he saw no harm in it.',
			translate: 'В Канзасе Тото всегда любил гоняться за мышами и не видел в этом ничего плохого.',
		},
		{
			sentence:
				'But the Tin Woodman caught the dog in his arms and held him tight, while he called to the mice, “Come back!',
			translate:
				'Но Железный Дровосек схватил собаку на руки и крепко удержал её, а сам крикнул мышам: «Вернитесь!',
		},
		{
			sentence: 'Come back!',
			translate: 'Вернитесь!',
		},
		{
			sentence:
				'Toto shall not hurt you.” At this the Queen of the Mice stuck her head out from underneath a clump of grass and asked, in a timid voice, “Are you sure he will not bite us?” “I will not let him,” said the Woodman; “so do not be afraid.” One by one the mice came creeping back, and Toto did not bark again, although he tried to get out of the Woodman’s arms, and would have bitten him had he not known very well he was made of tin.',
			translate:
				'Тото не причинит вам вреда». Услышав это, Королева Мышей высунула головку из‑под кочки травы и робко спросила: «Вы уверены, что он нас не укусит?» — «Я не позволю ему», — сказал Дровосек. — «Так что не бойтесь». Одна за другой мыши поползли обратно, и Тото больше не лаял, хотя пытался вырваться из объятий Дровосека и, пожалуй, укусил бы его, если бы не знал очень хорошо, что тот сделан из жести.',
		},
		{
			sentence: 'Finally one of the biggest mice spoke.',
			translate: 'Наконец заговорила одна из самых больших мышей.',
		},
		{
			sentence:
				'“Is there anything we can do,” it asked, “to repay you for saving the life of our Queen?” “Nothing that I know of,” answered the Woodman; but the Scarecrow, who had been trying to think, but could not because his head was stuffed with straw, said, quickly, “Oh, yes; you can save our friend, the Cowardly Lion, who is asleep in the poppy bed.” “A Lion!” cried the little Queen.',
			translate:
				'«Есть ли что-нибудь, что мы можем сделать, — спросила она, — чтобы отплатить вам за спасение жизни нашей Королевы?» — «Ничего, что я знаю», — ответил Дровосек; но Пугало, которое пыталось думать, но не могло, потому что его голова была набита соломой, быстро сказало: «О да; вы можете спасти нашего друга, Трусливого Льва, который спит на маковом поле». — «Лев!» — вскрикнула маленькая Королева.',
		},
		{
			sentence:
				'“Why, he would eat us all up.” “Oh, no,” declared the Scarecrow; “this Lion is a coward.” “Really?” asked the Mouse.',
			translate:
				'«Да он же нас всех съест». — «О нет, — возразило Пугало, — этот Лев трус». — «Правда?» — спросила Мышь.',
		},
		{
			sentence:
				'“He says so himself,” answered the Scarecrow, “and he would never hurt anyone who is our friend.',
			translate:
				'«Он сам так говорит, — ответило Пугало, — и он никогда не причинит вреда тому, кто является нашим другом.',
		},
		{
			sentence:
				'If you will help us to save him I promise that he shall treat you all with kindness.” “Very well,” said the Queen, “we trust you.',
			translate:
				'Если вы поможете нам его спасти, я обещаю, что он будет относиться ко всем вам с добротой». — «Хорошо, — сказала Королева, — мы доверяем вам.',
		},
		{
			sentence:
				'But what shall we do?” “Are there many of these mice which call you Queen and are willing to obey you?” “Oh, yes; there are thousands,” she replied.',
			translate:
				'Но что же нам делать?» — «Много ли у вас мышей, которые признают вас Королевой и готовы повиноваться?» — «О да; их тысячи», — ответила она.',
		},
		{
			sentence:
				'“Then send for them all to come here as soon as possible, and let each one bring a long piece of string.” The Queen turned to the mice that attended her and told them to go at once and get all her people.',
			translate:
				'«Тогда пошлите за всеми, пусть придут сюда как можно скорее, и пусть каждая принесёт длинный кусок верёвки». Королева обратилась к сопровождавшим её мышам и велела им немедленно собрать весь её народ.',
		},
		{
			sentence: 'As soon as they heard her orders they ran away in every direction as fast as possible.',
			translate: 'Едва услышав приказ, они бросились во все стороны так быстро, как только могли.',
		},
		{
			sentence:
				'“Now,” said the Scarecrow to the Tin Woodman, “you must go to those trees by the riverside and make a truck that will carry the Lion.” So the Woodman went at once to the trees and began to work; and he soon made a truck out of the limbs of trees, from which he chopped away all the leaves and branches.',
			translate:
				'«А теперь, — сказал Пугало Железному Дровосеку, — ты должен пойти к тем деревьям у реки и сделать тележку, на которой можно будет везти Льва». Дровосек тотчас отправился к деревьям и принялся за работу; вскоре он сделал тележку из сучьев, с которых обрубил все листья и ветки.',
		},
		{
			sentence:
				'He fastened it together with wooden pegs and made the four wheels out of short pieces of a big tree trunk.',
			translate:
				'Он скрепил её деревянными колышками, а четыре колеса сделал из коротких отрезков толстого ствола дерева.',
		},
		{
			sentence:
				'So fast and so well did he work that by the time the mice began to arrive the truck was all ready for them.',
			translate:
				'Он работал так быстро и так умело, что к тому времени, когда мыши начали прибывать, тележка была уже полностью готова.',
		},
		{
			sentence:
				'They came from all directions, and there were thousands of them: big mice and little mice and middle-sized mice; and each one brought a piece of string in his mouth.',
			translate:
				'Они приходили со всех сторон, и их были тысячи: большие, маленькие и средние мыши; и каждая несла в зубах кусочек верёвки.',
		},
		{
			sentence: 'It was about this time that Dorothy woke from her long sleep and opened her eyes.',
			translate: 'Примерно в это время Дороти проснулась после долгого сна и открыла глаза.',
		},
		{
			sentence:
				'She was greatly astonished to find herself lying upon the grass, with thousands of mice standing around and looking at her timidly.',
			translate:
				'Она была крайне поражена, обнаружив, что лежит на траве, а вокруг, робко на неё поглядывая, стоят тысячи мышей.',
		},
		{
			sentence:
				'But the Scarecrow told her about everything, and turning to the dignified little Mouse, he said: “Permit me to introduce to you her Majesty, the Queen.” Dorothy nodded gravely and the Queen made a curtsy, after which she became quite friendly with the little girl.',
			translate:
				'Но Пугало всё ей объяснило и, повернувшись к важной маленькой Мыши, сказало: «Позволь представить тебе её Величество, Королеву». Дороти серьёзно кивнула, а Королева присела в реверансе, после чего они обе очень расположились друг к другу.',
		},
		{
			sentence:
				'The Scarecrow and the Woodman now began to fasten the mice to the truck, using the strings they had brought.',
			translate:
				'Пугало и Дровосек теперь принялись запрягать мышей в тележку, используя принесённые ими верёвочки.',
		},
		{
			sentence: 'One end of a string was tied around the neck of each mouse and the other end to the truck.',
			translate: 'Один конец верёвочки привязали к шее каждой мыши, а другой — к тележке.',
		},
		{
			sentence:
				'Of course the truck was a thousand times bigger than any of the mice who were to draw it; but when all the mice had been harnessed, they were able to pull it quite easily.',
			translate:
				'Разумеется, тележка была в тысячу раз больше любой из мышей, которые должны были её тащить; но когда всех мышей запрягли, они смогли довольно легко её тянуть.',
		},
		{
			sentence:
				'Even the Scarecrow and the Tin Woodman could sit on it, and were drawn swiftly by their queer little horses to the place where the Lion lay asleep.',
			translate:
				'На неё могли даже сесть Пугало и Железный Дровосек, и их быстро повезли эти странные маленькие лошадки туда, где лежал спящий Лев.',
		},
		{
			sentence:
				'After a great deal of hard work, for the Lion was heavy, they managed to get him up on the truck.',
			translate: 'После больших усилий — ведь Лев был тяжёл — им удалось взгромоздить его на тележку.',
		},
		{
			sentence:
				'Then the Queen hurriedly gave her people the order to start, for she feared if the mice stayed among the poppies too long they also would fall asleep.',
			translate:
				'Тогда Королева поспешно отдала своим подданным приказ выступать, потому что боялась: если мыши задержатся среди маков слишком долго, они тоже уснут.',
		},
		{
			sentence:
				'At first the little creatures, many though they were, could hardly stir the heavily loaded truck; but the Woodman and the Scarecrow both pushed from behind, and they got along better.',
			translate:
				'Сначала эти крошечные существа, сколько бы их ни было, едва могли сдвинуть тяжело нагруженную тележку; но Дровосек и Пугало упёрлись сзади, и дело пошло лучше.',
		},
		{
			sentence:
				'Soon they rolled the Lion out of the poppy bed to the green fields, where he could breathe the sweet, fresh air again, instead of the poisonous scent of the flowers.',
			translate:
				'Вскоре они вывезли Льва из макового поля на зелёные луга, где он снова мог дышать сладким свежим воздухом вместо ядовитого запаха цветов.',
		},
		{
			sentence:
				'Dorothy came to meet them and thanked the little mice warmly for saving her companion from death.',
			translate:
				'Дороти вышла им навстречу и горячо поблагодарила маленьких мышек за то, что спасли её спутника от смерти.',
		},
		{
			sentence: 'She had grown so fond of the big Lion she was glad he had been rescued.',
			translate: 'Большой Лев стал ей так дорог, что она была очень рада его спасению.',
		},
		{
			sentence:
				'Then the mice were unharnessed from the truck and scampered away through the grass to their homes.',
			translate: 'Потом мышек отпрягли от тележки, и они стрелой разбежались по траве по домам.',
		},
		{
			sentence: 'The Queen of the Mice was the last to leave.',
			translate: 'Последней ушла Королева Мышей.',
		},
		{
			sentence:
				'“If ever you need us again,” she said, “come out into the field and call, and we shall hear you and come to your assistance.',
			translate:
				'«Если когда‑нибудь мы снова понадобимся вам, — сказала она, — выйдите в поле и позовите, и мы услышим вас и придём на помощь.',
		},
		{
			sentence:
				'Good-bye!” “Good-bye!” they all answered, and away the Queen ran, while Dorothy held Toto tightly lest he should run after her and frighten her.',
			translate:
				'Прощайте!» — «Прощайте!» — ответили все, и Королева умчалась, а Дороти крепко держала Тото, чтобы он не бросился за ней и не напугал её.',
		},
		{
			sentence:
				'After this they sat down beside the Lion until he should awaken; and the Scarecrow brought Dorothy some fruit from a tree near by, which she ate for her dinner.',
			translate:
				'После этого они уселись рядом со Львом и ждали, пока он проснётся; Пугало принесло Дороти несколько фруктов с ближайшего дерева, и она съела их на обед.',
		},
	],
}
