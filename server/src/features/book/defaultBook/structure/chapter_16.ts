export const chapter_16 = {
	name: 'Chapter XVI',
	header: 'The Magic Art of the Great Humbug',
	data: [
		{
			sentence: 'Next morning the Scarecrow said to his friends:',
			translate: 'На следующее утро Пугало сказал своим друзьям:',
		},
		{
			sentence:
				'“Congratulate me. I am going to Oz to get my brains at last. When I return I shall be as other men are.”',
			translate: '«Поздравьте меня. Я иду к Озу за мозгами, наконец. Когда вернусь, буду как прочие люди».',
		},
		{
			sentence: '“I have always liked you as you were,” said Dorothy simply.',
			translate: '«Ты мне всегда нравился таким, какой ты есть», — просто сказала Дороти.',
		},
		{
			sentence:
				'“It is kind of you to like a Scarecrow,” he replied. “But surely you will think more of me when you hear the splendid thoughts my new brain is going to turn out.”',
			translate:
				'«Это очень мило — любить Пугало, — ответил он. — Но ты наверняка будешь думать обо мне лучше, когда услышишь великолепные мысли, которые породит мой новый мозг».',
		},
		{
			sentence:
				'Then he said good-bye to them all in a cheerful voice and went to the Throne Room, where he rapped upon the door.',
			translate: 'Затем он весело попрощался со всеми и пошёл в Тронный зал, где постучал в дверь.',
		},
		{ sentence: '“Come in,” said Oz.', translate: '«Войдите», — сказал Оз.' },
		{
			sentence:
				'The Scarecrow went in and found the little man sitting down by the window, engaged in deep thought.',
			translate: 'Пугало вошёл и увидел маленького человека у окна, погружённого в глубокие раздумья.',
		},
		{
			sentence: '“I have come for my brains,” remarked the Scarecrow, a little uneasily.',
			translate: '«Я пришёл за своими мозгами», — несколько тревожно заметил Пугало.',
		},
		{
			sentence:
				'“Oh, yes; sit down in that chair, please,” replied Oz. “You must excuse me for taking your head off, but I shall have to do it in order to put your brains in their proper place.”',
			translate:
				'«О да; сядь вон на тот стул, пожалуйста, — ответил Оз. — Прости, что придётся снять тебе голову, но мне нужно это, чтобы поместить мозги на положенное место».',
		},
		{
			sentence:
				'“That’s all right,” said the Scarecrow. “You are quite welcome to take my head off, as long as it will be a better one when you put it on again.”',
			translate:
				'«Ничего страшного, — сказал Пугало. — Можешь снять мою голову, раз уж вернёшь её лучше, чем была».',
		},
		{
			sentence:
				'So the Wizard unfastened his head and emptied out the straw. Then he entered the back room and took up a measure of bran, which he mixed with a great many pins and needles.',
			translate:
				'Тогда Волшебник отстегнул ему голову и вытряхнул солому. Затем он прошёл в заднюю комнату и взял меру отрубей, которые смешал с множеством булавок и иголок.',
		},
		{
			sentence:
				'Having shaken them together thoroughly, he filled the top of the Scarecrow’s head with the mixture and stuffed the rest of the space with straw, to hold it in place.',
			translate:
				'Как следует встряхнув смесь, он заполнил ею верх головы Пугала, а остальное пространство набил соломой, чтобы всё держалось на месте.',
		},
		{
			sentence:
				'When he had fastened the Scarecrow’s head on his body again he said to him, “Hereafter you will be a great man, for I have given you a lot of bran-new brains.”',
			translate:
				'Крепко закрепив голову Пугала на теле, он сказал: «Отныне ты будешь великим человеком, ведь я дал тебе массу отрубных новёхоньких мозгов».',
		},
		{
			sentence:
				'The Scarecrow was both pleased and proud at the fulfillment of his greatest wish, and having thanked Oz warmly he went back to his friends.',
			translate:
				'Пугало был и рад, и горд исполнением своего заветного желания, тепло поблагодарил Оза и вернулся к друзьям.',
		},
		{
			sentence: 'Dorothy looked at him curiously. His head was quite bulged out at the top with brains.',
			translate: 'Дороти с любопытством посмотрела на него. Его голова заметно вздулась сверху от мозгов.',
		},
		{ sentence: '“How do you feel?” she asked.', translate: '«Как ты себя чувствуешь?» — спросила она.' },
		{
			sentence:
				'“I feel wise indeed,” he answered earnestly. “When I get used to my brains I shall know everything.”',
			translate:
				'«Чувствую себя по‑настоящему мудрым, — серьёзно ответил он. — Когда привыкну к мозгам, буду знать всё».',
		},
		{
			sentence: '“Why are those needles and pins sticking out of your head?” asked the Tin Woodman.',
			translate: '«Почему из твоей головы торчат иголки и булавки?» — спросил Железный Дровосек.',
		},
		{
			sentence: '“That is proof that he is sharp,” remarked the Lion.',
			translate: '«Это доказательство того, что он остроумен», — заметил Лев.',
		},
		{
			sentence: '“Well, I must go to Oz and get my heart,” said the Woodman.',
			translate: '«Ну, а мне пора к Озy за сердцем», — сказал Дровосек.',
		},
		{
			sentence: 'So he walked to the Throne Room and knocked at the door.',
			translate: 'Он направился в Тронный зал и постучал в дверь.',
		},
		{
			sentence: '“Come in,” called Oz, and the Woodman entered and said, “I have come for my heart.”',
			translate: '«Войдите», — позвал Оз, и Дровосек вошёл, сказав: «Я пришёл за своим сердцем».',
		},
		{
			sentence:
				'“Very well,” answered the little man. “But I shall have to cut a hole in your breast, so I can put your heart in the right place. I hope it won’t hurt you.”',
			translate:
				'«Хорошо, — ответил маленький человек. — Но мне придётся вырезать отверстие в твоей груди, чтобы поместить сердце на место. Надеюсь, тебе не будет больно».',
		},
		{
			sentence: '“Oh, no,” answered the Woodman. “I shall not feel it at all.”',
			translate: '«О нет, — ответил Дровосек. — Я ничего не почувствую».',
		},
		{
			sentence:
				'So Oz brought a pair of tinsmith’s shears and cut a small, square hole in the left side of the Tin Woodman’s breast.',
			translate:
				'Тогда Оз принёс ножницы жестянщика и вырезал небольшое квадратное отверстие в левой стороне груди Железного Дровосека.',
		},
		{
			sentence:
				'Then, going to a chest of drawers, he took out a pretty heart, made entirely of silk and stuffed with sawdust.',
			translate:
				'Затем он подошёл к комоду и достал красивое сердце, полностью сделанное из шёлка и набитое опилками.',
		},
		{ sentence: '“Isn’t it a beauty?” he asked.', translate: '«Разве оно не прекрасно?» — спросил он.' },
		{
			sentence: '“It is, indeed!” replied the Woodman, who was greatly pleased. “But is it a kind heart?”',
			translate: '«Ещё как!» — ответил Дровосек, очень довольный. — «Но доброе ли оно?»',
		},
		{ sentence: '“Oh, very!” answered Oz.', translate: '«О, очень доброе», — ответил Оз.' },
		{
			sentence:
				'He put the heart in the Woodman’s breast and then replaced the square of tin, soldering it neatly together where it had been cut.',
			translate:
				'Он вложил сердце в грудь Дровосека, затем поставил на место пластину из олова и аккуратно припаял по месту разреза.',
		},
		{
			sentence: '“There,” said he; “now you have a heart that any man might be proud of.',
			translate: '«Вот так, — сказал он. — Теперь у тебя сердце, которым любой мог бы гордиться.',
		},
		{
			sentence: 'I’m sorry I had to put a patch on your breast, but it really couldn’t be helped.”',
			translate: 'Жаль, что пришлось оставить заплату на груди, но иначе было нельзя».',
		},
		{
			sentence:
				'“Never mind the patch,” exclaimed the happy Woodman. “I am very grateful to you, and shall never forget your kindness.”',
			translate:
				'«Пустяки, — воскликнул счастливый Дровосек. — Я очень благодарен тебе и никогда не забуду твоей доброты».',
		},
		{ sentence: '“Don’t speak of it,” replied Oz.', translate: '«Не стоит благодарности», — ответил Оз.' },
		{
			sentence:
				'Then the Tin Woodman went back to his friends, who wished him every joy on account of his good fortune.',
			translate: 'Железный Дровосек вернулся к друзьям, и они от всей души поздравили его с удачей.',
		},
		{
			sentence: 'The Lion now walked to the Throne Room and knocked at the door.',
			translate: 'Теперь к Тронному залу подошёл Лев и постучал.',
		},
		{ sentence: '“Come in,” said Oz.', translate: '«Войдите», — сказал Оз.' },
		{
			sentence: '“I have come for my courage,” announced the Lion, entering the room.',
			translate: '«Я пришёл за своей храбростью», — объявил Лев, входя в комнату.',
		},
		{
			sentence: '“Very well,” answered the little man; “I will get it for you.”',
			translate: '«Очень хорошо, — ответил маленький человек. — Сейчас принесу».',
		},
		{
			sentence:
				'He went to a cupboard and reaching up to a high shelf took down a square green bottle, the contents of which he poured into a green-gold dish, beautifully carved.',
			translate:
				'Он подошёл к шкафу, снял с высокой полки квадратную зелёную бутылочку и вылил её содержимое в красиво вырезанную зелёно‑золотую чашу.',
		},
		{
			sentence:
				'Placing this before the Cowardly Lion, who sniffed at it as if he did not like it, the Wizard said:',
			translate:
				'Поставив сосуд перед Трусливым Львом, который понюхал его, будто ему это не понравилось, Волшебник сказал:',
		},
		{ sentence: '“Drink.”', translate: '«Пей».' },
		{ sentence: '“What is it?” asked the Lion.', translate: '«Что это?» — спросил Лев.' },
		{
			sentence:
				'“Well,” answered Oz, “if it were inside of you, it would be courage. You know, of course, that courage is always inside one; so that this really cannot be called courage until you have swallowed it. Therefore I advise you to drink it as soon as possible.”',
			translate:
				'«Видишь ли, — ответил Оз, — окажись это внутри тебя — это была бы храбрость. Ты ведь знаешь, что храбрость всегда внутри; так что пока ты это не проглотишь, назвать это храбростью нельзя. Поэтому советую выпить как можно скорее».',
		},
		{
			sentence: 'The Lion hesitated no longer, but drank till the dish was empty.',
			translate: 'Лев больше не колебался и пил, пока чаша не опустела.',
		},
		{ sentence: '“How do you feel now?” asked Oz.', translate: '«Как теперь себя чувствуешь?» — спросил Оз.' },
		{
			sentence:
				'“Full of courage,” replied the Lion, who went joyfully back to his friends to tell them of his good fortune.',
			translate: '«Полон храбрости», — ответил Лев и радостно отправился к друзьям сообщить им о своей удаче.',
		},
		{
			sentence:
				'Oz, left to himself, smiled to think of his success in giving the Scarecrow and the Tin Woodman and the Lion exactly what they thought they wanted.',
			translate:
				'Оставшись один, Оз улыбнулся, вспомнив, как удачно он дал Пугалу, Дровосеку и Льву именно то, что, по их мнению, им было нужно.',
		},
		{
			sentence:
				'“How can I help being a humbug,” he said, “when all these people make me do things that everybody knows can’t be done?',
			translate:
				'«Как же мне не быть шарлатаном, — сказал он, — когда все эти люди заставляют меня делать то, что, как всем известно, сделать нельзя?',
		},
		{
			sentence:
				'It was easy to make the Scarecrow and the Lion and the Woodman happy, because they imagined I could do anything.',
			translate:
				'Сделать Пугало, Льва и Дровосека счастливыми было легко, потому что они воображали, будто я могу всё.',
		},
		{
			sentence:
				'But it will take more than imagination to carry Dorothy back to Kansas, and I’m sure I don’t know how it can be done.”',
			translate:
				'Но чтобы вернуть Дороти в Канзас, одной фантазии мало, и я совершенно не знаю, как это можно сделать».',
		},
	],
}
