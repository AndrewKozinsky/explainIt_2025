export const chapter_20 = {
	name: 'Chapter XX',
	header: 'The Dainty China Country',
	data: [
		{
			sentence:
				'While the Woodman was making a ladder from wood which he found in the forest Dorothy lay down and slept, for she was tired by the long walk.',
			translate:
				'Пока Дровосек делал лестницу из дерева, которое нашёл в лесу, Дороти легла и уснула, потому что устала от долгой ходьбы.',
		},
		{
			sentence: 'The Lion also curled himself up to sleep and Toto lay beside him.',
			translate: 'Лев тоже свернулся калачиком, чтобы поспать, и Тото лёг рядом с ним.',
		},
		{
			sentence: 'The Scarecrow watched the Woodman while he worked, and said to him:',
			translate: 'Пугало наблюдало за Дровосеком, пока тот работал, и сказало ему:',
		},
		{
			sentence: '“I cannot think why this wall is here, nor what it is made of.”',
			translate: '«Не могу понять, зачем здесь эта стена и из чего она сделана.»',
		},
		{
			sentence: '“Rest your brains and do not worry about the wall,” replied the Woodman.',
			translate: '«Дай мозгам отдых и не думай о стене», — ответил Дровосек.',
		},
		{
			sentence: '“When we have climbed over it, we shall know what is on the other side.”',
			translate: '«Когда перелезем через неё, узнаем, что по ту сторону.»',
		},
		{
			sentence: 'After a time the ladder was finished.',
			translate: 'Через некоторое время лестница была готова.',
		},
		{
			sentence: 'It looked clumsy, but the Tin Woodman was sure it was strong and would answer their purpose.',
			translate: 'Она выглядела неуклюже, но Железный Дровосек был уверен, что она крепкая и послужит их цели.',
		},
		{
			sentence:
				'The Scarecrow waked Dorothy and the Lion and Toto, and told them that the ladder was ready.',
			translate:
				'Пугало разбудило Дороти, Льва и Тото и сказало им, что лестница готова.',
		},
		{
			sentence:
				'The Scarecrow climbed up the ladder first, but he was so awkward that Dorothy had to follow close behind and keep him from falling off.',
			translate:
				'Пугало первым взобралось по лестнице, но он был так неуклюж, что Дороти пришлось идти прямо за ним и не дать ему сорваться.',
		},
		{
			sentence:
				'When he got his head over the top of the wall the Scarecrow said, “Oh, my!”',
			translate:
				'Когда он высунул голову над верхом стены, Пугало сказал: «Ох, боже!»',
		},
		{
			sentence: '“Go on,” exclaimed Dorothy.',
			translate: '«Продолжай», — воскликнула Дороти.',
		},
		{
			sentence:
				'So the Scarecrow climbed farther up and sat down on the top of the wall, and Dorothy put her head over and cried, “Oh, my!” just as the Scarecrow had done.',
			translate:
				'Тогда Пугало поднялся выше и сел на вершине стены, а Дороти высунулась и вскрикнула: «Ох, боже!», точно как Пугало.',
		},
		{
			sentence:
				'Then Toto came up, and immediately began to bark, but Dorothy made him be still.',
			translate:
				'Затем поднялся Тото и сразу начал лаять, но Дороти заставила его замолчать.',
		},
		{
			sentence:
				'The Lion climbed the ladder next, and the Tin Woodman came last; but both of them cried, “Oh, my!” as soon as they looked over the wall.',
			translate:
				'Следом взобрался Лев, а последним пришёл Железный Дровосек; но оба воскликнули «Ох, боже!», как только выглянули за стену.',
		},
		{
			sentence:
				'When they were all sitting in a row on the top of the wall, they looked down and saw a strange sight.',
			translate:
				'Когда они все сидели рядком на вершине стены, они посмотрели вниз и увидели странную картину.',
		},
		{
			sentence:
				'Before them was a great stretch of country having a floor as smooth and shining and white as the bottom of a big platter.',
			translate:
				'Перед ними простиралась огромная страна с поверхностью такой же гладкой, блестящей и белой, как дно большого блюда.',
		},
		{
			sentence:
				'Scattered around were many houses made entirely of china and painted in the brightest colors.',
			translate:
				'Вокруг были разбросаны множество домиков, полностью сделанных из фарфора и раскрашенных в самые яркие цвета.',
		},
		{
			sentence:
				'These houses were quite small, the biggest of them reaching only as high as Dorothy’s waist.',
			translate:
				'Эти домики были совсем маленькими, самые большие доходили Дороти лишь до пояса.',
		},
		{
			sentence:
				'There were also pretty little barns, with china fences around them; and many cows and sheep and horses and pigs and chickens, all made of china, were standing about in groups.',
			translate:
				'Там были и милые маленькие амбары с фарфоровыми заборами; а ещё множество коров, овец, лошадей, свиней и кур, все фарфоровые, стояли группами.',
		},
		{
			sentence: 'But the strangest of all were the people who lived in this queer country.',
			translate: 'Но страннее всего были люди, жившие в этой диковинной стране.',
		},
		{
			sentence:
				'There were milkmaids and shepherdesses, with brightly colored bodices and golden spots all over their gowns; and princesses with most gorgeous frocks of silver and gold and purple; and shepherds dressed in knee breeches with pink and yellow and blue stripes down them, and golden buckles on their shoes; and princes with jeweled crowns upon their heads, wearing ermine robes and satin doublets; and funny clowns in ruffled gowns, with round red spots upon their cheeks and tall, pointed caps.',
			translate:
				'Там были доярки и пастушки в ярких корсажах и с золотыми пятнами на платьях; принцессы в великолепных нарядах из серебра, золота и пурпура; пастухи в бриджах с розовыми, жёлтыми и голубыми полосками и с золотыми пряжками на туфлях; принцы с усыпанными драгоценностями коронами на головах, в ермолковых мантиях и атласных камзолах; и забавные клоуны в платьях с оборками, с круглыми красными пятнами на щёках и в высоких заострённых колпаках.',
		},
		{
			sentence:
				'And, strangest of all, these people were all made of china, even to their clothes, and were so small that the tallest of them was no higher than Dorothy’s knee.',
			translate:
				'И, что самое странное, все эти люди были сделаны из фарфора, даже их одежда, и были настолько малы, что самые высокие едва доставали Дороти до колена.',
		},
		{
			sentence:
				'No one did so much as look at the travelers at first, except one little purple china dog with an extra-large head, which came to the wall and barked at them in a tiny voice, afterwards running away again.',
			translate:
				'Поначалу никто даже не посмотрел на путешественников, кроме одной маленькой лиловой фарфоровой собачки с необычно большой головой, которая подошла к стене, тоненьким голоском облаяла их и снова убежала.',
		},
		{
			sentence: '“How shall we get down?” asked Dorothy.',
			translate: '«Как же нам спуститься?» — спросила Дороти.',
		},
		{
			sentence:
				'They found the ladder so heavy they could not pull it up, so the Scarecrow fell off the wall and the others jumped down upon him so that the hard floor would not hurt their feet.',
			translate:
				'Они обнаружили, что лестница слишком тяжёлая, чтобы подтянуть её, поэтому Пугало свалился со стены, а остальные спрыгнули на него, чтобы твёрдая поверхность не повредила им ноги.',
		},
		{
			sentence: 'Of course they took pains not to light on his head and get the pins in their feet.',
			translate: 'Разумеется, они постарались не приземляться на его голову и не уколоть ноги булавками.',
		},
		{
			sentence:
				'When all were safely down they picked up the Scarecrow, whose body was quite flattened out, and patted his straw into shape again.',
			translate:
				'Когда все благополучно спустились, они подняли Пугало, тело которого совсем сплющилось, и расправили его солому, вернув ему форму.',
		},
		{
			sentence:
				'“We must cross this strange place in order to get to the other side,” said Dorothy, “for it would be unwise for us to go any other way except due South.”',
			translate:
				'«Мы должны пересечь это странное место, чтобы попасть на другую сторону, — сказала Дороти, — ведь было бы неразумно идти куда‑либо ещё, кроме как строго на юг.»',
		},
		{
			sentence:
				'They began walking through the country of the china people, and the first thing they came to was a china milkmaid milking a china cow.',
			translate:
				'Они пошли по стране фарфоровых людей, и первое, что им встретилось, — фарфоровая доярка, доившая фарфоровую корову.',
		},
		{
			sentence:
				'As they drew near, the cow suddenly gave a kick and kicked over the stool, the pail, and even the milkmaid herself, and all fell on the china ground with a great clatter.',
			translate:
				'Когда они подошли ближе, корова вдруг ударила ногой и опрокинула табурет, вёдро и даже саму доярку, и всё с громким грохотом рухнуло на фарфоровую землю.',
		},
		{
			sentence:
				'Dorothy was shocked to see that the cow had broken her leg off, and that the pail was lying in several small pieces, while the poor milkmaid had a nick in her left elbow.',
			translate:
				'Дороти была потрясена, увидев, что корова сломала ногу, вёдро разбилось на несколько кусочков, а у бедной доярки откололся кусочек на левом локте.',
		},
		{
			sentence: '“There!” cried the milkmaid angrily.',
			translate: '«Вот!» — сердито крикнула доярка.',
		},
		{
			sentence:
				'“See what you have done! My cow has broken her leg, and I must take her to the mender’s shop and have it glued on again.',
			translate:
				'«Посмотрите, что вы наделали! Моя корова сломала ногу, и мне придётся отнести её к мастеру, чтобы он снова её приклеил.',
		},
		{
			sentence: 'What do you mean by coming here and frightening my cow?”',
			translate: 'Как вы посмели прийти сюда и пугать мою корову?»',
		},
		{
			sentence: '“I’m very sorry,” returned Dorothy.',
			translate: '«Мне очень жаль», — ответила Дороти.',
		},
		{
			sentence: '“Please forgive us.”',
			translate: '«Пожалуйста, простите нас.»',
		},
		{
			sentence: 'But the pretty milkmaid was much too vexed to make any answer.',
			translate: 'Но красивая доярка была слишком раздосадована, чтобы отвечать.',
		},
		{
			sentence:
				'She picked up the leg sulkily and led her cow away, the poor animal limping on three legs.',
			translate:
				'Она сердито подняла ногу и увела свою корову, бедное животное хромало на трёх ногах.',
		},
		{
			sentence:
				'As she left them the milkmaid cast many reproachful glances over her shoulder at the clumsy strangers, holding her nicked elbow close to her side.',
			translate:
				'Уходя, доярка бросала через плечо множество укоризненных взглядов на неуклюжих незнакомцев, прижимая к боку свой надколотый локоть.',
		},
		{
			sentence: 'Dorothy was quite grieved at this mishap.',
			translate: 'Дороти очень огорчилась из‑за этой неприятности.',
		},
		{
			sentence:
				'“We must be very careful here,” said the kind-hearted Woodman, “or we may hurt these pretty little people so they will never get over it.”',
			translate:
				'«Здесь нужно быть очень осторожными, — сказал добросердечный Дровосек, — иначе мы можем ранить этих милых крошечных людей так, что они никогда не оправятся.»',
		},
		{
			sentence:
				'A little farther on Dorothy met a most beautifully dressed young Princess, who stopped short as she saw the strangers and started to run away.',
			translate:
				'Чуть дальше Дороти встретила молодую Принцессу в необычайно красивом наряде, которая, увидев незнакомцев, резко остановилась и собралась бежать.',
		},
		{
			sentence: 'Dorothy wanted to see more of the Princess, so she ran after her.',
			translate: 'Дороти захотела получше рассмотреть Принцессу и побежала за ней.',
		},
		{
			sentence: 'But the china girl cried out:',
			translate: 'Но фарфоровая девочка вскрикнула:',
		},
		{
			sentence: '“Don’t chase me! Don’t chase me!”',
			translate: '«Не гоняйся за мной! Не гоняйся за мной!»',
		},
		{
			sentence:
				'She had such a frightened little voice that Dorothy stopped and said, “Why not?”',
			translate:
				'Её голосок был таким испуганным, что Дороти остановилась и спросила: «Почему?»',
		},
		{
			sentence:
				'“Because,” answered the Princess, also stopping, a safe distance away, “if I run I may fall down and break myself.”',
			translate:
				'«Потому что, — ответила Принцесса, также остановившись на безопасном расстоянии, — если я побегу, то могу упасть и разбиться.»',
		},
		{
			sentence: '“But could you not be mended?” asked the girl.',
			translate: '«А разве тебя нельзя починить?» — спросила девочка.',
		},
		{
			sentence:
				'“Oh, yes; but one is never so pretty after being mended, you know,” replied the Princess.',
			translate:
				'«О, можно; но после починки уже никогда не бывает столь красивой, ты знаешь», — ответила Принцесса.',
		},
		{
			sentence: '“I suppose not,” said Dorothy.',
			translate: '«Полагаю, что так», — сказала Дороти.',
		},
		{
			sentence:
				'“Now there is Mr. Joker, one of our clowns,” continued the china lady, “who is always trying to stand upon his head.',
			translate:
				'«Вот, например, мистер Джокер, один из наших клоунов, — продолжала фарфоровая леди, — он всё время пытается стоять на голове.',
		},
		{
			sentence:
				'He has broken himself so often that he is mended in a hundred places, and doesn’t look at all pretty.',
			translate:
				'Он ломал себя столько раз, что починен в сотне мест и совсем некрасиво выглядит.',
		},
		{
			sentence: 'Here he comes now, so you can see for yourself.”',
			translate: 'Вот он как раз идёт, сейчас сама увидишь.»',
		},
		{
			sentence:
				'Indeed, a jolly little clown came walking toward them, and Dorothy could see that in spite of his pretty clothes of red and yellow and green he was completely covered with cracks, running every which way and showing plainly that he had been mended in many places.',
			translate:
				'И правда, к ним подошёл весёлый маленький клоун, и Дороти увидела, что, несмотря на красивые красно‑жёлто‑зелёные одежды, он весь был покрыт трещинами во все стороны — явный знак того, что его много где исправляли.',
		},
		{
			sentence:
				'The Clown put his hands in his pockets, and after puffing out his cheeks and nodding his head at them saucily, he said: “My lady fair, why do you stare at poor old Mr. Joker? You’re quite as stiff and prim as if you’d eaten up a poker!”',
			translate:
				'Клоун сунул руки в карманы и, надув щёки и дерзко кивнув им, сказал: «Прекрасная леди, отчего вы так уставились на бедного старину мистера Джокера? Вы такие же чопорные и зажатые, будто проглотили кочергу!»',
		},
		{
			sentence: '“Be quiet, sir!” said the Princess.',
			translate: '«Помолчите, сэр!» — сказала Принцесса.',
		},
		{
			sentence: '“Can’t you see these are strangers, and should be treated with respect?”',
			translate: '«Разве не видите — это незнакомцы, и с ними следует обращаться почтительно?»',
		},
		{
			sentence: '“Well, that’s respect, I expect,” declared the Clown, and immediately stood upon his head.',
			translate: '«Ну, это и есть почтение, полагаю», — заявил Клоун и тут же встал на голову.',
		},
		{
			sentence: '“Don’t mind Mr. Joker,” said the Princess to Dorothy.',
			translate: '«Не обращай внимания на мистера Джокера», — сказала Принцесса Дороти.',
		},
		{
			sentence: '“He is considerably cracked in his head, and that makes him foolish.”',
			translate: '«У него голова изрядно треснула, вот он и глупит.»',
		},
		{
			sentence: '“Oh, I don’t mind him a bit,” said Dorothy.',
			translate: '«О, я совсем не против него», — сказала Дороти.',
		},
		{
			sentence:
				'“But you are so beautiful,” she continued, “that I am sure I could love you dearly.',
			translate:
				'«Но вы так прекрасны, — продолжила она, — что я уверена, полюбила бы вас всей душой.',
		},
		{
			sentence: 'Won’t you let me carry you back to Kansas, and stand you on Aunt Em’s mantel?',
			translate: 'Не позволите ли мне забрать вас в Канзас и поставить на каминную полку тёти Эм?',
		},
		{
			sentence: 'I could carry you in my basket.”',
			translate: 'Я могла бы нести вас в своей корзинке.»',
		},
		{
			sentence: '“That would make me very unhappy,” answered the china Princess.',
			translate: '«Это сделало бы меня очень несчастной», — ответила фарфоровая Принцесса.',
		},
		{
			sentence: '“You see, here in our country we live contentedly, and can talk and move around as we please.',
			translate: '«Видишь ли, здесь, в нашей стране, мы живём довольные и можем говорить и двигаться, как нам угодно.',
		},
		{
			sentence:
				'But whenever any of us are taken away our joints at once stiffen, and we can only stand straight and look pretty.',
			translate:
				'Но как только кого‑нибудь из нас увозят, наши суставы сразу деревенеют, и мы можем лишь стоять прямо и быть красивыми.',
		},
		{
			sentence:
				'Of course that is all that is expected of us when we are on mantels and cabinets and drawing-room tables, but our lives are much pleasanter here in our own country.”',
			translate:
				'Разумеется, на каминных полках, в шкафах и на столах гостиных от нас этого и ожидают, но здесь, в нашей стране, жизнь гораздо приятнее.»',
		},
		{
			sentence: '“I would not make you unhappy for all the world!” exclaimed Dorothy.',
			translate: '«Я ни за что на свете не стала бы делать тебя несчастной!» — воскликнула Дороти.',
		},
		{
			sentence: '“So I’ll just say good-bye.”',
			translate: '«Так что я просто скажу: прощай.»',
		},
		{
			sentence: '“Good-bye,” replied the Princess.',
			translate: '«Прощай», — ответила Принцесса.',
		},
		{
			sentence: 'They walked carefully through the china country.',
			translate: 'Они осторожно шли через фарфоровую страну.',
		},
		{
			sentence:
				'The little animals and all the people scampered out of their way, fearing the strangers would break them, and after an hour or so the travelers reached the other side of the country and came to another china wall.',
			translate:
				'Маленькие зверьки и все люди разбегались им с дороги, боясь, что незнакомцы их разобьют, и вскоре, через час или около того, путешественники достигли другой стороны страны и подошли к ещё одной фарфоровой стене.',
		},
		{
			sentence:
				'It was not so high as the first, however, and by standing upon the Lion’s back they all managed to scramble to the top.',
			translate:
				'Однако она была не такой высокой, как первая, и, встав на спину Льва, они сумели вскарабкаться на вершину.',
		},
		{
			sentence:
				'Then the Lion gathered his legs under him and jumped on the wall; but just as he jumped, he upset a china church with his tail and smashed it all to pieces.',
			translate:
				'Затем Лев подобрал под себя ноги и перепрыгнул через стену; но в прыжке он задел хвостом фарфоровую церковь и разбил её в дребезги.',
		},
		{
			sentence:
				'“That was too bad,” said Dorothy, “but really I think we were lucky in not doing these little people more harm than breaking a cow’s leg and a church.',
			translate:
				'«Это очень плохо, — сказала Дороти, — но, по правде говоря, нам повезло, что мы не причинили этим маленьким людям большего вреда, чем сломанная коровья нога и церковь.',
		},
		{
			sentence: 'They are all so brittle!”',
			translate: 'Они же такие хрупкие!»',
		},
		{
			sentence:
				'“They are, indeed,” said the Scarecrow, “and I am thankful I am made of straw and cannot be easily damaged.',
			translate:
				'«И правда, — сказал Пугало, — и я благодарен, что сделан из соломы и меня нелегко повредить.',
		},
		{
			sentence: 'There are worse things in the world than being a Scarecrow.”',
			translate: 'В мире есть вещи и похуже, чем быть Пугалом.»',
		},
	],
}
