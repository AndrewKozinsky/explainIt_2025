export const chapter_2 = {
	name: 'Chapter II',
	header: 'The Council with the Munchkins',
	data: [
		{
			sentence:
				'She was awakened by a shock, so sudden and severe that if Dorothy had not been lying on the soft bed she might have been hurt.',
			translate:
				'Её разбудил толчок, такой внезапный и сильный, что если бы Дороти не лежала на мягкой кровати, она могла бы ушибиться.',
		},
		{
			sentence:
				'As it was, the jar made her catch her breath and wonder what had happened; and Toto put his cold little nose into her face and whined dismally.',
			translate:
				'От толчка у неё перехватило дыхание, и она задумалась, что случилось; а Тото ткнулся холодным носиком ей в лицо и жалобно заскулил.',
		},
		{
			sentence:
				'Dorothy sat up and noticed that the house was not moving; nor was it dark, for the bright sunshine came in at the window, flooding the little room.',
			translate:
				'Дороти села и заметила, что дом не двигается; и было не темно, потому что яркий солнечный свет лился в окно, заливая маленькую комнату.',
		},
		{
			sentence: 'She sprang from her bed and with Toto at her heels ran and opened the door.',
			translate: 'Она вскочила с постели и, с Тото по пятам, подбежала и распахнула дверь.',
		},
		{
			sentence:
				'The little girl gave a cry of amazement and looked about her, her eyes growing bigger and bigger at the wonderful sights she saw.',
			translate:
				'Девочка вскрикнула от изумления и огляделась; её глаза становились всё больше от чудесных видов, которые она увидела.',
		},
		{
			sentence:
				'The cyclone had set the house down very gently—for a cyclone—in the midst of a country of marvelous beauty.',
			translate: 'Циклон опустил дом очень мягко — как для циклона — посреди страны изумительной красоты.',
		},
		{
			sentence:
				'There were lovely patches of greensward all about, with stately trees bearing rich and luscious fruits.',
			translate:
				'Всюду были милые зелёные лужайки, со стройными деревьями, усыпанными сочными и вкусными плодами.',
		},
		{
			sentence:
				'Banks of gorgeous flowers were on every hand, and birds with rare and brilliant plumage sang and fluttered in the trees and bushes.',
			translate:
				'Повсюду были гряды роскошных цветов, а птицы с редким и ярким оперением пели и порхали в деревьях и кустах.',
		},
		{
			sentence:
				'A little way off was a small brook, rushing and sparkling along between green banks, and murmuring in a voice very grateful to a little girl who had lived so long on the dry, gray prairies.',
			translate:
				'Неподалёку журчал и искрился небольшой ручей между зелёными берегами и шелестел голосом, очень приятным для девочки, так долго жившей на сухих серых прериях.',
		},
		{
			sentence:
				'While she stood looking eagerly at the strange and beautiful sights, she noticed coming toward her a group of the queerest people she had ever seen.',
			translate:
				'Пока она с жадным любопытством рассматривала странные и прекрасные виды, она заметила, как к ней идёт группа самых необычных людей, каких она когда-либо видела.',
		},
		{
			sentence:
				'They were not as big as the grown folk she had always been used to; but neither were they very small.',
			translate:
				'Они были не такие большие, как взрослые, к которым она привыкла; но и совсем маленькими их назвать нельзя.',
		},
		{
			sentence:
				'In fact, they seemed about as tall as Dorothy, who was a well-grown child for her age, although they were, so far as looks go, many years older.',
			translate:
				'На деле они были примерно ростом с Дороти, которая была довольно высокой для своего возраста, хотя на вид они казались на много лет старше.',
		},
		{
			sentence: 'Three were men and one a woman, and all were oddly dressed.',
			translate: 'Трое были мужчинами, а одна — женщиной, и одеты они были странно.',
		},
		{
			sentence:
				'They wore round hats that rose to a small point a foot above their heads, with little bells around the brims that tinkled sweetly as they moved.',
			translate:
				'На них были круглые шляпы, поднимавшиеся маленьким остриём на фут над головой, с колокольчиками по полям, которые сладко звенели, когда они двигались.',
		},
		{
			sentence:
				'The hats of the men were blue; the little woman’s hat was white, and she wore a white gown that hung in pleats from her shoulders.',
			translate:
				'Шляпы мужчин были голубыми; шляпка маленькой женщины — белой, и на ней было белое платье, ниспадавшее складками с плеч.',
		},
		{
			sentence: 'Over it were sprinkled little stars that glistened in the sun like diamonds.',
			translate: 'Поверх него были рассыпаны маленькие звёздочки, которые на солнце сверкали, как бриллианты.',
		},
		{
			sentence:
				'The men were dressed in blue, of the same shade as their hats, and wore well-polished boots with a deep roll of blue at the tops.',
			translate:
				'Мужчины были одеты в голубое, в тон шляпам, и носили хорошо начищенные сапоги с высоким голубым отворотом сверху.',
		},
		{
			sentence: 'The men, Dorothy thought, were about as old as Uncle Henry, for two of them had beards.',
			translate: 'Мужчины, подумала Дороти, были примерно возраста дяди Генри, потому что у двоих были бороды.',
		},
		{
			sentence: 'But the little woman was doubtless much older.',
			translate: 'Но маленькая женщина, несомненно, была гораздо старше.',
		},
		{
			sentence: 'Her face was covered with wrinkles, her hair was nearly white, and she walked rather stiffly.',
			translate: 'Её лицо было покрыто морщинами, волосы почти белые, и ходила она довольно-таки неуклюже.',
		},
		{
			sentence:
				'When these people drew near the house where Dorothy was standing in the doorway, they paused and whispered among themselves, as if afraid to come farther.',
			translate:
				'Когда эти люди подошли к дому, где Дороти стояла в дверях, они остановились и зашептались между собой, будто боялись подойти ближе.',
		},
		{
			sentence:
				'But the little old woman walked up to Dorothy, made a low bow and said, in a sweet voice: “You are welcome, most noble Sorceress, to the land of the Munchkins.',
			translate:
				'Но маленькая старушка подошла к Дороти, низко поклонилась и ласково сказала: «Добро пожаловать, благороднейшая Волшебница, в страну Мункинов.',
		},
		{
			sentence:
				'We are so grateful to you for having killed the Wicked Witch of the East, and for setting our people free from bondage.” Dorothy listened to this speech with wonder.',
			translate:
				'«Мы бесконечно благодарны тебе за то, что ты убила Злую Ведьму Востока и освободила наш народ от рабства». Дороти с удивлением слушала эту речь.',
		},
		{
			sentence:
				'What could the little woman possibly mean by calling her a sorceress, and saying she had killed the Wicked Witch of the East?',
			translate:
				'Что могла иметь в виду эта маленькая женщина, называя её волшебницей и говоря, что она убила Злую Ведьму Востока?',
		},
		{
			sentence:
				'Dorothy was an innocent, harmless little girl, who had been carried by a cyclone many miles from home; and she had never killed anything in all her life.',
			translate:
				'Дороти была невинной, безобидной девочкой, которую циклон унёс на многие мили от дома; она никогда в жизни никого не убивала.',
		},
		{
			sentence:
				'But the little woman evidently expected her to answer; so Dorothy said, with hesitation, “You are very kind, but there must be some mistake.',
			translate:
				'Но маленькая женщина явно ожидала от неё ответа, и Дороти с нерешительностью сказала: «Вы очень добры, но, должно быть, какая‑то ошибка.',
		},
		{
			sentence:
				'I have not killed anything.” “Your house did, anyway,” replied the little old woman, with a laugh, “and that is the same thing.',
			translate:
				'«Я никого не убивала». — «Зато твой дом убил, — ответила старушка со смехом, — а это то же самое.',
		},
		{
			sentence: 'See!” she continued, pointing to the corner of the house.',
			translate: '«Смотри!» — продолжала она, указывая на угол дома.',
		},
		{
			sentence:
				'“There are her two feet, still sticking out from under a block of wood.” Dorothy looked, and gave a little cry of fright.',
			translate: '«Вот её две ноги, всё ещё торчат из‑под бруса». Дороти взглянула и тихо вскрикнула от испуга.',
		},
		{
			sentence:
				'There, indeed, just under the corner of the great beam the house rested on, two feet were sticking out, shod in silver shoes with pointed toes.',
			translate:
				'И правда, прямо под углом могучей балки, на которой держался дом, торчали две ноги, обутые в серебряные башмачки с острыми носами.',
		},
		{
			sentence: '“Oh, dear!',
			translate: '«О боже!',
		},
		{
			sentence: 'Oh, dear!” cried Dorothy, clasping her hands together in dismay.',
			translate: '«О боже!» — воскликнула Дороти, всплеснув руками от ужаса.',
		},
		{
			sentence: '“The house must have fallen on her.',
			translate: '«Дом, должно быть, упал на неё.',
		},
		{
			sentence: 'Whatever shall we do?” “There is nothing to be done,” said the little woman calmly.',
			translate: '«Что же нам делать?» — «Ничего уже не поделаешь», — спокойно сказала маленькая женщина.',
		},
		{
			sentence: '“But who was she?” asked Dorothy.',
			translate: '«Но кто она?» — спросила Дороти.',
		},
		{
			sentence: '“She was the Wicked Witch of the East, as I said,” answered the little woman.',
			translate: '«Это была Злая Ведьма Востока, как я и сказала», — ответила старушка.',
		},
		{
			sentence:
				'“She has held all the Munchkins in bondage for many years, making them slave for her night and day.',
			translate: '«Она держала всех мункинов в неволе много лет, заставляя их день и ночь работать на неё.',
		},
		{
			sentence:
				'Now they are all set free, and are grateful to you for the favor.” “Who are the Munchkins?” inquired Dorothy.',
			translate:
				'«Теперь они все свободны и благодарны тебе за эту милость». — «Кто такие мункины?» — спросила Дороти.',
		},
		{
			sentence:
				'“They are the people who live in this land of the East where the Wicked Witch ruled.” “Are you a Munchkin?” asked Dorothy.',
			translate:
				'«Это люди, живущие в этой восточной стране, где правила Злая Ведьма». — «А вы — мункин?» — спросила Дороти.',
		},
		{
			sentence: '“No, but I am their friend, although I live in the land of the North.',
			translate: '«Нет, но я их подруга, хотя живу в стране Севера.',
		},
		{
			sentence:
				'When they saw the Witch of the East was dead the Munchkins sent a swift messenger to me, and I came at once.',
			translate:
				'Когда они увидели, что Ведьма Востока мертва, мункины отправили ко мне быстрого гонца, и я пришла сразу.',
		},
		{
			sentence: 'I am the Witch of the North.” “Oh, gracious!” cried Dorothy.',
			translate: '«Я — Ведьма Севера». — «Ах, милость небес!» — воскликнула Дороти.',
		},
		{
			sentence: '“Are you a real witch?” “Yes, indeed,” answered the little woman.',
			translate: '«Вы настоящая ведьма?» — «Да, разумеется», — ответила маленькая женщина.',
		},
		{
			sentence: '“But I am a good witch, and the people love me.',
			translate: '«Но я добрая ведьма, и люди меня любят.',
		},
		{
			sentence:
				'I am not as powerful as the Wicked Witch was who ruled here, or I should have set the people free myself.” “But I thought all witches were wicked,” said the girl, who was half frightened at facing a real witch.',
			translate:
				'«Я не настолько могущественна, как Злая Ведьма, что правила здесь, иначе я бы сама освободила людей». — «А я думала, все ведьмы злые», — сказала девочка, наполовину испуганная встречей с настоящей ведьмой.',
		},
		{
			sentence: '“Oh, no, that is a great mistake.',
			translate: '«О нет, это большое заблуждение.',
		},
		{
			sentence:
				'There were only four witches in all the Land of Oz, and two of them, those who live in the North and the South, are good witches.',
			translate:
				'Во всей Стране Оз было всего четыре ведьмы, и две из них — те, что живут на Севере и на Юге, — добрые.',
		},
		{
			sentence: 'I know this is true, for I am one of them myself, and cannot be mistaken.',
			translate: 'Я знаю, что это так, потому что сама одна из них и не могу ошибаться.',
		},
		{
			sentence:
				'Those who dwelt in the East and the West were, indeed, wicked witches; but now that you have killed one of them, there is but one Wicked Witch in all the Land of Oz—the one who lives in the West.” “But,” said Dorothy, after a moment’s thought, “Aunt Em has told me that the witches were all dead—years and years ago.” “Who is Aunt Em?” inquired the little old woman.',
			translate:
				'Те, что жили на Востоке и Западе, действительно были злыми; но раз ты уничтожила одну из них, в Стране Оз осталась лишь одна Злая Ведьма — та, что живёт на Западе». — «Но, — сказала Дороти, немного подумав, — тётя Эм говорила, что все ведьмы давно умерли, много‑много лет назад». — «Кто такая тётя Эм?» — спросила маленькая старушка.',
		},
		{
			sentence:
				'“She is my aunt who lives in Kansas, where I came from.” The Witch of the North seemed to think for a time, with her head bowed and her eyes upon the ground.',
			translate:
				'«Это моя тётя, она живёт в Канзасе, откуда я приехала». Ведьма Севера ненадолго задумалась, опустив голову и уставив взгляд в землю.',
		},
		{
			sentence:
				'Then she looked up and said, “I do not know where Kansas is, for I have never heard that country mentioned before.',
			translate:
				'Потом подняла глаза и сказала: «Я не знаю, где находится Канзас, никогда прежде не слышала о такой стране.',
		},
		{
			sentence: 'But tell me, is it a civilized country?” “Oh, yes,” replied Dorothy.',
			translate: '«Но скажи, это цивилизованная страна?» — «О да», — ответила Дороти.',
		},
		{
			sentence: '“Then that accounts for it.',
			translate: '«Тогда всё понятно.',
		},
		{
			sentence:
				'In the civilized countries I believe there are no witches left, nor wizards, nor sorceresses, nor magicians.',
			translate: 'В цивилизованных странах, я думаю, ведьм, волшебников, чародеек и магов уже не осталось.',
		},
		{
			sentence:
				'But, you see, the Land of Oz has never been civilized, for we are cut off from all the rest of the world.',
			translate: 'Но Страна Оз никогда не была цивилизованной, ведь мы отрезаны от всего остального мира.',
		},
		{
			sentence: 'Therefore we still have witches and wizards amongst us.” “Who are the wizards?” asked Dorothy.',
			translate:
				'Поэтому у нас до сих пор живут ведьмы и волшебники». — «Кто такие волшебники?» — спросила Дороти.',
		},
		{
			sentence: '“Oz himself is the Great Wizard,” answered the Witch, sinking her voice to a whisper.',
			translate: '«Сам Оз — Великий Волшебник, — ответила Ведьма, понижая голос до шёпота.',
		},
		{
			sentence: '“He is more powerful than all the rest of us together.',
			translate: '«Он могущественнее всех нас вместе взятых.',
		},
		{
			sentence:
				'He lives in the City of Emeralds.” Dorothy was going to ask another question, but just then the Munchkins, who had been standing silently by, gave a loud shout and pointed to the corner of the house where the Wicked Witch had been lying.',
			translate:
				'Он живёт в Изумрудном городе». Дороти хотела задать ещё вопрос, но в этот момент мункины, которые до тех пор стояли молча, громко закричали и указали на угол дома, где лежала Злая Ведьма.',
		},
		{
			sentence: '“What is it?” asked the little old woman, and looked, and began to laugh.',
			translate: '«Что такое?» — спросила старушка, посмотрела и расхохоталась.',
		},
		{
			sentence: 'The feet of the dead Witch had disappeared entirely, and nothing was left but the silver shoes.',
			translate: 'Ноги мёртвой Ведьмы исчезли без следа, и остались только серебряные башмачки.',
		},
		{
			sentence: '“She was so old,” explained the Witch of the North, “that she dried up quickly in the sun.',
			translate: '«Она была такая старая, — объяснила Ведьма Севера, — что быстро высохла на солнце.',
		},
		{
			sentence: 'That is the end of her.',
			translate: 'Вот и конец ей.',
		},
		{
			sentence:
				'But the silver shoes are yours, and you shall have them to wear.” She reached down and picked up the shoes, and after shaking the dust out of them handed them to Dorothy.',
			translate:
				'А серебряные башмачки — твои, и ты будешь их носить». Она наклонилась, подняла башмачки и, вытряхнув из них пыль, вручила их Дороти.',
		},
		{
			sentence:
				'“The Witch of the East was proud of those silver shoes,” said one of the Munchkins, “and there is some charm connected with them; but what it is we never knew.” Dorothy carried the shoes into the house and placed them on the table.',
			translate:
				'«Ведьма Востока очень гордилась этими серебряными башмачками, — сказал один из мункинов, — и в них заключено какое‑то колдовство; вот только какое — мы так и не узнали». Дороти унесла башмачки в дом и положила их на стол.',
		},
		{
			sentence:
				'Then she came out again to the Munchkins and said: “I am anxious to get back to my aunt and uncle, for I am sure they will worry about me.',
			translate:
				'Потом она снова вышла к мункинам и сказала: «Я очень хочу вернуться к тёте и дяде — наверняка они тревожатся обо мне.',
		},
		{
			sentence:
				'Can you help me find my way?” The Munchkins and the Witch first looked at one another, and then at Dorothy, and then shook their heads.',
			translate:
				'«Поможете ли вы мне найти дорогу?» Мункины и Ведьма сперва переглянулись, потом посмотрели на Дороти и покачали головами.',
		},
		{
			sentence:
				'“At the East, not far from here,” said one, “there is a great desert, and none could live to cross it.” “It is the same at the South,” said another, “for I have been there and seen it.',
			translate:
				'«На востоке, недалеко отсюда, — сказал один, — огромная пустыня, и никто не сможет её пересечь и остаться в живых». — «На юге то же самое, — сказал другой, — я там был и видел.',
		},
		{
			sentence:
				'The South is the country of the Quadlings.” “I am told,” said the third man, “that it is the same at the West.',
			translate: '«Юг — это страна Квадлингов». — «А на западе, — сказал третий, — говорят, всё так же.',
		},
		{
			sentence:
				'And that country, where the Winkies live, is ruled by the Wicked Witch of the West, who would make you her slave if you passed her way.” “The North is my home,” said the old lady, “and at its edge is the same great desert that surrounds this Land of Oz.',
			translate:
				'А той страной, где живут Винки, правит Злая Ведьма Запада; если ты пойдёшь её дорогой, она сделает тебя своей рабыней». — «Север — мой дом, — сказала старушка, — а на его окраине лежит та же большая пустыня, что окружает Страну Оз.',
		},
		{
			sentence:
				'I’m afraid, my dear, you will have to live with us.” Dorothy began to sob at this, for she felt lonely among all these strange people.',
			translate:
				'«Боюсь, милая, тебе придётся жить с нами». От этих слов Дороти расплакалась, потому что чувствовала себя одиноко среди всех этих незнакомых людей.',
		},
		{
			sentence:
				'Her tears seemed to grieve the kind-hearted Munchkins, for they immediately took out their handkerchiefs and began to weep also.',
			translate:
				'Её слёзы, казалось, огорчили добрых мункинов, и они тотчас же достали платочки и тоже принялись вытирать глаза.',
		},
		{
			sentence:
				'As for the little old woman, she took off her cap and balanced the point on the end of her nose, while she counted “One, two, three” in a solemn voice.',
			translate:
				'Что до маленькой старушки, то она сняла свой колпак и уравновесила кончик на кончике носа, чинно считая: «Раз, два, три».',
		},
		{
			sentence:
				'At once the cap changed to a slate, on which was written in big, white chalk marks: “LET DOROTHY GO TO THE CITY OF EMERALDS”. The little old woman took the slate from her nose, and having read the words on it, asked, “Is your name Dorothy, my dear?” “Yes,” answered the child, looking up and drying her tears.',
			translate:
				'И тут же колпак превратился в грифельную дощечку, на которой крупными белыми буквами было написано: «ПУСТЬ ДОРОТИ ИДЁТ В ИЗУМРУДНЫЙ ГОРОД». Старушка сняла дощечку с носа и, прочитав написанное, спросила: «Твоё имя Дороти, милая?» — «Да», — ответила девочка, подняв глаза и вытирая слёзы.',
		},
		{
			sentence: '“Then you must go to the City of Emeralds.',
			translate: '«Тогда ты должна отправиться в Изумрудный город.',
		},
		{
			sentence: 'Perhaps Oz will help you.” “Where is this city?” asked Dorothy.',
			translate: '«Возможно, Оз тебе поможет». — «Где находится этот город?» — спросила Дороти.',
		},
		{
			sentence:
				'“It is exactly in the center of the country, and is ruled by Oz, the Great Wizard I told you of.” “Is he a good man?” inquired the girl anxiously.',
			translate:
				'«Он точно в центре страны, и им правит Оз, Великий Волшебник, о котором я тебе говорила». — «Он хороший человек?» — с тревогой спросила девочка.',
		},
		{
			sentence: '“He is a good Wizard.',
			translate: '«Он хороший Волшебник.',
		},
		{
			sentence:
				'Whether he is a man or not I cannot tell, for I have never seen him.” “How can I get there?” asked Dorothy.',
			translate:
				'«А человек он или нет — сказать не могу: я его никогда не видела». — «Как же мне туда добраться?» — спросила Дороти.',
		},
		{
			sentence: '“You must walk.',
			translate: '«Тебе придётся идти пешком.',
		},
		{
			sentence:
				'It is a long journey, through a country that is sometimes pleasant and sometimes dark and terrible.',
			translate: 'Это длинный путь через страну, которая местами приветлива, а местами мрачна и страшна.',
		},
		{
			sentence:
				'However, I will use all the magic arts I know of to keep you from harm.” “Won’t you go with me?” pleaded the girl, who had begun to look upon the little old woman as her only friend.',
			translate:
				'Однако я употреблю все известные мне чары, чтобы уберечь тебя от бед». — «А вы не пойдёте со мной?» — умоляла девочка, которая уже начала видеть в маленькой старушке свою единственную подругу.',
		},
		{
			sentence:
				'“No, I cannot do that,” she replied, “but I will give you my kiss, and no one will dare injure a person who has been kissed by the Witch of the North.”',
			translate:
				'«Нет, я не могу этого сделать, — ответила она, — но я дам тебе свой поцелуй, и никто не посмеет обидеть человека, которого поцеловала Ведьма Севера».',
		},
		{
			sentence: 'She came close to Dorothy and kissed her gently on the forehead.',
			translate: 'Она подошла к Дороти и нежно поцеловала её в лоб.',
		},
		{
			sentence:
				'Where her lips touched the girl they left a round, shining mark, as Dorothy found out soon after.',
			translate:
				'В том месте, где её губы коснулись девочки, остался круглый, сияющий знак, как вскоре увидела Дороти.',
		},
		{
			sentence:
				'“The road to the City of Emeralds is paved with yellow brick,” said the Witch, “so you cannot miss it.',
			translate:
				'«Дорога в Изумрудный город вымощена жёлтым кирпичом, — сказала Ведьма, — так что ты её не потеряешь.',
		},
		{
			sentence: 'When you get to Oz do not be afraid of him, but tell your story and ask him to help you.',
			translate: 'Когда придёшь к Озу, не бойся его, расскажи свою историю и попроси о помощи.',
		},
		{
			sentence:
				'Good-bye, my dear.” The three Munchkins bowed low to her and wished her a pleasant journey, after which they walked away through the trees.',
			translate:
				'«Прощай, милая». Трое мункинов низко поклонились ей и пожелали приятного пути, после чего ушли в лес.',
		},
		{
			sentence:
				'The Witch gave Dorothy a friendly little nod, whirled around on her left heel three times, and straightway disappeared, much to the surprise of little Toto, who barked after her loudly enough when she had gone, because he had been afraid even to growl while she stood by.',
			translate:
				'Ведьма дружелюбно кивнула Дороти, трижды повернулась на левой пятке и тотчас исчезла, к большому удивлению маленького Тото, который только тогда принялся громко лаять ей вслед, потому что пока она была рядом, он даже рычать боялся.',
		},
		{
			sentence:
				'But Dorothy, knowing her to be a witch, had expected her to disappear in just that way, and was not surprised in the least.',
			translate: 'Но Дороти, зная, что она ведьма, как раз такого исчезновения и ожидала и ничуть не удивилась.',
		},
	],
}
