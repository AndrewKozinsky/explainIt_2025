...
export class UniversalPhraseRepository {
	...
	/*@CatchDbError()
	async createUniversalPhrase(inputs: { text: string; sourceLanguageCode: LanguageCode }) {
		return await this.prisma.universalPhrase.create({
			data: {
				text: normalizeSentence(inputs.text),
				source_language_code: inputs.sourceLanguageCode,
			},
		})
	}*/
