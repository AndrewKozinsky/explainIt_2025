...
### Повторное добавление одной фразы разрешено
Уникального индекса `(user_id, language_code, phrase)` нет: одно и то же слово в разных предложениях может иметь разные переводы и контексты, и пользователь вправе сохранить каждый случай отдельной карточкой.
...
- **Mutation** `flashcard_add(input: { sentencePhraseTranslationId })` → `FlashcardOutModel`. Создаёт карточку из существующего разбора фразы.
...
2. Загружает `Sentence` со связями (`bookChapter` + `book`/`book_public`, `videoPrivate`, `videoPublic`).
3. Вызывает приватный `resolveSentenceSource(sentence, userId)`, который:
...
4. Нарезает `sentenceText = sourceFullText.slice(sentence.start_offset, sentence.start_offset + sentence.length)`.
5. Пересчитывает оффсеты фразы: `phrase_offset_in_sentence = phrase_offset_in_chapter - sentence.start_offset`.
6. Загружает первый доступный перевод предложения (`SentenceTranslationRepository.getFirstSentenceTranslationBySentenceId`).
7. Создаёт `Flashcard` и возвращает её через `FlashcardQueryRepository.getFlashcardById` (чтобы сразу получить и `phraseTranscription`).
...
2. Получает готовую `FlashcardOutModel` и добавляет её в локальный список карточек / показывает подтверждение.
...
### Удаление карточки
Кнопка «Удалить» → `flashcard_remove({ flashcardId })` → убрать из локального списка.
