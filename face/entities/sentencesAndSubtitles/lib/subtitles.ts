...
export function findActiveSubtitleIndex(subtitles: Subtitle[], time: number): number {
	for (let i = 0; i < subtitles.length; i++) {
		if (time >= subtitles[i].fromSeconds && time < subtitles[i].toSeconds) return i
	}
...
export function findPrevSubtitleIndex(subtitles: Subtitle[], time: number): number {
	let result = -1

	for (let i = 0; i < subtitles.length; i++) {
		if (subtitles[i].toSeconds <= time) result = i
		else break
	}
