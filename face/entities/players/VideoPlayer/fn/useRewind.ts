...
/*export function useRewind(
	...
	const { sendCommand } = usePlayerContext()
	...
	useGetHotKeysHandler({
		...
		handler(e) {
			...
			sendCommand({ type: 'REWIND', seconds: -seconds })
		},
	})
	...
	useGetHotKeysHandler({
		...
		handler(e) {
			e.preventDefault()
			const seconds = e.shiftKey ? playerControlConfig.rewindSecondsShift : playerControlConfig.rewindSeconds
			sendCommand({ type: 'REWIND', seconds })
		},
	})
	...
	useEffect(() => {
		...
		function disableFastRateIfEnabled() {
			...
			if (longPressModeRef.current === 'forward2x') {
				sendCommand({ type: 'STOP_FORWARD_HOLD' })
			}
			if (longPressModeRef.current === 'reverse2x') {
				sendCommand({ type: 'STOP_REVERSE_SEEK' })
			}
		...
		function handlePointerDown(e: PointerEvent) {
			...
			longPressTimeoutIdRef.current = setTimeout(() => {
				...
				if (down.area === 'right') {
					...
					sendCommand({ type: 'START_FORWARD_HOLD', rate: playerControlConfig.forwardHoldSpeed })
				}
				if (down.area === 'left') {
					longPressModeRef.current = 'reverse2x'
					sendCommand({ type: 'START_REVERSE_SEEK', rate: playerControlConfig.forwardHoldSpeed })
				}
		...
		function handlePointerUp(e: PointerEvent) {
			...
			sendCommand({ type: 'REWIND', seconds: seconds * direction })
	...
	}, [playerWrapperRef, sendCommand, getPlayerWrapperPressArea, playerControlConfig])
}*/
