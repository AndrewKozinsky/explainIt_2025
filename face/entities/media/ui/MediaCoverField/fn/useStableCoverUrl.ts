 ...
 * Поэтому после перефетча медиа (например, после сохранения формы или работы с главами)
 * строка URL меняется, хотя картинка та же. Браузер считает это новым изображением
 * и загружает его заново — обложка мигает.
 *
 ...
 * @param coverUrl — текущий URL обложки из данных медиа
 */
export function useStableCoverUrl(coverIdentity: string | null, coverUrl: string | null): string | null {
	...
	// Картинка сменилась (загрузили новую обложку, удалили её или открыли другое медиа)
