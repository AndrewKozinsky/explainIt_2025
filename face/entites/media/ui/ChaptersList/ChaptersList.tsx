import React from 'react'
import cn from 'classnames'
import ChapterLink from '@/entites/media/ui/ChapterLink/ChapterLink'
import Paragraph from '@/shared/ui/Paragraph/Paragraph'
import './ChaptersList.scss'

type ChaptersListProps = {
	chapters: ChaptersListItem[]
	extraClass?: string
	emptyText?: string
}

type ChaptersListItem = {
	name?: null | string
	subName?: null | string
	href: string
}

function ChaptersList(props: ChaptersListProps) {
	const { chapters, extraClass, emptyText = 'Нет ни одной главы' } = props

	if (!chapters.length) {
		return <Paragraph fontSize={16}>{emptyText}</Paragraph>
	}

	return (
		<div className={cn('chapters-list', extraClass)}>
			{chapters.map((chapter) => {
				return <ChapterLink {...chapter} key={chapter.href} />
			})}
		</div>
	)
}

export default ChaptersList
